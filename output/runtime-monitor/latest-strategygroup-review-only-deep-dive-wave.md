...
- Deep-dive artifacts: `6`
- Owner policy confirmation required: 是
- Runtime Owner intervention required: 否
- Output: `output/runtime-monitor/latest-strategygroup-review-only-deep-dive-wave.json`
...
| `CPM-RO-001` | CPM-RO has meaningful signal evidence, but the quality is mixed relative to MI and its family boundary is unresolved. Observation asset plus merge review is the safer next policy direction. | `keep_observation_asset_run_merge_review_before_independent_admission` | `false` |
