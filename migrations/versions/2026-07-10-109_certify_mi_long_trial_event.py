"""Certify MI-LONG v2 as a trial-grade event.
...
GROUP_ID = "MI-001"
VERSION_V1 = "sgv:MI-001:v1"
VERSION_V2 = "sgv:MI-001:v2"
EVENT_V1 = "event_spec:MI-001:MI-LONG:v1"
EVENT_V2 = "event_spec:MI-001:MI-LONG:v2"
MIGRATION_AT_MS = 1783648800000
...
def upgrade() -> None:
    ...
    if not version_v1 or not event_v1:
        raise RuntimeError("MI-LONG v1 authority rows are required before migration 109")
    ...
    version_v2.update(
        {
            ...
            "evidence_refs": _append_unique(
                ...
                "migration:109:MI-LONG-v2-comparative-strength-contract",
            ),
    ...
    if not old_contracts:
        raise RuntimeError("MI v1 RequiredFact contracts are required before migration 109")
    for old_contract in old_contracts:
        fact_key = str(old_contract["fact_key"])
        payload = _json_dict(old_contract.get("definition_payload"))
        if fact_key == "relative_strength_confirmed":
    ...
    if len(old_facts) != 3:
        raise RuntimeError("MI-LONG v1 requires exactly three RequiredFacts")
    ...
    if not old_policy:
        raise RuntimeError("MI-LONG v1 execution policy is required before migration 109")
    ...
    if len(old_bindings) != 3:
        raise RuntimeError("MI-LONG v1 requires exactly three active candidate bindings")
    ...
    conn.execute(
        versions.update().where(versions.c.strategy_group_version_id == VERSION_V1).values(status="superseded")
    )
    ...
    conn.execute(
        versions.update().where(versions.c.strategy_group_version_id == VERSION_V2).values(status="current")
    )
...
def downgrade() -> None:
    ...
    if conn.execute(
        ...
        raise RuntimeError("cannot downgrade migration 109 while MI-LONG v2 signals exist")
