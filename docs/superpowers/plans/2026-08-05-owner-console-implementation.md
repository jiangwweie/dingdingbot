...
- Verification is risk-layered. Each task runs only focused tests for changed behavior, one related module-level regression when integration risk justifies it, and necessary Ruff, Mypy, typecheck, build, or `git diff --check` checks. Small fix rounds do not repeat an unchanged module or full-suite matrix, and one semantic rule is not duplicated across unit, integration, and architecture layers without a distinct risk. Security, database boundaries, identity consistency, and financial semantics retain high-risk focused coverage. Full Owner Console and whole-branch verification is reserved for cross-module risk, Task 24 local acceptance, and the final branch review. Reviewers reuse the implementer's recorded test evidence and run an additional test only for a concrete unanswered doubt.
