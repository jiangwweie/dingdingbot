...
### 2026-04-29 补充：策略研究下一阶段计划（P0/R1b 后）

经过 P0 official 验证和 R1b 资金参数二次审计，当前策略研究主线更新为：

1. **Pinbar 稳健线：风险因子从硬过滤转向降权**
   - P0 证明 `donchian_distance` 是真实风险因子，但固定阈值硬过滤会牺牲 2024/2025 主要收益。
   - 下一步不直接推进 `Pinbar(E4) + T1`，先做 `P0a 被过滤交易质量切片`。
   - 目标：判断 E4 应作为“跳过信号”还是“降低仓位 / 调整退出”的状态标签。

2. **资金管理线：分离稳健约束与激励上限**
   - R1b 证明在 `MaxDD <= 35%` 下存在可行配置，但收益不够激励。
   - 当前推荐稳健配置仅作为准入参考：`exposure=1.25, risk=0.5%`。
   - 下一步设计 R2：激励型资金上限实验，不再用单一 `MaxDD <= 35%` 压制全部目标。

3. **趋势补充线：T1 暂缓组合，先处理 fragility**
   - T1 仍是理想的“大趋势收益来源”候选，但此前组合对 top winners 依赖较强。
   - 在 Pinbar(E4) 硬过滤失败后，不直接复验组合。
   - 后续先研究 T1 的样本外稳定性和大赢家依赖，再考虑组合。

4. **暂缓方向**
   - 暂不继续 Engulfing entry 搜索。
   - 暂不继续 1h Donchian / 简单突破搜索。
   - 暂不把 E4 写入 runtime profile。

#### 下一步优先级

1. `P0a`: E4 skipped trades attribution，按年份拆解被过滤交易是否为 2024/2025 主要收益来源。
2. `R2`: 激励型资金上限实验设计，输出“最佳可能性”但明确不同于实盘准入。
3. `R1c`: 若需要，将 `debug_equity_curve` 口径固化为资金管理实验标准。
4. `T1b`: T1 fragility/OOS 复核，判断是否继续作为组合候选。
