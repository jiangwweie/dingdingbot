...
| # | 任务 | 优先级 | 状态 | 依赖 |
|---|------|--------|------|------|
| 1 | 旧页面功能迁移（Dry Run + Apply）+ 删除旧路由 | P0 | 🔓 待启动 | 无 |
| 2 | 修复策略下发断裂，统一走 apply API | P0 | 🔓 待启动 | Task 1 |
| 3 | 移除 MTF 冗余映射配置 | P1 | 🔓 待启动 | 无 |
| 4 | 策略详情预览（卡片添加查看详情） | P1 | 🔓 待启动 | 无 |
| 5 | 回测页面一键导入已保存策略 | P0 | 🔓 待启动 | 无 |
| 6 | 修复前端 RiskConfig 类型与后端不匹配 | P0 | 🔓 待启动 | 无 |
| 7 | 修复 YAML 全局 Decimal 构造器劫持 | P0 | 🔓 待启动 | 无 |
| 8 | 修复热重载缓存未刷新 | P1 | 🔓 待启动 | 无 |
