...
**订单入库需求澄清**:
- ✅ 不改动现有表
- ✅ 不复用现有表 (signals/signal_take_profits)
- ✅ 新建独立 `orders` 表

**OrderORM 状态**:
- 位置：`src/infrastructure/v3_orm.py` L396-514
- 状态：类已定义，表已创建 (Alembic migration 002)
- 需补充字段：`filled_at`, `parent_order_id`

**完整任务清单**:
| ID | 任务名称 | 优先级 | 预计工时 | 状态 |
|----|----------|--------|----------|------|
| T1 | 修复 MTF 未来函数 | P0 | 1h | ✅ 已完成 |
| T2 | 修复止盈撮合 (添加滑点) | P0 | 1h | ☐ |
