...
> **Status**: ETH 1h LONG-only 最优配置已锁定，参数全局冻结

---

## 当前阶段总结（2026-04-22 00:05）

### 核心结论

**ETH 1h LONG-only 是唯一可行路径，最优配置已锁定，参数全局冻结。**
...
1. **ATR 过滤器冗余**：有无 ATR 结果完全相同，可移除
2. **2023 失败非出场问题**：测试 4 种出场变体全部恶化
3. **参数敏感性**：ema_period 高敏感，ATR 不敏感，distance 中等敏感
