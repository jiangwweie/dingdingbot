...
| ID | 任务名称 | 优先级 | 预计工时 | 状态 |
|----|----------|--------|----------|------|
| T3 | 创建 orders 表 Alembic 迁移 | P0 | 0.5h | ☐ 待开始 |
| T4 | 实现订单保存逻辑 | P0 | 2h | ✅ 已完成 |
| T5 | 创建 backtest_reports 表 | P0 | 1h | ☐ 待开始 |
| T6 | 实现回测报告保存 | P0 | 2h | ☐ 待开始 |

**T4 完成情况**:
- ✅ 扩展 `Order` 领域模型添加 `filled_at` 字段
- ✅ 扩展 `OrderORM` 和转换函数支持 `filled_at`
- ✅ 扩展 `OrderRepository` 支持 `filled_at` 字段保存和查询
- ✅ 添加标准接口方法：`save_order`, `get_order_detail`, `mark_order_filled`
- ✅ 在 `_run_v3_pms_backtest` 中集成订单保存逻辑
- ✅ 编写 5 个 SST 测试用例（全部通过）
- ✅ 测试覆盖：save_order, mark_order_filled, get_orders_by_signal, get_open_orders, parent_order_id 追踪

**T5 设计更新** (符合 3NF 原则):
- ✅ 新增 `strategy_snapshot` 字段 (Text, JSON) - 记录回测时的完整参数组合
- ✅ 新增 `parameters_hash` 字段 (String, indexed) - 用于自动调参聚类分析
- ✅ 新增 `strategy_version` 字段 (String) - 策略版本号
...
| T3 | 创建 orders 表 Alembic 迁移 | P0 | 0.5h | ☐ |
| T4 | 实现订单保存逻辑 | P0 | 2h | ✅ 已完成 |
| T5 | 创建 backtest_reports 表 | P0 | 1h | ☐ |
| T6 | 实现回测报告保存 | P0 | 2h | ☐ |
| T7 | 回测记录列表页面 | P0 | 3h | ☐ |
