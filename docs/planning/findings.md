...
**问题陈述**:
WebSocket K 线选择逻辑错误，系统处理 `ohlcv[-1]`（未收盘 K 线）进行形态检测，导致信号误判。

**技术发现**:

1. **CCXT Pro OHLCV 规范**
   - `candle[6]` 可能包含 `info` 字典
   - `info['x']` 字段表示 K 线收盘状态（`true` = 已收盘，`false` = 未收盘）
   - 部分交易所可能不支持 `x` 字段，需要后备机制

2. **时间戳推断机制**
   - 无 `x` 字段时，使用 `ohlcv[-2]`（前一根 K 线）作为已收盘数据
   - 通过时间戳变化检测 K 线收盘
   - 需要维护 `_candle_timestamps` 字典追踪每个品种/时间框架

3. **KlineData 模型扩展**
   - 新增 `info: Optional[Dict[str, Any]]` 字段保留交易所原始数据
   - `is_closed` 字段从硬编码改为动态计算

### Pinbar 最小波幅检查架构决策

**问题**: 开盘初期波幅极小时可能误判 Pinbar

**决策**: 使用动态百分比阈值而非固定值

| 方案 | 优点 | 缺点 | 选择 |
|------|------|------|------|
| 固定值 (如 0.5 USDT) | 实现简单 | ❌ 过滤低价格币种（如 DOGE） | ❌ |
| 动态百分比 (0.1%) | ✅ 适配所有价格级别 | 实现稍复杂 | ✅ **已选择** |

**公式**:
- 有 ATR: `min_required = atr * 0.1`
- 无 ATR: `min_required = close * 0.001`
...
**用户约束**:
- 单人使用系统
- 主要交易周期：1h/4h 中长线，偶尔 15m 短线
- 不存在多人并发修改配置场景

**技术评估**:

| 原并发问题 | 原优先级 | 实际评估 | 新优先级 |
|----------|---------|---------|---------|
| 全局状态依赖注入 | P1 | ❌ 单人使用，不存在并发 | P3 |
| 仓位同步竞态修复 | P1 | ❌ 1h/4h 低频交易，几乎无并发 | P3 |
| 多锁管理简化 | P2 | ❌ 单人使用，无竞态 | P3 |

**结论**: 并发复杂度大幅降低，节省 10h 工时

---

### 产品策略：最小交付版本（MVP）

**用户目标**: 先跑通 Pinbar 策略验证有效性

**MVP 范围调整**:

| 维度 | 完整版本 | 最小交付版本 | 节省 |
|------|---------|-------------|------|
| 支持策略 | Pinbar + Engulfing + Doji + Hammer | **仅 Pinbar** | -3 策略 |
| 形态测试 | 所有形态全覆盖 | **Pinbar 全覆盖** | -60% 工作量 |
| 总工时 | 55.5h | **33h** | **节省 22.5h** |

**阶段目标**:
```
阶段 1: 普通回测 → 验证 Pinbar 形态有效性
阶段 2: PMS 回测 → 验证 Pinbar 正收益
阶段 3: 模拟盘运行 → 检查系统问题
阶段 4: 实盘上线 → 正式交易
```
...
1. **回测数据对齐优化** ✅
   - **提交**: 36c7563 (2026-04-01)
   - **问题**: MTF 过滤器在回测中使用未收盘 K 线，导致未来函数问题
   - **修复**: 往前偏移 1 根 K 线，使用正确的时间判断逻辑
   - **测试**: 10 个单元测试，100% 覆盖率
   - **文档**: `docs/designs/t1-mtf-future-function-fix.md`

2. **ConfigRepository 修复** ✅
   - **提交**: 5afd6a5 (P1-5 项目)
   - **问题**: `export_to_yaml` 调用未定义函数 `_convert_decimals_to_str`
   - **修复**: 实现 `_convert_decimals_to_str` 函数，支持 Decimal 精度和 Pydantic 模型
   - **位置**: `src/application/config/config_parser.py:69`, `config_repository.py:1211-1244`
   - **验收**: export_to_yaml 功能正常，可导出配置

**影响**:
- 阶段 1 实际剩余工时：9.5h（节省 1.5h）
- 明日优先任务：Pinbar 单元测试补充 + 多品种组合回测
- 无功能回退
...
**创建日期**: 2026-04-07  
**审查负责人**: Code Reviewer Agent  
**状态**: ✅ 已完成（批准进入下一阶段）

### 审查范围

- `src/application/config/__init__.py` - 包初始化
- `src/application/config/models.py` - 数据模型导出
- `src/application/config/config_parser.py` - 核心实现
- `tests/unit/test_config_parser.py` - 测试代码

### 红线检查结果

| 检查项 | 状态 | 说明 |
|--------|------|------|
| 领域层纯净性 | ✅ | 仅导入 `yaml`（解析层职责允许） |
| Decimal 使用 | ✅ | 所有比率使用 `Decimal`，测试覆盖 20+ 位精度 |
| 类型安全 | ✅ | 方法签名使用具名 Pydantic 类 |
| 向后兼容 | ✅ | 复用 `config_manager.py` 模型，无重复定义 |

### 边界情况检查结果

| 检查项 | 状态 | 处理方式 |
|--------|------|----------|
| YAML 文件不存在 | ✅ | `FileNotFoundError` + 回退默认值 |
| YAML 语法错误 | ✅ | `yaml.YAMLError` + 回退默认值 |
| 空 YAML 文件 | ✅ | 返回空字典 `{}` |
| Decimal 精度 | ✅ | 字符串序列化保持精度 |
| Unicode 支持 | ✅ | `encoding='utf-8'` 读取 |

### 发现的问题

| 优先级 | 问题 | 位置 | 建议 |
|--------|------|------|------|
| P1 | 冗余导入 `mask_secret` | `config_parser.py:24` | 删除未使用导入 |
...
完整审查报告：`docs/reviews/p1_5_parser_layer_review.md`
