...
1. **EMA 距离已是更好的替代品**
   - EMA 距离小 = 横盘震荡 = K 线波幅小 = ATR 低
   - 两者高度相关，信息重叠
   - 如果 EMA 距离已过滤横盘信号，ATR 能额外过滤的非常有限

2. **过度过滤风险**
   - 当前：90 天 × 3 币种 = 40 笔交易，平均每月 4.4 笔/币种
   - 再加 ATR 过滤，信号可能降到 30 笔以下
   - 实盘空仓时间长，心理压力大，难判断策略是否失效

3. **何时值得做**
   - 如果未来回测发现：高频低质信号在特定市况下集中出现
   - ATR 才是针对性工具

**优先级**：
- ✅ 已完成：找到可行配置（双 TP + EMA 距离）
- ➡️ 下一步：实盘验证（保守仓位跑 1-2 个月）
- ⏳ ATR 优化：等实盘数据反馈后再决定
