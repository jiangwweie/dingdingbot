...
2. 当前剩余最真实的缺口已经从“逻辑没测”收缩为两类：
   - **模型语义缺口**：`PositionInfo` 缺少 `current_price` / `mark_price`，导致 runtime positions 与 `/api/v3/positions` 虽然已改成 PG-first，但来自交易所/快照的价格 enrich 仍不完整。
   - **PG repo 真库验证缺口**：`PgOrderRepository` 补齐了 `/api/v3/orders*` 所需方法，但目前覆盖主要是 API 层和 readmodel 层，仍缺真实 PostgreSQL 集成验证来证明 SQLAlchemy / ORM / 约束语义。
3. 这意味着当前窗口可以把“代码与单测层面的 execution PG 主线闭环”视为基本成立，但还不能把“真实 PG 语义已经被证实”也一起宣告完成。
