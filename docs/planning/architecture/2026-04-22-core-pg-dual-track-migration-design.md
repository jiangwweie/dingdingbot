...
### 2.5 迁移不是 1:1 搬表，迁移时同步收口坏设计

本轮 PG 迁移**不是**对 SQLite 历史表结构做机械复制。凡是纳入本轮迁移的核心表，都必须在迁移过程中同步收口其明显不合理设计，避免把 SQLite 的历史包袱原样带到 PG。

强制约束如下：

1. 不再延续 SQLite 中以 `TEXT` 存储价格、数量、金额的做法；PG 核心数值字段统一使用 `NUMERIC`
2. 不再继续 `TEXT / INTEGER / TIMESTAMP` 混用；同一波次迁移的核心表必须统一时间字段口径
3. 必须补齐最小必要约束，包括但不限于：
   - 非空约束
   - 正数/非负数校验
   - 状态/角色合法值约束
   - 必要的唯一索引
4. 必须按真实查询路径补索引，而不是简单照搬旧表索引
5. 若某个设计问题本轮故意不修，必须在实现说明中显式写明“为何暂不处理”
