...
1. 将 `src/infrastructure/pg_models.py` 与 `db_scripts` 中的核心 PG 基线对齐
2. 同步修正初版基线中的两类不一致：
   - `orders` 的状态 / 类型集合改为当前领域枚举真实值
   - `execution_intents` 的状态集合改为当前领域模型真实值（小写）
3. 保留 `blocked_message` 作为当前执行链兼容字段，避免迁移时丢失拦截上下文
4. 将 `PgExecutionIntentRepository` 切到 `JSONB` 结构化 payload 映射
5. 将 `PgPositionRepository` 的排序字段切到新的 `opened_at`
6. 对核心变更文件完成 `python3 -m py_compile` 语法检查

### 当前状态

- PG ORM 模型已开始对齐批准后的目标真源 schema
- PG repository 仍需继续补齐完整读写与连通性验证
- `positions` 目前仍是过渡 schema，尚未进入完整领域映射阶段

### 备注

- 本次未执行测试（按项目红线，测试前需用户确认）

## 2026-04-22 23:58 -- 已将“迁移时同步优化表设计”写入 PG 迁移约束

### 本次完成

1. 将“迁移不是 1:1 搬表，而是迁移时同步收口坏设计”写入 PG 双轨迁移设计稿
2. 在 `task_plan.md` 中补充 PG 迁移验收约束
3. 记录当前执行原则：
   - 迁移核心表时同步统一字段类型
   - 迁移核心表时同步统一时间口径
   - 迁移核心表时同步补最小必要约束与索引
...
- 后续 `orders / execution_intents / positions` 的 PG 迁移不再按 SQLite 表结构机械复制执行
- “迁移时修表”已经从口头约定升级为 planning 约束
...
## 2026-04-22 23:40 -- `signal-only advisory deployment` 已写入后续规划

### 本次完成

1. 将“仅检测信号 + 推荐止盈止损仓位 + 飞书 webhook 推送”的部署形态写入 planning 文档
2. 明确该方向当前仅作为 **后续规划候选项**
3. 明确建议推进方式：
   - 后续新开独立分支 / 工作区开发
   - 不与当前订单链路 / 对账主线混做
...
## 2026-04-22 10:35 -- 新回测前端进入后续规划：独立 `backtest-studio`，低优先级并行

### 本次完成

1. **检查了现有前端与文档记录**
   - 确认仓库内已有 PMS 回测页、回测报告页、回测组件与相关检查文档
...
3. **确定推进节奏**
   - 采用 B 方案：与订单对账主线并行
   - 但当前只进入“后续规划”层
   - 优先级不高，先做设计与静态壳子，不立即抢主线资源

### 当前状态

- 订单对账 / 执行链 / PG 迁移仍是主线
- `backtest-studio` 已进入 planning backlog
- 后续如启动，先做设计稿和 mock 驱动壳子，再接真实 API
...
## 2026-04-22 10:55 -- `backtest-studio` PRD 已产出
...
2. 文档已明确：
   - 项目定位：独立回测前端
   - 当前方案：B 方案
   - MVP 边界：只做回测，不承接实盘/订单/仓位/旧工作台职责
   - 推进节奏：设计 -> 静态壳子 -> API 接线

### 当前状态

- `backtest-studio` 已从“想法”进入“有文档定义的 backlog”
- 仍保持低优先级，不影响订单对账主线
...
## 2026-04-22 19:25 -- TP 数据边界已补入执行设计稿与 planning

### 本次完成

1. **确认现有 TP 相关数据分层**
   - `signal_take_profits` 继续保留为信号层建议目标位
   - 真实执行 `TP1 / TP2 / SL` 明确收口到 `orders` 订单链

2. **同步设计意图到 planning**
   - 在最小实盘执行主链设计稿中新增“信号层 TP vs 执行层 TP/SL”边界说明
   - 在 `task_plan.md` 中补充执行层设计决策
   - 在 `findings.md` 中补充该边界结论

### 当前状态

- 后续 PG first scope 不再考虑新增执行层 TP 专表
- 设计边界已明确：
...
## 2026-04-22 19:05 -- 最小实盘执行主链设计稿已落盘
...
1. **基于现有实现重新收敛执行层方向**
   - API 不纳入自动执行主链
   - 自动执行由信号触发
   - 继续复用 `OrderLifecycleService`
   - 新增薄的 `ExecutionOrchestrator`

2. **形成最小实盘执行主链设计稿**
   - 目标与非目标
   - 模块职责边界
   - 最小状态流
   - `entry_filled_unprotected` 危险空窗状态
   - `recovery_required` 的第一版处理原则
   - 当前已知代码风险
   - 第一版实现顺序与验收标准

3. **保证重开上下文可恢复设计意图**
   - 设计稿已写入 planning/architecture
   - task_plan / findings / progress 已同步摘要
...
## 2026-04-22 18:10 -- 阶段总结已收口：主线冻结 + 环境边界明确

### 本次完成

1. **补齐 2021 / 2022 / 2026 Q1 验证**
   - `2021`：严重失效
   - `2022`：轻度失效
   - `2026 Q1`：forward check 继续盈利，且 Sharpe / 回撤表现优秀
...
3. **完成两个简单不开仓条件候选的验证**
   - `ema_distance_1h`：解释力存在，但不可执行
   - `4h` 结构连续性：无解释力
   - 两者均不进入下一轮正式策略验证

### 当前状态

- 主线已冻结：`ETH 1h LONG-only`
- 环境边界已明确：`2024/2025/2026 Q1` 适配，`2021/2022/2023` 不适配或边缘
- 当前更适合进入“阶段性实盘候选基线”整理，而不是继续补简单 filter
...
## 2026-04-22 12:10 -- 2023 失效归因口径修正完成

### 本次完成

1. **修正 `TP1=0%` 的错误口径**
   - 旧脚本错误地使用 `Position.exit_reason` 统计 `TP1/TP2/SL`
   - 已确认该字段只表示整笔仓位最终结束原因，不能统计分批止盈事件
   - 修正后改用 `report.close_events` 作为唯一事件源
...
3. **确认修正后主结论**
   ...
   - `2023` 失效主结论不变：环境不适配仍是主因，`exit` 不是独立主因

### 当前状态

- `TP1=0%` 旧结论已撤销
- `2023` 研究方向收敛为：`LONG-only` 在熊市/震荡环境中不适配
- 后续若继续推进，应优先研究 regime filter，而不是继续纠结 `TP1` 机制本身
