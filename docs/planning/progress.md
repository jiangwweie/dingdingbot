...
## 2026-03-30 - 会话：实盘导向任务规划 (已完成)

**目标**: 基于实盘目标（止盈追踪 → 交易所测试/实盘），重新梳理任务优先级和架构设计
...
| 优先级 | 任务 | 工作量 | 说明 |
|--------|------|--------|------|
| P0 | 止盈追踪逻辑 | 6-8h | 实盘核心功能 - 没有这个无法实盘 |
| P0 | 多交易所适配 | 6-8h | 实盘刚需 - 测试盘/实盘需要在不同交易所验证 |
| P1 | 可视化 - 逻辑路径 | 4-6h | 实盘信心来源 - 知道为什么信号被过滤 |
| P1 | 可视化 - 资金监控 | 4-6h | 实盘后实时查看资金曲线和信号状态 |
| P2 | 性能统计 | 5-8h | 异步导出 + Python 数据分析，复盘优化 |

**执行顺序**:
```
第 1 周：止盈追踪逻辑 (6-8h)
         └─ 复现 PerformanceTracker，集成到 K 线处理流程
第 2 周：多交易所适配 (6-8h)
         └─ 抽象 ExchangeAdapter，先支持 Binance，后续扩展 OKX
第 3 周：可视化 - 逻辑路径 (4-6h)
         └─ 树形缩进 MVP，展示策略判定 Trace 树
第 4 周：可视化 - 资金监控 (4-6h)
         └─ Lightweight Charts 绘制资金曲线
第 5 周+: 性能统计 (5-8h) + 实盘迭代
         └─ CSV 导出 + Jupyter 分析模板
```

**关键决策**:
1. 止盈追踪实时性：复用现有 K 线流，无需额外 WebSocket 订阅
2. 多交易所优先级：Binance(已有) → OKX(按需) → Bybit(后续)
3. 可视化粒度：树形缩进 MVP 先行，验证价值后再升级流程图

**交付物**:
- 创建：`docs/arch/S6-1-冷却缓存优化 - 废弃说明.md`
