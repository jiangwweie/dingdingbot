...
> **状态更新**: 2026-04-07 - T1-T8, T10 已完成 (83%)

### 4.1 任务清单

| ID | 任务名称 | 优先级 | 预计工时 | 依赖 | 状态 |
|----|----------|--------|----------|------|------|
| T1 | 修复 MTF 未来函数 | P0 | 1h | 无 | ✅ 已完成 |
| T2 | 修复止盈撮合 (添加滑点) | P0 | 1h | 无 | ✅ 已完成 |
| T3 | 创建 orders 表 Alembic 迁移 | P0 | 0.5h | 无 | ✅ 已完成 |
| T4 | 实现订单保存逻辑 | P0 | 2h | T3 | ✅ 已完成 |
| T5 | 创建 backtest_reports 表 | P0 | 1h | 无 | ✅ 已完成 |
| T6 | 实现回测报告保存 | P0 | 2h | T5 | ✅ 已完成 |
| T7 | 回测记录列表页面 | P0 | 3h | T6 | ✅ 已完成 |
| T8 | 订单详情与 K 线图渲染 | P0 | 3h | T4 | ✅ 已完成 |
| T9 | 时间段分页获取 (CCXT) | P1 | 2h | 无 | 🟡 部分完成 |
| T10 | 删除功能 (单条/批量) | P1 | 1h | T7 | ✅ 已完成 |
| T11 | 同时同向持仓限制 | P2 | 1h | 无 | ☐ 待确认 |
| T12 | 权益金检查修复 | P2 | 1h | 无 | ☐ 待确认 |
