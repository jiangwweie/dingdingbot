...
| Broad read-model source | Tradeability still reads many files, but production Candidate Pool / Daily Table no longer consume its export | Tradeability becomes a DB-backed read model over current projections |
...
| Generated output as runtime input | Candidate Pool, Daily Table, and Goal Status are PG-only; Packet still needs full PG-only closure | Runtime builders read repository/current projections |
