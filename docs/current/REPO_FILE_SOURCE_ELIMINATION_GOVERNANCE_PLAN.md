...
| `bootstrap_strategygroup_runtime_pilot.py` file inputs | `local_diagnostic_only` for plan mode, rejected for execute | PG candidate scope, Owner policy current, runtime scope bindings | Execute path requires DB-backed source and rejects Candidate Pool / handoff / intake JSON authority |
