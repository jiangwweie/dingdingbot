...
| **P1** | Active observation monitor could soften legacy blocked artifacts into `waiting_for_signal` and carry `signal_input_json` as current-looking readiness | **closed locally** | `runtime_active_observation_monitor.py` now preserves blocked state and moves blocked-artifact signal refs into `legacy_diagnostics`; `test_active_monitor_preserves_non_actionable_historical_observation_blockers`, `test_active_monitor_preserves_observe_only_stop_reference_gap`, and `test_active_monitor_ignores_legacy_signal_input_as_readiness` guard this | Continue removing remaining `downgrade_mode` Owner/readmodel terminology as semantic cleanup; do not let it drive PG readiness |
