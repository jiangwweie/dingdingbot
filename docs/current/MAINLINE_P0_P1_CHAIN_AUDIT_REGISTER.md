...
| **P1** | Ticket-bound API imported FinalGate / Operation Layer / protected-submit / post-submit materializers from `scripts/` | **closed locally**: action-time materializer ownership moved to `src/application/action_time`; scripts are executable wrappers only | `src/application/action_time/{action_time_ticket,finalgate_preflight,operation_layer_handoff,protected_submit_attempt,post_submit_closure}.py`; `test_ticket_bound_api_imports_application_action_time_services_not_scripts` guards API imports | Continue migrating remaining production-cadence script-owned materializers into application services |
