...
| --- | --- | --- | --- | --- |
| **P0-RCI Runtime Causal Integrity And Adversarial Certification** | natural-event follow-up deploy | Prove exact identity, durable command, lifecycle, and projection invariants across real PostgreSQL and independent-process failure boundaries | `P0_RUNTIME_CAUSAL_INTEGRITY_AND_ADVERSARIAL_CERTIFICATION_DESIGN.md`, `P0_RUNTIME_CAUSAL_INTEGRITY_AND_ADVERSARIAL_CERTIFICATION_IMPLEMENTATION_PLAN.md` | Closed locally with 12 scenarios passing, five implementation defects repaired, no duplicate external effect, no authority expansion, and zero production cadence impact |
