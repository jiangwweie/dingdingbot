...
| `brc_runtime_process_outcomes` current slice | exact 22 `lane_identity_key` values crossed with the authoritative runtime lane process set; latest per pair by indexed `LATERAL ... ORDER BY updated_at_ms DESC,process_outcome_id DESC LIMIT 1`, plus exact release-activation `process_outcome_id`; bound is `22 × process-name count + 1` / **133** with the current six names | `CANARY_PROCESS_OUTCOME_COLUMNS_V1` |
| `brc_runtime_process_outcomes` canary-window slice | `updated_at_ms >= canary_window_floor_ms` and either an exact 22 `lane_identity_key`, `scope_key='production:tokyo'`, or one authoritative runtime lane process name; no process restriction inside exact lane/production scopes / **256** | `CANARY_PROCESS_OUTCOME_COLUMNS_V1`; pre/post set and digest must match |
