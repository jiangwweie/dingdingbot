...
`action_time_if_needed` typed coordinator。2026-07-21 的自然信号已经创建并持久化
Action-Time Invocation；同一 Invocation 未物化 Ticket 时，后续 watcher tick 没有恢复其
typed causal context，最终以 `action_time_typed_invocation_required` 失败。
...
| **自然信号** | `signal:b1e1fb73bb5e2f733081e5dbe947848e`（`SOR-001 / ETHUSDT / long`）曾是 fresh `live_market` Signal，随后自然过期 | 策略信号条件已经至少一次满足，不能归类为纯市场等待 |
| **Action-Time 进度** | `action_time_invocation:6a1abeacce3eae0b02c410eec4334a87` 已持久化，且无 Ticket、无 account/action-time fact refs、未 terminal | producer 已取得精确 causal context；当前 first blocker 是其 retry continuation selection |
| **Watcher 结果** | `85-action-time-refresh-if-needed.conf` 后置刷新报 `action_time_typed_invocation_required` | production entrypoint 正确拒绝无 typed Invocation 的执行，缺口在 trigger/continuation 工程语义 |
| **Ticket 结果** | PG Ticket current 为 0；本次 Invocation 从未绑定 Ticket | 无 Ticket 是上游工程失败的结果，不能由 consumer 解释成正常空闲 |
...
| **只读复核** | Tokyo SSH/PG 已恢复可读；backend、watcher、lifecycle units active；无持仓、无 open order、无 exchange write | 不需要 restart；下一步是修复并部署已证实的 continuation 语义 |
...
stage_reached: selected Invocation persisted but not resumed before Ticket materialization
first_blocker_class: engineering_handoff_gap
first_blocker_detail: unfinished_action_time_invocation_not_resumed -> action_time_typed_invocation_required
...
| --- | --- | --- | --- | --- |
| Signal → Invocation | bounded Signal intake、确定性 arbitration、每 Signal Invocation/process outcome 代码已部署 | 选中的未完成 Invocation 不参与后续 continuation selection；retry 可能缺失 typed Invocation ID | 工程 causal-context 丢失 | **P0** |
| Invocation → Account Facts | typed coordinator 已调用真实只读 account collector；本地已修复 `0/2` 与合法 `1/2` capacity facts | 尚未在 Tokyo 的下一自然信号验证事实束→Ticket | 运行验收缺口 | **P0** |
...
本次东京可复现的直接根因不是授权或策略条件，而是以下状态机缺口：

```text
watcher 只负责 Signal，边界正确
-> arbitration 创建 selected Invocation
-> Invocation 尚未形成 Ticket/facts，仍属于同一 causal context
-> next watcher tick 的 continuation selector 只识别 Ticket
-> trigger 仍成立但缺少 typed Invocation ID
-> production entrypoint fail-closed: action_time_typed_invocation_required
-> Signal 过期，未产生 Ticket 或 exchange write
```
...
#### 0.6.0 Invocation continuation invariant

在 fresh Signal 到 Ticket 的任意中断点，下一 watcher tick 必须先选择**唯一仍有效的
unfinished Invocation**，再进行新的 Signal arbitration：

1. 当前 Ticket continuation 优先；
2. 没有当前 Ticket 时，选择 `ticket_id IS NULL`、`closed_at_ms IS NULL`、Invocation 与
   source Signal 都未过期、Signal 仍为 `live_market/facts_validated/fresh` 的唯一 Invocation；
3. 已 terminal、已过期、已失效或已绑定 Ticket 的 Invocation 绝不复活；
4. 多条当前 Ticket 或多条当前 unfinished Invocation 都 fail-closed，禁止按最新行猜选；
5. continuation 恢复的是相同 `action_time_invocation_id`、source watermark 和 deadline，
   不创建第二 Invocation，不扩张任何 order authority。

该不变量是工程重试语义，**不改变** StrategyGroup、Owner policy、profile、sizing、
FinalGate、Operation Layer 或 exchange-write authority。
