...
- **Local integration evidence**：the release baseline `ffc73899` and account-risk commit
  `5b67181e` were combined in an isolated worktree. Alembic has one local head, revision `133`;
  PostgreSQL RCI reported `14 passed`, explicit concurrent/full-chain/100000-row capacity tests
  reported `9 passed, 0 skipped`, and the complete suite reported
  `3564 passed, 1 skipped, 0 failed` in `816.02s`. The sole skip records the intentional removal
  of the legacy Trading Console proxy. Production runtime file I/O audit reported
  `suspicious_runtime_file_authority=0`, `frequent_report_write=0`, and
...
| `migrations/versions/2026-07-17-126_create_account_risk_policy.py` | Create | 建立账户风险 policy event/current 和 versioned cluster mapping |
| `migrations/versions/2026-07-17-127_create_account_risk_current_projections.py` | Create | 建立 exposure/budget current 与 semantic-change event tables |
| `migrations/versions/2026-07-17-128_repair_terminal_budget_reservations.py` | Create | 受约束修复 terminal pre-submit Ticket 的 consumed reservation |
| `migrations/versions/2026-07-17-129_add_account_capacity_reservation_scope.py` | Create | 令 reservation 绑定 account policy、instrument、risk cluster 与 capacity projection version |
...
- Create: `migrations/versions/2026-07-17-126_create_account_risk_policy.py`
...
git add migrations/versions/2026-07-17-126_create_account_risk_policy.py \
...
- Create: `migrations/versions/2026-07-17-127_create_account_risk_current_projections.py`
...
python3 -m pytest -q \
  tests/unit/test_account_risk_current_migration.py \
  tests/unit/test_account_exposure_current.py
git add migrations/versions/2026-07-17-127_create_account_risk_current_projections.py \
...
- Create: `migrations/versions/2026-07-17-128_repair_terminal_budget_reservations.py`
