...
| R10-T04 | schema `143` 增加 `dispatch_command_id`；typed coordinator 仅在 Runtime Safety ready 后写入命令；systemd claim worker 以 typed application port 物化 submit attempt / Exchange Command；lifecycle worker 领取 `protected_submit` | **本地完成** | Console session/HTTP 不再是定时自动链路依赖；仍需 Tokyo migration 与 runtime 验收 |
...
| R10-T06 | production-shaped fixture 已证明 Runtime Safety→claim→submit attempt→3 条 command→fake exchange→attempt submitted；还缺 Tokyo PostgreSQL 与自然信号验收 | **本地完成，远端待验收** | 不得将本地 fake exchange 等同于真实订单结果 |

**R10-T04 已实现的收敛：** typed coordinator 在 Runtime Safety ready 后写入一个
...
crash/replay outcome projection 和 fake-exchange two-worker proof。本地 fake-exchange
full-chain 已覆盖命令幂等、claim/reclaim、commit-before-I/O 与 attempt outcome projection；
Tokyo migration 后仍必须以真实 PG 与下一自然信号复验。
