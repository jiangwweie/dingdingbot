...
| `output/runtime-monitor/latest-daily-live-enablement-table.json/md` | `build_daily_live_enablement_table.py` | validators, Single Lane Packet legacy/local mode, human review exports | Main control table | `brc_control_read_model_snapshots` export over current projections | Builder CLI is PG-only; latest file is export/diagnostic only |
| `output/runtime-monitor/latest-single-lane-task-packet.json/md` | `build_single_lane_task_packet.py` | agents and human review exports | One-lane task packet | task export only | Should never reclassify market waits into engineering closure; still needs PG-only closure |
...
| Tradeability as broad generated source | Tradeability still reads registry/tier policy and many optional latest artifacts, but production Candidate Pool and Daily Table no longer consume its export | One stale optional artifact can still alter remaining legacy diagnostics or task exports | Tradeability becomes a DB-backed read model over typed current projections |
...
| Candidate Pool / Daily Table / Packet feedback loop | Candidate Pool and Daily Table are PG-only; Packet still needs full PG-only closure | A stale previous export must not influence a new current projection | Candidate Pool and Daily Table read DB current projections and write exports only; Packet must follow the same rule |
