...
| `/home/ubuntu/brc-deploy/reports/runtime-signal-watcher/strategy-group-live-facts-input.json` | product-state refresh | account-safe facts builder, product-state refresh | readonly live/private fact collector input | `brc_runtime_fact_snapshots` | Must not be consumed by action-time boundary or materializers |
| `/home/ubuntu/brc-deploy/reports/runtime-monitor/latest-account-safe-facts.json` | `build_runtime_account_safe_facts.py` | human review / diagnostics only | Account/open-order/balance safety facts export; production action-time boundary reads PG `account_safe` / `account_mode` snapshots | `brc_runtime_fact_snapshots` / `brc_runtime_safety_state_snapshots` | Must not be read as runtime input |
