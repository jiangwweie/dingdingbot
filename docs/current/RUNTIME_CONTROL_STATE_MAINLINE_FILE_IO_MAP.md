...
| `/home/ubuntu/brc-deploy/reports/runtime-signal-watcher/strategy-group-live-facts-input.json` | retired product-state precollect path | diagnostics only when manually generated | readonly live/private fact collector export | `brc_runtime_fact_snapshots` | Server refresh no longer writes or feeds this into account-safe facts |
