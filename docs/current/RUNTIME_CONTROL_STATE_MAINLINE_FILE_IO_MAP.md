...
| 40 | retired `40-resume-dispatcher.conf` | none in production | none in production | Retired JSON resume-pack dispatcher bridge | Production dispatcher is `90` and reads PG Action-Time Ticket identity |
| 60 | retired `runtime_dry_run_audit_chain.py` post-step | none in production | none in production | Dry-run audit is manual diagnostic only | Must not run on watcher tick or set current blockers |
...
| 80-B | `refresh_strategygroup_runtime_product_state_artifacts.py` | operator-authenticated readmodel APIs plus dry-run/closure/deploy diagnostic exports only | runtime status readiness, pilot status, chain closure, live-closure evidence, product refresh packet | Product-state and closure evidence export refresh; no live-facts precollect and no runtime fact authority | fact/event rows, legacy diagnostics, closure evidence refs |
| 85 | `run_server_product_state_refresh_sequence.py --mode action_time_if_needed` | PG trigger counts for fresh signal / promotion / lane / ticket | action-time sequence report; action-time exports only when triggered | Runs account/action-time materialization only when PG says action-time work exists | No-signal ticks skip heavy materializers; fresh/promotion/lane/ticket states advance to PG Action-Time Ticket |
| 90 | `runtime_signal_watcher_resume_dispatcher.py --identity-source pg_ticket` | `brc_action_time_lane_inputs`, `brc_action_time_tickets` | `resume-dispatch-artifact.json`, optional operation evidence export | Ticket-bound dispatcher after PG ticket materialization | FinalGate/Operation Layer handoff identity is PG `ticket_id`; JSON resume pack is diagnostic only |
...
| `strategy-group-live-facts-readiness.json` | product-state refresh from PG-backed readmodel | runtime diagnostics only | PG-derived live fact readiness export | `brc_runtime_fact_snapshots`, current safety projection | Goal Status and trading decisions must not read this file; the readmodel itself reads PG fact snapshots |
| legacy runtime-status export | product-state refresh | Goal Status | legacy runtime status readiness compatibility export | runtime status readmodel export | Readiness export is not trading authority |
...
| File | Current writers | Current readers | Current role | PG target | Key issue |
| --- | --- | --- | --- | --- | --- |
| `post-signal-resume-pack.json` | readiness-pack export | diagnostics only | historical compatibility export | `brc_promotion_candidates`, `brc_action_time_lane_inputs`, `brc_action_time_tickets` | Must not be used as production trade identity |
| `resume-dispatch-artifact.json` | PG ticket-bound dispatcher | Goal Status / diagnostics | dispatcher outcome export | action-time/execution-chain evidence refs | Must include PG `ticket_id`; export is not identity authority |
...
| `runtime-dry-run-audit-chain.json` | manual dry-run audit only | diagnostics only | non-executing rehearsal audit | rehearsal evidence refs, `brc_legacy_diagnostics` | Must not be generated by production watcher tick |
