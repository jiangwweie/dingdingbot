...
| `output/runtime-monitor/latest-sor-session-detector-facts.json/md` | `build_sor_session_scope_detector.py` | action-time boundary, Candidate Pool | SOR detector facts | `brc_live_signal_events`, `brc_runtime_fact_snapshots` | Detector fact output is currently a file source |
| `output/runtime-monitor/latest-mi-trial-admission-decision.json/md` | `build_mi_trial_admission_decision.py` | Daily Table, Candidate Pool | MI admission/trial decision | `brc_strategy_governance_decisions`, `brc_admission_decisions`, export | Governance decision is mixed into runtime output |
| `output/runtime-monitor/latest-brf2-runtime-signal-facts.json/md` | `build_brf2_runtime_signal_facts.py` | Candidate Pool | BRF2 runtime signal facts | `brc_live_signal_events`, `brc_runtime_fact_snapshots` | Short-side conditional state should not be inferred from latest file |
| `output/runtime-monitor/latest-strategy-fresh-signal-action-time-boundary.json/md` | `build_strategy_fresh_signal_action_time_boundary.py` | Daily Table, Candidate Pool | Fresh-signal/action-time boundary view | `brc_promotion_candidates`, `brc_action_time_lane_inputs`, export | Action-time boundary is a projection, not a source |

### Control Read Model Files

| File | Current writers | Current readers | Current role | PG target | Key issue |
| --- | --- | --- | --- | --- | --- |
| `output/runtime-monitor/latest-strategygroup-tradeability-decision.json/md` | `build_strategygroup_tradeability_decision.py` | Daily Table, Candidate Pool, Single Lane Packet evidence refs | Can-trade / first-blocker read-model export | DB-backed read model over registry, policy, facts, safety, readiness | Production path reads `PgBackedRuntimeControlStateRepository`; local JSON inputs require explicit `--allow-local-file-diagnostic` |
...
| `strategy-group-live-facts-readiness.json` | product-state refresh | Goal Status | live fact readiness | `brc_runtime_fact_snapshots`, current safety projection | Goal Status should not independently arbitrate this file |
...
| --- | --- | --- | --- |
| Goal Status multiple writer history | `70-goal-status.conf`, product-state refresh, and final 80 shell have all been possible writers | Last writer can overwrite current status with older inputs | `brc_goal_status_current` has one owner projector |
...
| No generated-view feedback | Candidate Pool, Daily Table, Packet, Goal Status, and server monitor do not read each other's latest JSON as authority |
