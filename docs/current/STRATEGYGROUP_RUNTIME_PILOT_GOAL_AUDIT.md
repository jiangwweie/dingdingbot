...
| Item | Evidence |
| --- | --- |
| Systemd hook | `80-product-state-refresh.conf` no longer relies on product-state `--refresh-goal-status`; final Goal Status is generated by the dedicated PG-backed `build_strategygroup_runtime_goal_status.py --require-database-url` step |
