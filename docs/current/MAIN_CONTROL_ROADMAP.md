...
| Local progress cache | `--output-json` and `--output-owner-progress` can persist the latest daily-check report and Owner progress text; `--report-json-path` can re-render a saved report locally with zero Tokyo interaction |
...
| Cache freshness visibility | Owner progress output includes `报告时间` and `缓存年龄`, so zero-interaction local reviews do not look like newly refreshed Tokyo checks |
