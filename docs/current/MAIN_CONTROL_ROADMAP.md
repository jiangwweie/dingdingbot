...
| P0.5 Signal Coverage Diagnostic | Mainline no-signal periods can be compared against the broader read-only StrategyGroup shelf and summarized for Owner review | Main runtime window | local diagnostic + expansion review ready | Use `latest-signal-coverage-diagnostic.json` and `latest-signal-coverage-expansion-review.json` to decide whether the issue is market waiting, narrow scope, or strategy signal quality; do not treat broader preview signals as real-submit authority or L4 promotion authority |
