...
| P0.5 Replay Corpus | Historical market/signal windows can exercise StrategyGroup and runtime behavior without waiting for live market signals | Main runtime window + strategy research input | local corpus ready; `BTPC-001` L2 shadow replay and `VCB-001` L1 observe replay added | Keep MPG-001 eight-window corpus wired into local dry-run audit; use BTPC-001 L2 shadow replay and VCB-001 L1 observe replay for no-action / would-enter diagnostics without expanding L4 real-order scope |
