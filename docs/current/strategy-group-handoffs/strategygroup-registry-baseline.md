...
| --- | --- | --- | --- | --- | --- |
| `MPG-001` | 动量延续 | `L4` | `true` | `false` | `reviewed_handoff_plus_replay` |
| `TEQ-001` | 类股权永续动量 | `L2` | `false` | `false` | `reviewed_handoff_partial_runtime_history` |
| `FBS-001` | 资金费率/基差压力 | `L3` | `false` | `false` | `reviewed_handoff_derivatives_heavy` |
| `SOR-001` | 开盘区间结构 | `L3` | `false` | `false` | `reviewed_handoff_session_conditional` |
| `PMR-001` | 贵金属制度覆盖 | `L1` | `false` | `false` | `reviewed_handoff_observe_overlay` |
| `BTPC-001` | 熊市回抽延续 | `L2` | `false` | `false` | `reviewed_l2_handoff_plus_proxy_replay_decision` |
| `VCB-001` | 波动压缩突破 | `L1` | `false` | `false` | `partial_replay_and_decision_evidence` |
| `LSR-001` | 流动性扫盘/短线复活 | `L1` | `false` | `false` | `partial_replay_and_decision_evidence` |
| `BRF-001` | 熊市反弹失败 | `L1` | `false` | `false` | `partial_replay_and_decision_evidence` |
| `RBR-001` | 区间边界回归 | `L1` | `false` | `false` | `partial_generated_decision_only` |
...
- 层级 / 可试运行: `L3` / `false`
- 当前可行动性: `false`，只能由运行时根据当前实盘状态判断
- 晋级条件: Derivatives facts and margin/liquidation mapping must support a Decision Ledger promote/go-live review after P0 closure or Owner lane change.
...
- 层级 / 可试运行: `L2` / `false`
- 当前可行动性: `false`，只能由运行时根据当前实盘状态判断
- 晋级条件: Complete live derivatives fact-source mapping, classifier review, and Decision Ledger revise resolution before higher-tier review.
...
- 证据引用: `docs/current/strategy-group-handoffs/BTPC-001/handoff.json`, `docs/current/strategy-group-handoffs/BTPC-001/replay/btpc-001-l2-replay-corpus.json`, `output/runtime-monitor/latest-btpc-l2-keep-revise-fact-source-decision.json`, `output/runtime-monitor/latest-strategygroup-decision-ledger.json`
...
- 证据引用: `docs/current/strategy-group-handoffs/VCB-001/replay/vcb-001-l1-observe-replay-corpus.json`, `output/runtime-monitor/latest-strategygroup-decision-ledger.json`
...
- 下一证据: post-revision stage review before any tier change
- 证据引用: `docs/current/strategy-group-handoffs/LSR-001/replay/lsr-001-l1-observe-replay-corpus.json`, `output/runtime-monitor/latest-strategygroup-decision-ledger.json`
...
- 下一证据: post-revision stage review before any tier change
- 证据引用: `docs/current/strategy-group-handoffs/BRF-001/replay/brf-001-l1-observe-replay-corpus.json`, `output/runtime-monitor/latest-strategygroup-decision-ledger.json`
...
- 证据引用: `output/runtime-monitor/latest-strategygroup-decision-ledger.json`
