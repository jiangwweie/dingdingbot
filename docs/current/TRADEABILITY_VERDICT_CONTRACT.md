...
The Tradeability Verdict is the main-control answer to one product question:
...
| `verdict` | One of the verdict values above |
...
| `actionable_now` | Runtime-derived only |
...
| --- | --- | --- | --- |
| `tiny_live_intake_candidate` | Research output is worth main-control review | Intake review, final-owned snapshot, verdict row | Runtime observation or submit authority |
...
```json
{
  ...
  "verdict": "not_tradable_asset_admission",
...
A Tradeability Verdict implementation is accepted only when:

| Requirement | Rule |
| --- | --- |
| One current verdict | Every active selected, admitted, or intake candidate has exactly one current verdict |
...
| No authority leakage | Research, replay, paper, verdict, and policy rows never set `actionable_now=true` or `real_order_authority=true` |
| Monitor integration | Local monitor sequence surfaces the verdict without turning internal gate names into the primary Owner interface |
