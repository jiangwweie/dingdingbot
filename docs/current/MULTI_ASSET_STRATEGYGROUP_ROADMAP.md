...
| **P3-X：动态标的生产实验** | 把已通过 Replay 的 SOR 选择规则接入唯一 Instrument membership 与 time-bounded ENTRY authority，并以原风险小资金取得真实证据 | 已批准三 Plane设计与P3-X.3A Implementation Plan；DS-00至DS-07已经完成，现已具备Selection Runner、immutable Snapshot、Selection-Period`PRE_FENCE_CONTINUITY`、`NO_CHANGE/VALID_EMPTY`、durable Generation handoff、Entry Vacuum全边界negative fence、durable cancel/unknown recovery、合法双腿partial protection、通用Authority Gap Audit、trigger suppression、`first_eligible_close_time_ms`、串行warming、staged pair、原子切换、supersession、exact post-fence fallback，以及Observation/Admission/Ticket/dispatch四边界的birth Authority lineage；当前进入runtime hosting、recovery、Owner control和thin release compatibility projection | 设计状态`DESIGN_APPROVED`；计划状态`PLAN_APPROVED / CODE_AND_TEST_ONLY / DS-08`；Owner已授权DS-02至DS-10本地顺序实施，首次Dynamic Session仍须独立授权，且不提高风险或绕过Strategy Control |
...
3. **Production design / Implementation Plan / certification**：生产设计与Implementation Plan
   已经批准；DS-00至DS-07已经完成并通过各自Focused/Fast验证，当前Active Execution Scope为
   `DS-08`。后续实现为三个经PostgreSQL durable handoff解耦的Plane：Selection只提交
