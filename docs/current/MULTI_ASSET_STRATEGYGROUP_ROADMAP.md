...
| **P3-X：动态标的生产实验** | 把已通过 Replay 的 SOR 选择规则接入唯一 Instrument membership 与 time-bounded ENTRY authority，并以原风险小资金取得真实证据 | **DS-00至DS-10本地实现与证据包已完成**：Selection/Materialization/Deployment三Plane、Snapshot、continuity、Vacuum、Gap Audit、串行warming、双边原子切换、fallback、四边界birth Authority、独立runtime leases、Owner Pause、TOTP-protected Dynamic activation、24-Candidate readonly audit、exact R4 Golden parity与zero-runtime-side-effect postflight均已闭合 | 设计`DESIGN_APPROVED`；计划`LOCAL_IMPLEMENTATION_COMPLETE / production_authority=NONE`；下一Gate为独立软件部署复核，首次Dynamic Session仍须另一独立授权，且不提高风险或绕过Strategy Control |
...
3. **Production design / Implementation Plan / certification**：生产设计与Implementation Plan
   已经批准；**DS-00至DS-10本地实现与证据包已经完成**，exact candidate只由clean-HEAD R4
   manifest识别。实现为三个经PostgreSQL durable handoff解耦的Plane：Selection只提交
...
14. P3-X 实施计划：
    ...
    DS-00至DS-10本地实现与证据包已经完成。该完成不包含生产Migration执行、Tokyo部署、
    Crypto SOR resume或首次Dynamic activation；软件部署与首次Dynamic Session继续分开授权。
