...
| **P3-X：动态标的生产实验** | 把已通过 Replay 的 SOR 选择规则接入唯一 Instrument membership 与 time-bounded ENTRY authority，并以原风险小资金取得真实证据 | 已批准三 Plane设计与P3-X.3A Implementation Plan；DS-00至DS-05已经完成，现已具备Selection Runner、immutable Snapshot、Selection-Period`PRE_FENCE_CONTINUITY`、`NO_CHANGE/VALID_EMPTY`、durable Generation handoff、Entry Vacuum全边界negative fence、durable cancel/unknown recovery、合法双腿partial protection、通用Authority Gap Audit、trigger suppression与`first_eligible_close_time_ms`；当前进入LONG/SHORT串行warming、staged pair、原子切换与post-fence fallback，后续是四边界Authority lineage、runtime hosting和thin release compatibility projection | 设计状态`DESIGN_APPROVED`；计划状态`PLAN_APPROVED / CODE_AND_TEST_ONLY / DS-06`；Owner已授权DS-02至DS-10本地顺序实施，首次Dynamic Session仍须独立授权，且不提高风险或绕过Strategy Control |
...
3. **Production design / Implementation Plan / certification**：生产设计与Implementation Plan
   已经批准；DS-00至DS-05已经完成并通过各自Focused/Fast验证，当前Active Execution Scope为
   `DS-06`。后续实现为三个经PostgreSQL durable handoff解耦的Plane：Selection只提交
...
1. **P3-X.3A DS-00 Golden/Evidence/Test Portfolio**：**已完成**。已冻结961×24 member-level
   ...
   DS-02已经通过本地Migration/Schema/Repository验收；DS-03 SelectionCore/Runner已完成961×24 Golden parity与本地PostgreSQL验收；DS-04已完成continuity、Disposition、Vacuum intent、Generation handoff与Gap Audit；DS-05已完成Admission/Ticket/dispatch Vacuum enforcement、durable cancel、unknown recovery、retained partial和`VALID_EMPTY`原子闭环并通过228项focused和916项Fast验证，当前Active Execution Scope为`DS-06`；生产Migration、部署和首次Dynamic activation仍无授权。
2. **P3-X.3B Implementation/Certification**：DS-00至DS-05已经完成；当前从DS-06继续顺序执行
   串行warming/双边原子切换、post-fence fallback、
