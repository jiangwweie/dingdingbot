...
| **P3-X：动态标的生产实验** | 把已通过 Replay 的 SOR 选择规则接入唯一 Instrument membership 与 time-bounded ENTRY authority，并以原风险小资金取得真实证据 | 已批准三 Plane设计与P3-X.3A Implementation Plan；DS-00至DS-04已经完成，现已具备Selection Runner、immutable Snapshot、Selection-Period`PRE_FENCE_CONTINUITY`、`NO_CHANGE/VALID_EMPTY` intent、durable Generation handoff、Entry Vacuum negative fence、通用Authority Gap Audit、trigger suppression与`first_eligible_close_time_ms`；当前进入unfinished ENTRY drain、durable cancel、unknown outcome和合法双腿partial protection，后续是LONG/SHORT串行warming+原子切换、post-fence fallback和thin release compatibility projection | 设计状态`DESIGN_APPROVED`；计划状态`PLAN_APPROVED / CODE_AND_TEST_ONLY / DS-05`；Owner已授权DS-02至DS-10本地顺序实施，首次Dynamic Session仍须独立授权，且不提高风险或绕过Strategy Control |
...
3. **Production design / Implementation Plan / certification**：生产设计与Implementation Plan
   已经批准；DS-00至DS-04已经完成并通过各自Focused/Fast验证，当前Active Execution Scope为
   `DS-05`。后续实现为三个经PostgreSQL durable handoff解耦的Plane：Selection只提交
...
1. **P3-X.3A DS-00 Golden/Evidence/Test Portfolio**：**已完成**。已冻结961×24 member-level
   ...
   DS-02已经通过本地Migration/Schema/Repository验收；DS-03 SelectionCore/Runner已完成961×24 Golden parity与本地PostgreSQL验收；DS-04已完成continuity、Disposition、Vacuum intent、Generation handoff与Gap Audit并通过50项focused和902项Fast验证，当前Active Execution Scope为`DS-05`；生产Migration、部署和首次Dynamic activation仍无授权。
2. **P3-X.3B Implementation/Certification**：DS-00至DS-04已经完成；当前从DS-05继续顺序执行
   Vacuum/ENTRY cancel/partial-retained、串行warming/双边原子切换、post-fence fallback、
   Observation/Admission/Ticket/dispatch enforcement、independent runtime leases、thin release
   classification projection和full-chain fault certification；规则修改创建V1，不原地重写V0。
