...
| --- | --- | --- | --- | --- |
| **L1-1** | Strategy governance exists, but many users still cannot see a single "strategy meaning" summary. | Registry contract, PG seed/current rows, tier review, temporary L2-L7 draft are separate. | Owner must mentally join several views. | Create a PG-backed strategy semantics read model with one row per active StrategyGroup: what it eats, allowed symbols, allowed side, event, invalidation, and current concern. |
| **L1-2** | Strategy handoff JSON/MD has been retired from current, but reintroduction remains a risk. | Retired StrategyGroup handoff JSON family. | If code reads handoff files after PG cutover, old source authority returns. | Keep handoffs as git-history/archive provenance only; production readers must use PG strategy/event/version rows. |
