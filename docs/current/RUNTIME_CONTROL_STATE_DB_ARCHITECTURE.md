...
| Account fact collector | live-facts report JSON | `latest-account-safe-facts.json` | Private/account fact source can still drift until the account-fact PG cutover | `brc_runtime_fact_snapshots` / `brc_runtime_safety_state_snapshots` |
