...
| Account fact collector | live-facts readonly input from product-state refresh | `latest-account-safe-facts.json` export | Account-safe JSON must not feed action-time decisions | `brc_runtime_fact_snapshots` global `account_safe` / `account_mode` rows with short valid-until timestamps |
