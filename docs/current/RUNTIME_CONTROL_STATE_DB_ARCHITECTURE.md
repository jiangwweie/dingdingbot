...
| --- | --- | --- | --- | --- |
| Watcher tick / active monitor | systemd runtime, exchange/public inputs, PG candidate scope/runtime bindings | server report JSON such as `latest-status.json`, local `latest-runtime-active-observation-status.json` exports | Watcher coverage fails closed without PG instead of letting a previous-cycle generated view drive the observer universe | `brc_strategy_group_candidate_scope`, `brc_watcher_runtime_coverage`, plus fact/event rows |
| Runtime bootstrap | PG candidate scope/runtime bindings only | runtime bootstrap artifact and optional shadow runtime records through official API | File-backed bootstrap can create runtime records from stale handoff or Candidate Pool scope | `brc_strategy_group_candidate_scope`, `brc_owner_policy_current`, `brc_runtime_scope_bindings` drive runtime admission; bootstrap rejects handoff/intake/Candidate Pool/active-runtime JSON inputs |
| Public fact collector | exchange public APIs, PG candidate scope/runtime bindings | `latest-binance-usdm-public-facts.json` export | Public fact JSON must not become a fallback source | `brc_runtime_fact_snapshots` lane-scoped `pretrade_public` rows with observed/valid-until timestamps |
...
| Candidate Pool side/scope constants | Candidate Pool historically kept StrategyGroup side scope, scoped-live-submit membership, and primary symbol rules in code constants | Code constants can override the StrategyGroup / symbol / side scope approved in PG | Production Candidate Pool validates PG Owner policy/runtime scope and derives side support from PG authorization scope only; old side/scope constants must be deleted or rewritten into PG/event specs |
| Generated view consumed as source | Candidate Pool, Daily Table, Single Lane Packet, and Goal Status are PG-only | Read models must not become second-order truth | Builders read repository/current projections; JSON is export |
| Fresh attempt readiness bridge | `runtime_fresh_attempt_readiness_projection` previously aggregated JSON evidence and exposed dispatcher-ready actions | It bypasses the PG ticket identity chain and can make "this trade" depend on loose files | Remove the bridge; dispatcher must reject the retired scope and require PG ticket-bound progression |
| RequiredFacts readiness artifact | `runtime_strategy_required_facts_readiness_artifact` previously read local strategy semantics plus optional JSON fact-source reports | It can disagree with PG RequiredFacts contracts and fact snapshots | Remove the script; RequiredFacts readiness is computed from PG contracts/facts and exposed through Candidate Pool/action-time projections |
