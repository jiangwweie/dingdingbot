...
| Generated view consumed as source | Candidate Pool, Daily Table, and Goal Status are PG-only; Packet still needs full PG-only closure | Read models must not become second-order truth | Builders read repository/current projections; JSON is export |
