...
This file is historical provenance only. It is not current project authority,
not a runtime input, not a monitor source, and must not reintroduce the retired
local monitor sequence or generated JSON/MD report chain into current code,
tests, task packets, or deployment paths.
...
| Latest deployed runtime head | `c8304bec74b6675a8c36e6e7c3d2f134e3243bd8` |
...
| Historical Tokyo deploy apply | `status=applied`, `blockers=[]`, `remote_interaction_count=7`, `calls_exchange_write=false`, `places_order=false` |
...
`same_tick_product_state_visibility_contract`. This upgrades the runtime status readiness
...
| Required order | The local contract verifies `dry_run -> chain_closure -> live_closure -> legacy runtime-status export API`; Goal Status is generated by the dedicated PG-backed projector outside product-state refresh |
...
The 80 watcher refresh sequence now refreshes live-closure evidence first and
then lets the dedicated PG-backed Goal Status projector generate
`strategygroup-runtime-goal-status.json`. Product-state refresh no longer owns
that write. This closes the remaining one-tick visibility gap without
reintroducing a second Goal Status writer.

| Item | Evidence |
| --- | --- |
| Refresh order | `refresh_strategygroup_runtime_product_state_artifacts.py` runs dry-run refresh, chain-closure refresh, live-closure refresh, then readmodel/API artifact refresh |
| Runtime status readiness impact | legacy runtime-status export can consume the same-tick `strategygroup-runtime-goal-status.json` instead of reading the previous watcher tick |
| Test coverage | `test_refresh_product_state_artifacts_can_refresh_dry_run_and_goal_status` keeps the injected test hook for local ordering; production Goal Status is not written by product-state refresh |
...
The watcher 80 sequence now lets the dedicated PG-backed Goal Status projector
generate `strategygroup-runtime-goal-status.json` after
`runtime-live-closure-evidence*.json` is generated. This prevents a future first
bounded real-order closure from being visible only one watcher
tick later.

| Item | Evidence |
| --- | --- |
| Systemd hook | `80-product-state-refresh.conf` does not call product-state Goal Status refresh; the old `--refresh-goal-status` product-state entrypoint is removed, and final Goal Status is generated by the dedicated PG-backed `build_strategygroup_runtime_goal_status.py --require-database-url` step |
| Refresh order | `refresh_strategygroup_runtime_product_state_artifacts.py` runs dry-run refresh, chain-closure refresh, and live-closure refresh; the dedicated PG Goal Status step runs separately in the 80 sequence |
| Test coverage | `test_refresh_product_state_artifacts_retires_default_goal_status_writer` asserts that default product-state refresh no longer writes Goal Status |
| Owner impact | When the first live closure evidence is complete, the dedicated PG Goal Status step can observe it in the same 80 sequence without reintroducing a second writer |
