...
| **P1** | Active observation monitor could soften legacy blocked artifacts into `waiting_for_signal` and carry `signal_input_json` as current-looking readiness | **closed locally** | `runtime_active_observation_monitor.py` now preserves blocked state and moves blocked-artifact signal refs into `legacy_diagnostics`; `test_active_monitor_preserves_non_actionable_historical_observation_blockers`, `test_active_monitor_preserves_observe_only_stop_reference_gap`, and `test_active_monitor_ignores_legacy_signal_input_as_readiness` guard this | Keep blocked artifacts blocked; do not let retired signal refs drive PG readiness |
| **P1** | Owner/readmodel and watcher artifacts preserved old fallback口径 in current chain outputs | **closed locally** | Current scripts, readmodels, systemd, and docs now use `authority_mode`; `test_l2_l7_current_sources_do_not_reintroduce_retired_authority_key` rejects the retired authority key and return-to-observation fallback wording | Keep execution authority described as explicit mode, not as a fallback path |
| **P1** | Action-time refresh used wall-clock time independently in each materializer, so a fresh ticket could reach FinalGate / Operation Layer / Runtime Safety but miss protected-submit because only a few seconds of TTL remained | **closed locally** | `run_server_product_state_refresh_sequence.py` now pins one `action_time_sequence_now_ms` for the whole action-time batch and passes it to every action-time materializer; `test_server_product_state_refresh_sequence_pins_action_time_materializers_to_one_batch_time` guards the invariant | Keep real exchange submit API using call-time safety checks; the batch timestamp only applies to non-authority materialization cadence |
| **P1** | Goal Status / Server Monitor could regress to `action_time_boundary_not_reproduced` after a ticket-bound protected submit completed because consumer projections did not model terminal submit rehearsal state | **closed locally** | `Goal Status` now treats `disabled_smoke_passed` as `protected_submit_rehearsal_completed` and `submitted` as `real_order_submitted`; monitor bounded reads retain `attempt -> safety -> handoff -> ticket -> lane -> promotion -> signal` lineage; `test_pg_goal_status_reports_completed_disabled_smoke_after_lane_expires`, `test_pg_goal_status_newer_lane_supersedes_previous_disabled_smoke_attempt`, `test_repository_monitor_read_profile_retains_protected_submit_lineage`, and `test_pg_completed_disabled_smoke_attempt_is_quiet_not_boundary_blocked` guard the consumer matrix | Keep terminal ticket-bound submit facts as later chain facts than Candidate Pool action-time blockers, but do not let an older terminal attempt suppress a newer fresh lane |
| **P1** | Same `live_signal_event` could be reopened into multiple promotion / action-time lane / ticket / disabled-smoke attempts when only `observed_at_ms` changed | **closed locally** | `promotion_candidate_id` is now stable on `signal_event_id`; action-time lane creation rejects any same-signal prior lane, ticket, or protected-submit progression; `test_same_signal_observation_update_does_not_reopen_after_expiry`, `test_new_signal_event_id_creates_new_attempt_identity_after_expiry`, and `test_same_signal_is_not_reopened_after_disabled_smoke_completion` guard the event identity boundary | Treat a new opportunity as a new `signal_event_id` with new event time; do not treat observation refresh as a new trade candidate |
...
pytest tests/unit/test_runtime_active_observation_monitor.py tests/unit/test_runtime_signal_watcher_tick.py tests/unit/test_l2_l7_mainline_chain_invariants.py -q
```
