...
| RequiredFacts / freshness / missing-fact behavior need runtime source integration | Local B0 semantics declare and test missing/stale behavior, the local context builder maps existing signal/runtime snapshots into RequiredFacts, and the trusted runtime fact overlay can fail closed on unavailable account/position sources or missing/stale trusted funding/OI/crowding facts; Trading Console internal assembly can inject PG positions and cached account facts, but live scheduler/runtime evaluation binding plus deployment-backed funding/OI/crowding reader implementations are not fully wired | Each StrategyImplementation must declare required_facts, optional_facts, freshness_requirement, and missing_fact_behavior |
