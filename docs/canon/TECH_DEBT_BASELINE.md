...
| attempt / budget confirmation partially specified locally | Runtime reservation/mutation now prefers concrete max-loss evidence for `budget_reserved` and falls back to notional only when loss-budget evidence is missing; the promotion gate blocks runtime promotion and first-real-submit review until max-loss, notional, leverage, margin, liquidation-buffer, protection readiness, stale-fact behavior, attempt, reservation, release/consume, duplicate-submit, concrete protection-failure policy ID, post-submit settlement persistence-path evidence, account facts, active-position facts, and deployment confirmations are present. First-real-submit outcome accounting can now record submit outcome review evidence and derive an attempt-outcome policy from resolved local order facts plus post-submit reconciliation read-model evidence without mutating budget/runtime state; missing reconciliation evidence or severe mismatch blocks accounting, while warning-only mismatch is recorded as warning. PG-persisted post-submit budget settlement can apply that resolved accounting to runtime state by releasing no-fill/rejected reserved budget or recording filled-outcome budget as held/consumed while leaving attempt counts unchanged. A read-only first-real-submit final review packet can now aggregate current-head postdeploy acceptance with Owner/action evidence for review. The required first-real-submit evidence is that the settlement persistence path exists, not that a settlement record already exists before submit | First real submit must still verify deployed order facts/reconciliation, then use the settlement path for actual release/hold/consume state instead of manual interpretation |
...
| Runtime ExecutionIntent is not production exchange-submit integrated | In tracked code, runtime candidate can become an audit intent, Owner submit authorization, controlled-submit plan, submit-time FinalGate preflight, attempt/budget reservation and mutation audit facts, runtime-native protection plan facts, OrderLifecycle handoff facts, local `Order(status=CREATED)` registration through the default-disabled adapter result skeleton, explicit local-order linkage, non-mutating submit rehearsal, exchange-submit packet/enablement/action authorization evidence, default-disabled exchange-submit adapter/execution result skeletons, Owner-controlled first-real-submit action wrapper, post-submit outcome accounting evidence, PG-persisted post-submit budget settlement, and a read-only first-real-submit final review packet. Adapter-result now revalidates persisted first-real-submit prerequisite evidence before acquiring a duplicate-submit lock and must not call exchange. Exchange-submit execution revalidates the same evidence before any gateway call and remains default-disabled unless explicit execution enablement, scoped action authorization, gateway readiness, recovery/idempotency/protection-failure checks, deployment evidence, and Owner authorization all validate. Deployment state must be verified from the current release manifest and postdeploy reports, not inferred from this row | This is intentional until current-head deploy/postdeploy evidence is generated for the new commit, explicit real-submit action authorization is granted, real exchange order IDs exist, and reconciliation closure is verified |
