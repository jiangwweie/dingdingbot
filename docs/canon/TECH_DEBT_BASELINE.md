...
| Review lacks full right-tail and capital-base productization | Local domain/application review can classify Owner manual withdrawal, manual profit extraction, capital injection, and capital-base reset against read-only account-equity facts without creating withdrawal/transfer/order/exchange instructions. A first productized slice now persists Owner capital adjustment facts, exposes BRC record/list APIs, and surfaces withdrawal-adjusted capital-base review in Trading Console `review-state` / `owner-capital-review`. Full right-tail metrics, automatic capital-base fact orchestration, and full strategy semantic ID propagation remain incomplete | Review should track MFE/MAE, payoff asymmetry, tail wins, bounded max loss, manual withdrawals, capital base adjustments, and unresolved equity deltas separately from strategy performance |
