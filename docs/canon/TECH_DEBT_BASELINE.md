...
| Review lacks full right-tail and capital-base productization | Local domain/application review can classify Owner manual withdrawal, manual profit extraction, capital injection, and capital-base reset against read-only account-equity facts without creating withdrawal/transfer/order/exchange instructions. Productized slices now persist Owner capital adjustment facts, expose BRC record/list APIs, surface withdrawal-adjusted capital-base review in Trading Console `review-state` / `owner-capital-review`, compute/display right-tail trade-path metrics from explicit `live_lifecycle_review.metadata.right_tail_trade_path` facts via `review-state` / `right-tail-review`, and generate non-executing closed-trade semantic review packets from lifecycle review records. Automatic capital-base fact orchestration, automated review-source orchestration, and full strategy semantic ID propagation through order/position remain incomplete | Review should track MFE/MAE, payoff asymmetry, tail wins, bounded max loss, manual withdrawals, capital base adjustments, and unresolved equity deltas separately from strategy performance |
