...
| Leverage / margin / liquidation checks not fully productized | Current local runtime checks include max leverage and notional/loss-budget separation, but real runtime execution still needs explicit margin usage caps and liquidation-vs-stop buffer checks before live submit | Leverage must not expand loss budget; FinalGate should require loss budget, notional, leverage, margin usage, liquidation buffer, and protection readiness to pass together |
...
| Review lacks full right-tail and capital-base productization | Local pure domain/application review slice can classify Owner manual withdrawal, manual profit extraction, capital injection, and capital-base reset against read-only account-equity facts without creating withdrawal/transfer/order/exchange instructions; persistence/API/Console productization and full strategy semantic ID propagation remain incomplete | Review should track MFE/MAE, payoff asymmetry, tail wins, bounded max loss, manual withdrawals, capital base adjustments, and unresolved equity deltas separately from strategy performance |
