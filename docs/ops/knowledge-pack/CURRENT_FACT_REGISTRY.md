...
| CF-003 | Real live trading prohibited unless Owner separately authorizes | ADR-0009 | HIGH |
| CF-004 | Testnet trading available for controlled scenarios with Owner authorization | Binance testnet, verified | HIGH |
...
| CF-017 | Execution permission system has 4 levels: READ_ONLY / INTENT_RECORDING / EXECUTION_INTENT_ALLOWED / ORDER_CAPABLE | `execution_permission.py` | HIGH |
...
|---|---|---|---|
| BLK-001 | `account_equity` unavailable | Pre-trial readiness blocked; cannot compute ratio-based budget | `_account_facts()`: `wallet_equity` and `available_margin` = `not_available` |
| BLK-002 | 3 trial candidates lack cost/slippage/funding/baseline enrichment | Cannot judge if candidates warrant deepening | Broad smoke screen was intentionally cost-free |
...
| DC-004 | account_equity read | NOT AVAILABLE | `wallet_equity` and `available_margin` = `not_available` in `_account_facts()` |
...
| PA-003 | Automated strategy execution | No runtime-eligible strategy |
...
| SB-002 | Testnet operations | Available only for controlled scenarios with Owner authorization |
