...
| PLC-PHASE4-001 | PLC Phase 4 tiny-live-style readiness review | REVIEW | Codex | Owner authorized Phase 4 review. `docs/ops/plc-phase4-tiny-live-style-readiness-review.md` now concludes `phase4_hardening_local_complete / real_live_not_authorized / runtime_smoke_pending`. Real live remains blocked by missing non-real-live runtime smoke evidence and no promoted strategy contract. |
...
| ID | Task | Status | Owner | Notes |
| --- | --- | --- | --- | --- |
| PLC-LT-001 | Full PLC promotion ladder | REVIEW | Codex | Phase 0 local sandbox, Phase 1 read-only adapter, Phase 2 paper observation packet, Phase 3 Binance testnet rehearsal, and Phase 4 tiny-live-style readiness review are complete for review. Real live remains not authorized; next work is non-real-live hardening P4-001 through P4-005. |
| P4-001 | Runtime account risk gate | REVIEW | Codex | Implemented `AccountRiskService`, liquidation-price parsing, and fail-closed orchestrator gate before `CapitalProtection`. Targeted account-risk and orchestrator tests pass; runtime smoke still pending. |
| P4-002 | Durable campaign state machine | REVIEW | Codex | Implemented `runtime_campaign_state` PG migration, repository, service, owner-control API, and new-entry gate. Local transition/API tests pass; local PG runtime `create_all` verified. Older Alembic clean-upgrade chain still needs repair; runtime smoke pending. |
| P4-003 | Conditional protection visibility hardening | REVIEW | Codex | Reconciliation now fetches normal plus conditional STOP_MARKET open-order views and dedupes payloads before protection-health checks. Local read-model tests pass; active-position testnet observation still pending. |
| P4-004 | Runtime control lifecycle reset | REVIEW | Codex | Added startup-guard block/reset control API and shutdown reset to `RUNTIME_SHUTDOWN_RESET`. Local API/shutdown tests pass; API port-release runtime smoke still pending. |
| P4-005 | Phase 4 non-real-live rehearsal design | TODO | Codex | After P4-001 through P4-004, design any future tiny-live-style non-real-live rehearsal under ADR-0009 with exact commands, caps, stop conditions, and rollback path. |
| PLC-LT-002 | Campaign risk state machine | DESIGN_REVIEW | Codex | Spec added in `docs/ops/plc-campaign-risk-state-machine-spec.md`. Defines observe/armed/paused/profit-protect/loss-locked/hard-locked/closed states, audit evidence, and Phase 3 entry/close acceptance boundaries. Runtime implementation remains future work. |
| LS-006 | Account risk state machine | DESIGN_REVIEW | Codex | Spec added in `docs/ops/plc-account-risk-liquidation-safety-spec.md`. Defines unknown/healthy/degraded/critical states and fail-closed behavior for entries while allowing reduce-only close. Runtime implementation remains future work. |
| LS-007 | Liquidation distance and margin safety checks | DESIGN_REVIEW | Codex | Spec added in `docs/ops/plc-account-risk-liquidation-safety-spec.md`. Defines side-aware liquidation distance thresholds and missing/stale-field behavior. Larger notional or multi-position rehearsal remains blocked. |
