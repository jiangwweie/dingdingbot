...
| PLC-PHASE4-001 | PLC Phase 4 tiny-live-style readiness review | REVIEW | Codex | Owner authorized Phase 4 review. `docs/ops/plc-phase4-tiny-live-style-readiness-review.md` concludes `phase4_review_complete / real_live_not_authorized / continue_non_real_live_hardening`. Real live remains blocked by design-only account risk, design-only campaign state, conditional SL visibility noise, runtime control lifecycle cleanup, and no promoted strategy contract. |
...
| P4-001 | Runtime account risk gate | TODO | Codex | Implement account state read model, fail-closed entry gate for unknown/degraded/critical states, and liquidation distance boundary tests before any further live-readiness claim. |
| P4-002 | Durable campaign state machine | TODO | Codex | Implement persisted campaign states and audited transitions for observe/armed/paused/profit-protect/loss-locked/hard-locked/closed. |
| P4-003 | Conditional protection visibility hardening | TODO | Codex | Align reconciliation/protection-health with Binance conditional SL evidence so an active testnet position with confirmed SL does not create false severe protection blocks. |
| P4-004 | Runtime control lifecycle reset | TODO | Codex | Add explicit startup-guard reset/disarm and shutdown/port-release verification after authorized runtime smokes. |
