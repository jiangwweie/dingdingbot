...
| PLC-TESTNET-001 | PLC Phase 3 testnet rehearsal package | READY_FOR_OWNER_AUTH | Codex | Adds bounded Binance testnet rehearsal design, draft ADR-0009 authorization request, campaign risk state-machine spec, account/liquidation safety spec, and runtime-managed close implementation. Scoped tests/compile/diff passed; local PG active orders are clean and `EXIT` is allowed; Binance testnet read-only preflight is flat. Execution is still blocked until Owner authorizes one exact ADR-0009 rehearsal cycle. |
...
| ID | Task | Status | Owner | Notes |
| --- | --- | --- | --- | --- |
| PLC-LT-001 | Full PLC promotion ladder | READY_FOR_OWNER_AUTH | Codex | Phase 0 local sandbox, Phase 1 read-only adapter, Phase 2 paper observation packet, and Phase 3 pre-execution package are implemented for review. Phase 3 still requires Owner authorization for the actual one-cycle Binance testnet rehearsal. Phase 4 tiny-live-style review remains blocked. |
