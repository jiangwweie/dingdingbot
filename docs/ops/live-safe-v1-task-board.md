...
| BRC-R5-001 | Trend playbook carrier full-chain validation | SPEC | Codex | Recommended next implementation slice. Use `TF-001` as a carrier to validate Owner natural-language input, LLM risk advice, action card, monitor state, simulation/testnet trade flow, pause/stop/flatten controls, audit trace, and review. This does not optimize trend parameters, prove alpha, build a strategy pool, or authorize live trading. |
...
| BRC-R5-001D | TF-001 carrier decision review smoke | REVIEW | Codex | Adds `--mode tf001-carrier-decision-review` to the Owner Console smoke helper. Current verdict: `switch_playbook` to `TF-001` is blocked because `TF-001` is not in the BRC playbook allowlist, while `enter_strategy_or_monitor` verifies the monitor-carrier noop path. No catalog expansion, strategy execution, live/mainnet, order cancel/close, actual flatten, withdrawal/transfer, or LLM execution authority is added. |
