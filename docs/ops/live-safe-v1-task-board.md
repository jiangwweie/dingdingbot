...
| RTF-004 | Next-attempt Release Packet | REVIEW | Codex | Local proof added: `RuntimeNextAttemptReleasePacket` joins active-position resolution with an optional non-executing next-attempt gate packet and classifies release into `waiting_for_position_resolution`, `waiting_for_closed_review`, `waiting_for_next_attempt_gate`, `ready_for_strategy_signal`, or `blocked`. Script `runtime_next_attempt_release_from_reports.py` turns existing reports into one packet and tolerates noisy log prefixes. Release only allows strategy signal observation / shadow candidate planning after flat/review/gate proof; executable submit remains disabled and still requires fresh signal, fresh authorization, and official FinalGate. Focused tests pass with `24 passed`. No PG, exchange, order, OrderLifecycle, runtime mutation, close, withdrawal, or transfer occurred. |
