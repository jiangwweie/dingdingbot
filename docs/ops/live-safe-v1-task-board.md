...
| PLC-LT-002 | Campaign risk state machine | REVIEW | Codex | Spec added in `docs/ops/plc-campaign-risk-state-machine-spec.md`. Runtime now has PG-backed state, entry gate, owner-control API, runtime-event transitions, table-driven transition rules, audit records, and replay proof for entry-filled, profit-protect, stop-loss lock, position close, risk-critical, and Owner control transitions. Full automatic wiring from every order lifecycle event remains future work. |
