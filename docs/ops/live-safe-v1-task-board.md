...
| RTF-002 | Strategy-driven next attempt planning reconnection | IMPLEMENTING | Codex | Local service proof added: `RuntimeNextAttemptStrategyPlanningService` gates fresh StrategySignal -> shadow SignalEvaluation / shadow OrderCandidate planning behind `RuntimePostSubmitFinalizePacket` readiness. It refuses blocked post-submit gates and runtime mismatches before planner calls, marks the consumed authorization replay-only, and requires a fresh authorization before any later submit path. Next stage should expose a local/API dry-run entry and Tokyo probe path; current Tokyo BNB runtime remains next-attempt blocked while one runtime-owned active position is open. |
