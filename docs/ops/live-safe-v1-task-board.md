...
| BRC-CONSOLE-V0 | Owner Console v0 IA freeze and implementation | IMPLEMENTING | Codex | Freezes the v0 console around `Command Center`, `LLM Copilot`, `Strategy / Playbook`, `Risk & Account`, and `Runtime Control`. `Summary` becomes Command Center; Markets/Orders and Parameters are folded into readonly Risk & Account. `/api/brc/readiness` is the v0 SSOT for environment boundary, runtime state, risk decision, risk/account summary, strategy/playbook summary, application-owned action cards, cut-off controls, and latest audit. Testnet acceptance reuses the fixed BRC ETH/BTC rehearsal; TF-001 R5 carrier remains a later slice. |
