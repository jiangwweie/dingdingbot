...
| PLC-LT-003 | Long-term PLC capability roadmap | REVIEW | Codex | Adds `docs/ops/plc-long-term-capability-roadmap-v1.md` as the post-Phase-5E planning authority for turning the controlled testnet tool into a reliable personal strategy execution platform. It separates campaign risk state machine, account risk state machine, multi-symbol runtime foundation, Strategy Contract promotion governance, and runtime evidence/rollback. First follow-up capability `PLC-STATE-001` is now implemented locally for review. |
