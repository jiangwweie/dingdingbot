...
| RTF-012 | Fresh Submit Authorization Resolution / Runtime Grant Binding | TODO | Codex | Required because RTF-011 proved the handoff-to-official disabled-smoke call reaches the official endpoint, but rehearsal-only fresh submit authorization IDs cannot be resolved by the official path. Next step is to design and implement the runtime-level fresh submit authorization source so a ready handoff can use a persisted, non-consumed authorization while still preserving consumed-authorization replay-only semantics, duplicate-submit protection, and right-tail bounded-runtime constraints. |
