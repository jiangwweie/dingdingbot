...
| RTF-005 | Release-ready Strategy Planning Reconnection | REVIEW | Codex | Local proof added: `RuntimeNextAttemptStrategyPlanningService.plan_from_release_gate(...)` accepts `RuntimeNextAttemptReleasePacket` and blocks before strategy planning unless release status is `ready_for_strategy_signal`, strategy observation is allowed, and shadow candidate planning is allowed. When release is ready it calls the existing shadow planner and may create only shadow `SignalEvaluation` / shadow `OrderCandidate` planning output; executable submit remains false and still requires fresh authorization plus official FinalGate. Existing post-submit path remains intact. Focused tests pass with `29 passed`. No deployment, PG, exchange, order, OrderLifecycle, runtime mutation, close, withdrawal, or transfer occurred. |
