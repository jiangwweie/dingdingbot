...
| RTF-003 | Active Position Resolution + Close Review + Next Gate Release | TESTING | Codex | Local resolution packet added: `RuntimeActivePositionResolutionPacket` consolidates live-position monitor, exit plan, and post-close follow-up into `hold_with_hard_stop`, `waiting_for_owner_close_authorization`, `ready_for_closed_review`, `ready_for_next_attempt_gate`, or `blocked`. Script `runtime_active_position_resolution_from_reports.py` turns existing reports into one packet and tolerates noisy log prefixes. Current Tokyo BNB evidence resolves to `hold_with_hard_stop`: hard stop exists, continuing to hold is allowed under right-tail semantics, new attempts remain blocked by the active-position slot, TP1 partial is infeasible due min qty/step, and full reduce-only close is feasible only with exact Owner authorization. |
