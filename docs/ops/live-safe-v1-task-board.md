...
| BRC-R4-000 | API surface cleanup and Web contract plan | SPEC | Codex | Adds `docs/ops/brc-r4-api-surface-cleanup-plan.md`. Current API surface has 192 routes across legacy monolith, runtime console, research, config, and profile modules. BRC-R4 should make Web depend only on a BRC-first API contract, split BRC/runtime/dev-testnet routers, and replace module-global compatibility reads with `RuntimeContext` dependencies. No implementation yet. |
| BRC-R4-001 | Local Operator Console | TODO | Codex | Recommended next capability. Build a local-only Web operator surface over existing BRC APIs/ledgers/workflows: current campaign, review packet, next eligibility, LLM/operator plan, explicit confirmation gate, action/workflow ledger, review decision, and next-campaign gate. No new order path, withdrawal/transfer, real-live authority, strategy execution, automatic sizing, or auto-filled confirmation phrase. |
