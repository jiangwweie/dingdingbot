...
| `HAT-001` | `research_candidate` | Heikin-Ashi smoothed trend / stop-reslot revival. | Clean-combo and stop-reslot rows preserve large right-tail windows; boundary fixed in `hat-heikin-ashi-stop-reslot-boundary-20260616.md`. | Drawdown, stop-fill/gap, HA smoothing lag, exit/disable quality, session/product transferability, and real margin facts. |
...
StrategyGroups yet. `DMI-001`, `SCF-001`, and `MASS-001` now have observe-only
handoff drafts, but they are still not armed observation or execution intake.
...
3. Keep `HAT-001` as a revival candidate after
   `hat-heikin-ashi-stop-reslot-boundary-20260616.md`; do not handoff until
   drawdown, stop-fill/gap, HA smoothing lag, product/session/fill, and real
   margin facts improve.
4. Keep `LSR-001` as a revival candidate until its full-sequence, cost/fill,
   slot/M2M, classifier-quality, and real-margin problems have materially
   stronger disable or exit facts.
5. Do not compare these rows as a return leaderboard; their evidence models are
