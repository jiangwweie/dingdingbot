...
> **文档级别**: P0 关键缺陷修复  
> **创建日期**: 2026-04-08  
> **状态**: 已确认实施 (Approved for Implementation)  
...
| 文件路径                                 | 修改类型   | 修改内容                                                     |
| ---------------------------------------- | ---------- | ------------------------------------------------------------ |
| `src/infrastructure/exchange_gateway.py` | 🔴 核心修复 | `_parse_ohlcv()` 增加 `x` 字段支持；`_get_closed_candle()` 修复 K 线选择逻辑 |
| `src/domain/strategy_engine.py`          | 🟡 增强     | `PinbarStrategy.detect()` 增加最小波幅检查                   |
| `src/domain/models.py`                   | 🟡 增强     | `KlineData` 增加 `info` 字段                                 |
