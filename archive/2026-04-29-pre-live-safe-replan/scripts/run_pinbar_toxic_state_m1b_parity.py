...
在与官方 Backtester v3_pms 一致的参数口径下复跑 E0/E1/E4，
验证 M1 proxy 的改善结论是否成立。

参数对齐（vs M1 proxy 的差异标注）：
- TP targets: [1.0, 2.5] (was [1.0, 3.5])  ← 关键差异
- TP split: [0.6, 0.4] (was [0.5, 0.5])     ← 关键差异
- Partial close at TP1: 60% at 1.0R          ← 关键差异（proxy 无 partial close）
- Breakeven: OFF (was ON)                     ← 关键差异
- Entry slippage: 0.10% (was 0.01%)          ← 10x 更高
- TP slippage: 0.05% (was 0%)                ← 新增
- Fee rate: 0.0400% (was 0.0405%)
- EMA 1h period: 60 (was 50)
- Exposure cap: 2.0x (same)
- Max loss %: 1% (same)
"""
...
EMA_1H_PERIOD = 60                    # 官方: 60 (M1 proxy: 50)
EMA_4H_PERIOD = 60
ATR_PERIOD = 14
DONCHIAN_PERIOD = 20

ENTRY_SLIPPAGE = Decimal("0.001")     # 官方: 0.10% (M1 proxy: 0.01%)
FEE_RATE = Decimal("0.0004")          # 官方: 0.0400% (M1 proxy: 0.0405%)
TP_SLIPPAGE = Decimal("0.0005")       # 官方: 0.05% (M1 proxy: 0%)

TP_RATIOS = [Decimal("0.6"), Decimal("0.4")]    # 官方: 60/40 (M1 proxy: 50/50)
TP_TARGETS = [Decimal("1.0"), Decimal("2.5")]    # 官方: [1.0, 2.5] (M1 proxy: [1.0, 3.5])

INITIAL_BALANCE = Decimal("10000")
MAX_LOSS_PCT = Decimal("0.01")
MAX_TOTAL_EXPOSURE = Decimal("2.0")    # 官方 research profile: 2.0x

# M0 tercile boundaries（与 M1 相同）
TOXIC_THRESHOLDS = {
...
@dataclass
class TradeRecord:
    """一条交易记录，支持 partial close（TP1 部分平仓）"""
    signal: PinbarSignal
    ...
    holding_bars: int = 0
    _tp1_hit: bool = False
    _remaining_qty: Decimal = Decimal("0")   # TP1 后剩余仓位
    _tp1_pnl: Decimal = Decimal("0")         # TP1 已实现 PnL
...
class ParityEngine:
    ...
    def run_year(
        ...
        for idx in range(start_idx, len(klines_1h) - 1):
            ...
            if active_trade is not None:
                ...
                # SL checked first (pessimistic: SL > TP on same bar)
                ...
                # TP1: partial close (60% at 1.0R), NO breakeven
                if not active_trade._tp1_hit:
                    if sig.direction == "LONG" and kline.high >= sig.tp1_price:
                        active_trade._tp1_hit = True
                        # Partial close: 60% at TP1
                        tp1_qty = active_trade._remaining_qty * TP_RATIOS[0]
                        if sig.direction == "LONG":
                            tp1_gross = (sig.tp1_price - sig.entry_price) * tp1_qty
                        else:
                            tp1_gross = (sig.entry_price - sig.tp1_price) * tp1_qty
                        tp1_fee = abs(sig.tp1_price * tp1_qty) * FEE_RATE
                        tp1_slip = abs(sig.tp1_price * tp1_qty) * TP_SLIPPAGE
                        active_trade._tp1_pnl = tp1_gross - tp1_fee - tp1_slip
                        active_trade._remaining_qty -= tp1_qty
                        # NO breakeven: keep original SL
                ...
                # TP2: close remaining (40% at 2.5R)
                if active_trade._tp1_hit:
                    if sig.direction == "LONG" and kline.high >= sig.tp2_price:
                        self._close_partial_at_price(active_trade, sig.tp2_price, "TP2", trades)
                        active_trade = None
                        continue
                ...
                if is_year_end:
                    self._close_partial_at_price(active_trade, kline.close, "EOD", trades)
                    active_trade = None
                    continue
            ...
            # EMA + MTF 过滤（使用 EMA_1H_PERIOD=60）
            ...
            max_qty = INITIAL_BALANCE * MAX_TOTAL_EXPOSURE / entry_price
            if qty > max_qty:
                qty = max_qty
        ...
        if active_trade is not None:
            self._close_partial_at_price(active_trade, klines_1h[-1].close, "EOD", trades)
    ...
    def _close_trade_at_price(self, pos: TradeRecord, exit_price: Decimal, reason: str, trades: List[TradeRecord]):
        ...
        if sig.direction == "LONG":
            r_achieved = (exit_price - sig.entry_price) / sig.r_multiple if sig.r_multiple > 0 else Decimal("0")
        else:
            r_achieved = (sig.entry_price - exit_price) / sig.r_multiple if sig.r_multiple > 0 else Decimal("0")
    ...
    def _close_partial_at_price(self, pos: TradeRecord, exit_price: Decimal, reason: str, trades: List[TradeRecord]):
        """
        平仓剩余仓位（TP2 或 EOD after TP1）。
        总 PnL = TP1 已实现 PnL + 剩余仓位 PnL。
        """
        sig = pos.signal
        ...
        if remaining_qty > 0:
            if sig.direction == "LONG":
                ...
                remaining_gross = (sig.entry_price - exit_price) * remaining_qty
            remaining_fee = abs(exit_price * remaining_qty) * FEE_RATE
            remaining_slip = abs(exit_price * remaining_qty) * TP_SLIPPAGE if reason == "TP2" else Decimal("0")
        ...
        # R achieved
        if sig.r_multiple > 0:
            total_exit_value = pos._tp1_pnl + (remaining_pnl if remaining_qty > 0 else Decimal("0"))
            # 简化：用总 PnL / (risk_amount per R) 估算 R
            risk_per_r = INITIAL_BALANCE * MAX_LOSS_PCT  # 100 per R
            pos.r_achieved = float(total_exit_value / risk_per_r) if risk_per_r > 0 else 0.0
        else:
            pos.r_achieved = 0.0
...
# 统计
# ============================================================
def compute_year_stats(trades: List[TradeRecord], year: int) -> Dict:
    if not trades:
        return {
            "year": year, "trades": 0, "pnl": 0.0, "wr": 0.0,
            "max_dd": 0.0, "max_dd_pct": 0.0,
            "total_signals": 0, "filtered_count": 0,
        }
...
async def main():
    ...
    try:
        ...
        EXPERIMENTS = [
            ("E0: Pinbar baseline (parity)", None),
        ...
        # 保存 JSON
        output = {
            "title": "M1b Pinbar Toxic State Avoidance — Backtester Parity Check",
            "date": "2026-04-28",
            "hypothesis": "M1b: 在官方 Backtester 口径下验证 E1/E4 filter 是否仍有效",
            "config": {
                "tp_targets": [float(x) for x in TP_TARGETS],
                "tp_ratios": [float(x) for x in TP_RATIOS],
                ...
                "note": "Matches BACKTEST_PARAM_DEFAULTS + research risk profile",
            },
            "experiments": all_results,
        }

        out_path = Path("reports/research/pinbar_toxic_state_m1b_parity_2026-04-28.json")
