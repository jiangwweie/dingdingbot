...
> **日期**: 2026-04-26
> **作者**: PM (Claude Code)
> **状态**: 架构设计稿（部分已过时 — 见下方状态说明）
> **约束**: 本窗口仅做架构设计和迁移规划，不修改主线代码，不删除任何 SQLite 表/repo/脚本

> ⚠️ **状态说明 (2026-04-27)**：本文档编写时 signals 标记为"SQLite-only / 无 PG 实现"，但此后 live signals + signal_take_profits 已通过 `HybridSignalRepository` 迁移到 PG。Window 1 执行主线 PG 闭环已完成。当前主线已从"继续迁库"切换为"Sim-1 观察 + 策略研究"。文档中关于 signals 位置和迁移优先级的描述已过时，仅保留作历史参考。
