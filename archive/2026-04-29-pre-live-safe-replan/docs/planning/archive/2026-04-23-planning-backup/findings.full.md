...
## 2026-04-22 10:35 -- 回测前端方向收口：不再沿用 `gemimi-web-front` 主模块，改走独立 `backtest-studio`

### 新增结论

1. **现有回测前端“可用”，但不适合继续作为主演进载体**
   - 仓库中已有 `PMSBacktest.tsx`、`BacktestReports.tsx` 等页面
   - 也有功能检查文档认为其“可正常使用”
   - 但页面职责明显混有旧策略工作台、旧导航、旧模板导入语义
   - 因此更适合作为字段和交互参考，而不是下一阶段的主承载模块

2. **新前端应定义为 `backtest-studio`，不是 `strategy workbench 2.0`**
   - 目标是提升个人回测效率
   - 核心闭环应是：参数输入 -> 执行回测 -> 查看结果 -> 与基准参数对比
   - 不应继续承担策略模板管理、实盘页面、订单/仓位工作台职责

3. **当前最合适的推进方式是 B 方案**
   - 与订单对账主线并行
   - 先做设计和静态壳子
   - 后续再接真实 API
   - 这样既不阻塞当前主线，也能提前把产品信息结构确定下来

4. **工程策略应是“产品上放弃旧模块，工程上保留参考资产”**
   - 不建议现在删除整个 `gemimi-web-front`
   - 现有类型定义、回测组件、报告展示逻辑仍有参考价值
   - 但后续不再把它作为主演进方向

### 当前判断

**`backtest-studio` 已可作为低优先级并行线进入设计准备，但不应上升为当前主线。**

### 设计稿

- [2026-04-22-backtest-studio-prd.md](/Users/jiangwei/Documents/dingdingbot/docs/planning/architecture/2026-04-22-backtest-studio-prd.md)
...
## 2026-04-22 22:55 -- PG 双轨迁移骨架落地：先加新实现，不重写旧 SQLite

### 新增结论

1. **最稳的切入点是新增 PG 实现，而不是重写旧仓储**
   现有 SQLite Repository 已经承担大量业务，强行“原地改写”为 PG 会把迁移和重构绑在一起。新增 PG 实现更适合当前阶段。

2. **核心仓储协议可以先抽最小能力集**
   不需要一开始覆盖所有旧查询能力，只要先覆盖核心执行链真实用到的方法即可。

3. **`execution_intents` 适合作为 PG 首批新增表**
   该数据当前仍偏内存态，历史包袱最少，且对后续 partial-fill / 恢复 / 重启续跑价值很高。

4. **PG sessionmaker 必须惰性初始化**
   迁移期内不能在 import 阶段强依赖 `PG_DATABASE_URL`，否则会直接破坏现有 SQLite 旧业务启动。

---

## 2026-04-22 22:25 -- 数据库迁移方向收口：SQLite 保留，PG 新增实现，核心表先切

### 新增结论

1. **当前目标不是“开发 SQLite / 生产 PG”**
   而是迁移期内采用 **SQLite + PG 双轨并行**：SQLite 继续保留，PG 以新增实现方式接入，核心交易链路优先切到 PG。

2. **问题不在表结构，而在仓储基础设施强绑定 SQLite**
   当前 `orders` 表本身不复杂，字段层面可迁；真正的迁移成本主要来自 Repository 层大量使用 `aiosqlite + 内联 SQL + SQLite 专用连接池/PRAGMA`。
...
4. **迁移策略应是“新增 PG 实现”而不是“重写旧 SQLite 实现”**
   - 保留旧 SQLite Repository
   - 新增 PG 基础设施、ORM、仓储实现
   - 服务层通过最小接口/依赖注入切换

5. **开发顺序必须前置设计与测试**
   先固定设计稿和测试计划，再写 PG 骨架，再让执行开发补实现与测试。
...
## 2026-04-22 19:05 -- 实盘执行链方向收敛：API 退位，进入最小执行主链设计

### 新增结论

1. **API 不再作为未来自动执行主链**
   - API 保留给手工操作 / 查询 / 调试 / 人工干预
   - 自动执行必须直接由内部信号链触发

2. **ExecutionOrchestrator 是必需层**
   - `SignalPipeline` 只负责产出 `SignalResult`
   - `OrderLifecycleService` 只负责订单事实与状态机
   - 需要一个独立的执行编排层把信号、风控、下单、保护单闭环串起来

3. **第一优先级不是继续策略研究，而是收口执行链**
   - 先核对 `ExchangeGateway -> OrderLifecycleService` 的 WS 回写契约
   - 再补 `ENTRY filled -> TP/SL` 自动挂载闭环

4. **必须显式定义危险空窗状态**
   - `entry_filled_unprotected`
   - 表示 ENTRY 已成交但 TP/SL 尚未全部挂好
   - 第一版先做重试 + 告警 + `recovery_required`，暂不默认自动强平
...
## 2026-04-22 18:10 -- 环境边界验证完成：主线适配窗口与失效窗口已明确
...
1. **这条策略不是全周期策略**
   - `2021/2023` 明显不适配
   - `2022` 属于边缘区 / 过渡态
   - `2024/2025/2026 Q1` 为当前已确认适配窗口

2. **2023 失效的真正主因是延续性不足**
   - `MFE` 显著低于 `2024/2025`
   - `first-touch` 更容易先到 `-0.5R`
   - `+1R / +2R / +3.5R` 可达率显著低于 `2024/2025`
   - 结论：问题在于 LONG 信号后续空间不足，而非撮合或 exit bug

3. **简单不开仓条件未通过**
   - `ema_distance_1h`：有解释力，但阈值一落地就明显伤害 `2024/2025`
   - `4h` 结构连续性：无稳定解释力，不值得进入策略
   - 结论：当前没有证据支持用单一简单特征修补 `2021-2023`

### 阶段性判断

**当前主线应进入“接受边界、谨慎落地”阶段，而不是继续追加简单过滤器。**

## 2026-04-22 12:10 -- 2023 失效归因口径修正：撤销 `TP1=0%` 旧结论

### 核心修正

- 旧版 `2023` 归因报告里 `TP1=0%` 的结论是**错误数据源导致的假象**
- 原因：脚本使用了 `Position.exit_reason` 统计 `TP1/TP2/SL`，但该字段只表示**整笔仓位最终结束原因**，不能统计分批止盈事件
- 修正后改为使用 `report.close_events` 统计真实事件占比

### 修正后关键数据

| 年份 | TP1 事件占比 | TP2 事件占比 | SL 事件占比 |
|------|--------------|--------------|-------------|
| 2023 | 27.27% | 11.69% | 61.04% |
| 2024 | 35.42% | 20.83% | 43.75% |
| 2025 | 38.78% | 19.39% | 41.84% |

### 修正后的解释

1. `2023` 并不是“完全打不到 TP1”
2. `2023` 失效主结论仍成立：环境不适配仍是主因
3. 更准确的说法是：`2023` 的趋势延续性更差，导致 `SL` 事件占比显著更高；`exit` 不是独立主因
4. `335.26h` 代表的是**整笔仓位最终结束时间**，不是“到 TP1 的时间”

### 对后续研究的影响

- 不再允许引用“`TP1=0%` 说明 exit 完全失灵”这一旧结论
- `2023` 研究主线继续保留为：`LONG-only` 在熊市/震荡环境中不适配
- 若要继续细化，应研究“环境过滤 / regime filter”，而不是优先回头折腾 `TP1` 机制本身
...
## 2026-04-22 19:25 -- TP 数据边界确认：保留 signal_take_profits，执行层统一收口到 orders

### 结论

当前系统里已经存在两层不同含义的 TP 数据：

1. `signal_take_profits`
   - 属于**信号层**
   - 对应 `SignalResult.take_profit_levels`
   - 用于研究记录、前端展示、建议目标位查看

2. `orders(order_role=TP1/TP2/SL)`
   - 属于**执行层**
   - 表示真实止盈/止损订单事实
   - 需要承接订单状态机、交易所 ID、WS 回写、OCO、审计日志、对账恢复

### 决策

- 不新增专门的执行层 TP 独立事实表
- 后续 PG first scope 中：
  - `signal_take_profits` 可继续暂留研究链
  - 真实执行 `TP1 / TP2 / SL` 统一进入 `orders` 订单链

### 影响

- 避免把“建议目标位”和“真实执行保护单”混为一谈
- 避免后续 PG 迁移时重复设计一张语义重叠的 TP 专表

## 2026-04-22 19:40 -- 执行层设计已按“低频个人量化”场景收敛
...
1. 本地主单先创建，再提交交易所
2. `entry_filled_unprotected` 危险状态
3. `unknown_submitted` 提交超时未知态
4. `WS` 回写契约修复（P0）
5. `recovery_required` 后停止对应币种新开仓
6. 真实 `TP1 / TP2 / SL` 统一进入 `orders` 订单链
...
1. 单进程 `asyncio.Lock(symbol)` 串行化
2. symbol 级熔断
3. `PARTIALLY_FILLED` 要兼容，但不把复杂挂单型 ENTRY 作为 MVP 目标
...
2. `unknown_submitted`
   - 对 `place_order()` 网络超时 / 无法确认交易所是否已接收的场景
   - 不能直接标记为 `failed`
   - 必须转给专项确认 / 对账逻辑

3. 保护单幂等
   - 已成功的 `SL / TP1 / TP2` 重试时必须跳过
   - 避免重复挂单与状态卡死
