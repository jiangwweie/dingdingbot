...
def test_exchange_arm_packet_waits_for_owner_confirmation():
    module = _load_module()

    packet = module.build_exchange_arm_authorization_packet(
        local_registration_report=_local_registration_report(),
    )

    assert packet["status"] == "waiting_for_owner_exchange_arm_authorization"
    assert packet["checks"]["ready_for_owner_exchange_arm_authorization"] is True
    assert packet["checks"]["action_authorized"] is False
    assert packet["authorization_id"] == AUTHORIZATION_ID
    assert packet["owner_confirmation"]["required_value"] == EXCHANGE_ARM_VALUE
    assert packet["operator_command_plan"]["authorized_exchange_arm_command"] is None
    assert packet["safety_invariants"]["api_called"] is False
    assert packet["safety_invariants"]["exchange_called"] is False


def test_exchange_arm_packet_exposes_command_after_exact_confirmation():
    module = _load_module()
    ...
    command = packet["operator_command_plan"]["authorized_exchange_arm_command"]
    assert packet["status"] == "owner_exchange_arm_authorization_packet_ready"
...
def test_exchange_arm_packet_blocks_without_local_registration_result():
    ...
    assert packet["status"] == "blocked_before_exchange_arm_authorization"
    assert "local_registration_evidence_ids_missing" in packet["checks"]["blockers"]
    assert packet["operator_command_plan"]["authorized_exchange_arm_command"] is None


def test_exchange_arm_packet_blocks_when_exchange_already_armed():
    module = _load_module()
    report = _local_registration_report()
    ...
    assert packet["status"] == "blocked_before_exchange_arm_authorization"
    assert "exchange_or_submit_evidence_already_present" in packet["checks"]["blockers"]


def test_exchange_arm_packet_blocks_mismatched_confirmation():
    module = _load_module()
...
def test_exchange_arm_packet_cli_reads_json(tmp_path: Path, capsys):
    module = _load_module()
    ...
    assert json.loads(output_path.read_text())["status"] == (
        "owner_exchange_arm_authorization_packet_ready"
    )
