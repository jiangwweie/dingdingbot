...
def test_builds_owner_codex_runtime_profile_confirmation_record_for_rbr_short() -> None:
    ...
    packet = confirmation_record_script.build_record(
    ...
    assert template["metadata"]["proposal_source_status"] == (
        "ready_for_owner_runtime_profile_decision"
    )
    assert "proposal_packet_status" not in template["metadata"]
    assert "approval_not_recorded_by_this_packet" not in template["metadata"]
    assert template["metadata"]["approval_not_recorded_by_this_record"] is True
...
def test_confirmation_record_can_include_trial_binding_for_future_runtime_draft() -> None:
    proposal_artifact = proposal_script.build_profile_proposal_artifact(
        selector_artifact=_selector_artifact([_rbr_signal()]),
        capital_base=Decimal("30"),
    )
    ...
    plan = packet["profile_confirmation_record_plan"]
    assert "this_packet_creates_runtime" not in plan
    assert plan["this_record_creates_runtime"] is False
    assert plan["this_record_places_order"] is False


def test_confirmation_record_upgrades_legacy_short_proposal_keys() -> None:
    proposal_artifact = proposal_script.build_profile_proposal_artifact(
        selector_artifact=_selector_artifact([_rbr_signal()]),
        capital_base=Decimal("30"),
    )
    proposal_artifact["experimental_runtime_profile_proposal"][
...
def test_confirmation_record_blocks_when_profile_proposal_missing() -> None:
    packet = confirmation_record_script.build_record(
        proposal_artifact={
    ...
    assert packet["status"] == "blocked_runtime_profile_confirmation_record"
    assert packet["blockers"] == ["runtime_profile_proposal_missing"]
    assert packet["promotion_confirmation_request_template"] is None
    assert packet["safety_invariants"]["runtime_created"] is False


def test_confirmation_record_blocks_non_trade_strategy_profile() -> None:
    rmr_signal = {
        **_rbr_signal(),
    ...
    assert packet["status"] == "blocked_runtime_profile_confirmation_record"
    assert "source_profile_artifact_not_ready" in packet["blockers"]
    assert "regime_classifier_not_runtime_trade_strategy" in packet["blockers"]
    assert packet["promotion_confirmation_request_template"] is None
    assert packet["safety_invariants"]["order_created"] is False


def test_confirmation_record_has_no_legacy_packet_builder() -> None:
    assert not hasattr(confirmation_record_script, "build_packet")
