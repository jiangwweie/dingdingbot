...
from src.application.runtime_persisted_draft_source_readiness_adapter_service import (
    ...
    strategy_planning_artifact_from_intent_draft_source,
)
...
async def _ready_source() -> RuntimeStrategySignalIntentDraftSourceArtifact:
...
@pytest.mark.asyncio
async def test_projection_builds_ready_readiness_from_persisted_draft_source():
    ...
    packet = await service.preview_from_intent_draft_source(
        intent_draft_source_artifact=source,
        evidence=_evidence(),
    )
...
@pytest.mark.asyncio
async def test_projection_blocks_when_source_is_not_ready():
    source = await _ready_source()
    ...
    blocked_source = RuntimeStrategySignalIntentDraftSourceArtifact(**values)
    strategy_artifact = strategy_planning_artifact_from_intent_draft_source(
        blocked_source,
    )

    assert strategy_artifact.status.value == "blocked_by_strategy_planning"
    assert "unit_source_blocker" in strategy_artifact.blockers
    assert "intent_draft_source_not_ready" in strategy_artifact.blockers
    assert "runtime_execution_intent_draft_id_missing" in strategy_artifact.blockers

    service = RuntimePersistedDraftSourceReadinessAdapterService()
    packet = await service.preview_from_intent_draft_source(
        intent_draft_source_artifact=blocked_source,
        evidence=_evidence(),
    )
...
async def test_trading_console_persisted_draft_source_readiness_endpoint_blocks_runtime_mismatch():
    ...
    assert exc.value.detail == "intent_draft_source_artifact_runtime_mismatch"
...
def test_persisted_draft_source_readiness_api_flow_posts_request(tmp_path):
    ...
    report = api_flow._build_artifact(
        _args(tmp_path, source_path=source_path, evidence_path=evidence_path),
        client=client,
    )

    assert report["status"] == "ready_for_executable_submit"
    preview = report["operator_action_preview"]
    assert preview["readiness_artifact_id"] == (
        "runtime-executable-submit-readiness-unit"
    )
    assert preview["source_strategy_planning_artifact_id"] == "strategy-plan-unit"
    assert "readiness_packet_id" not in preview
    assert "source_strategy_planning_packet_id" not in preview
    ...
    assert (
        call["body"]["intent_draft_source_artifact"]["runtime_instance_id"]
        == "runtime-rtf014"
    )
    assert call["body"]["non_executing"] is True


def _ready_source_sync() -> RuntimeStrategySignalIntentDraftSourceArtifact:
    scheduler = _scheduler_result(candidate=_candidate())
    draft = _draft()
    return RuntimeStrategySignalIntentDraftSourceArtifact(
        artifact_id="runtime-strategy-signal-intent-draft-source-eval-rtf014",
    ...
    def request_json(self, method, path, *, query=None, body=None):
        ...
        return {
            ...
            "body": {
                ...
                "artifact_id": "runtime-executable-submit-readiness-unit",
                "source_strategy_planning_artifact_id": "strategy-plan-unit",
