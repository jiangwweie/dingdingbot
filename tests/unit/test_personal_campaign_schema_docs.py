...
def test_strategy_contract_schema_enforces_disabled_local_contract_boundary():
    ...
    assert "contract_status" in schema["required"]
    assert properties["contract_status"]["const"] == "frozen"
...
def test_read_only_runtime_adapter_schema_forbids_order_authority():
    schema = _load_schema("read_only_runtime_adapter_preview.schema.json")
    properties = schema["properties"]

    assert properties["read_only"]["const"] is True
    assert properties["authority"]["const"] == "read_only_no_order_authority"
