...
from scripts import runtime_live_signal_routing_artifact as script


def test_routes_runtime_compatible_signal_to_prepare_without_records() -> None:
    artifact = script.build_routing_artifact(
    ...
    assert artifact["status"] == "ready_for_current_runtime_signal_prepare"
    assert artifact["signal_input_json"] == "/tmp/runtime-ready-signal.json"
    assert "operator_command_plan" not in artifact
    assert artifact["signal_routing_plan"]["current_runtime_prepare_allowed"] is True
    assert artifact["signal_routing_plan"]["creates_shadow_candidate"] is False
    assert artifact["signal_routing_plan"]["places_order"] is False
    assert artifact["profile_proposal_artifact"] is None
...
def test_routes_non_runtime_signal_to_owner_profile_proposal() -> None:
    ...
    proposal = artifact["profile_proposal_artifact"]
    ...
    assert artifact["signal_routing_plan"]["requires_owner_runtime_profile_confirmation"] is True
    assert artifact["signal_routing_plan"]["creates_runtime"] is False
    assert artifact["signal_routing_plan"]["places_order"] is False
...
def test_routes_no_signal_to_waiting_without_profile_or_candidate() -> None:
    ...
    assert artifact["blockers"] == ["runtime_strategy_signal_not_found_in_strategy_shelf"]
    assert artifact["signal_routing_plan"]["next_step"] == (
        "continue_live_signal_observation_without_forcing_entry"
    )
    assert artifact["signal_routing_plan"]["creates_shadow_candidate"] is False
    assert artifact["profile_proposal_artifact"] is None
    assert artifact["right_tail_objective_context"]["forcing_entry_without_signal_forbidden"] is True


def test_cli_stdout_is_json_only(monkeypatch, capsys):
    async def fake_selector(args):
        print("inner noisy selector")
        return _selector_artifact(
            status="no_would_enter_signal_available",
            blockers=["runtime_strategy_signal_not_found_in_strategy_shelf"],
        )
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "runtime_live_signal_routing_artifact.py",
            "--runtime-instance-id",
            "runtime-1",
        ],
    )
