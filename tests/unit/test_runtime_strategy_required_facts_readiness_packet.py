...
from scripts.build_runtime_strategy_required_facts_readiness_packet import (
    build_strategy_required_facts_readiness_packet,
)
...
def test_cpm_and_brf_are_ready_when_required_fact_sources_are_fresh() -> None:
    packet = build_strategy_required_facts_readiness_packet(
        fact_sources=_ready_fact_sources(),
        strategies=["CPM-RO-001:CPM-RO-001-v0", "BRF-001:BRF-001-v0"],
        generated_at_ms=1,
    )

    assert packet["status"] == "ready_for_non_executing_strategy_runtime_planning"
    assert packet["operator_policy"]["requires_trusted_account_facts"] is True
    assert packet["operator_policy"]["executable_submit_allowed_by_packet"] is False
    assert {item["semantic_snapshot"]["strategy_family_id"] for item in packet["strategies"]} == {
        "CPM-RO-001",
        "BRF-001",
    }
...
def test_missing_trusted_account_source_blocks_candidate_planning() -> None:
    ...
    assert packet["status"] == "blocked_strategy_required_facts"
    assert packet["strategies"][0]["status"] == "blocked_required_facts"
    assert "account_facts_source_missing" in packet["blockers"]
...
def test_stale_position_projection_blocks_candidate_planning() -> None:
    ...
    assert packet["status"] == "blocked_strategy_required_facts"
    assert "position_projection_source_stale" in packet["blockers"]
...
def test_rmr_and_fco_remain_non_candidate_semantics() -> None:
    ...
    statuses = {
        ...
        for item in packet["strategies"]
    }
    assert packet["status"] == "observe_only_reference_semantics"
    assert statuses == {
        "RMR-001": "observe_only_reference_semantics",
        "FCO-001": "data_backlog_only",
    }
    assert packet["operator_policy"]["does_not_create_shadow_candidate"] is True
...
def test_unknown_strategy_binding_blocks_semantics_readiness() -> None:
    ...
    assert packet["status"] == "blocked_strategy_semantics_missing"
    assert packet["missing_strategy_semantics"][0]["strategy_family_id"] == "UNKNOWN-001"
    assert "strategy_semantics_binding_missing" in packet["blockers"]
