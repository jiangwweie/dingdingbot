...
from scripts import runtime_position_lifecycle_exit_readiness_artifact as script
...
def _monitor_artifact(**overrides):
    payload = {
...
def _followup(**overrides):
    ...
    return {
        "scope": "runtime_post_close_followup_artifact",
...
def test_lifecycle_classifies_hold_or_standing_recovery_ready():
    artifact = script.build_lifecycle_artifact(
    ...
    assert artifact["status"] == "position_lifecycle_hold_or_standing_recovery_ready"
    ...
    assert artifact["exit_path"]["full_reduce_only_close_feasible"] is True
    assert artifact["exit_path"]["tp1_quantity_feasible"] is False
    assert "operator_command_plan" not in artifact
    assert artifact["position_lifecycle_plan"][
        "reduce_only_close_ready_for_owner_authorization"
    ] is False
    assert artifact["position_lifecycle_plan"][
        "reduce_only_recovery_ready_for_standing_authorization"
    ] is True
    assert artifact["position_lifecycle_plan"]["requires_official_operation_layer"] is True
    assert artifact["position_lifecycle_plan"]["execute_reduce_only_close_now"] is False
    assert artifact["position_lifecycle_plan"]["allows_new_attempt_now"] is False
    assert artifact["safety_invariants"]["lifecycle_projection_only"] is True
    assert "packet_only" not in artifact["safety_invariants"]
    assert "pg_called_by_lifecycle_packet" not in artifact["safety_invariants"]
    assert artifact["safety_invariants"]["pg_called_by_lifecycle_artifact"] is False
...
def test_lifecycle_classifies_hard_stop_hold_when_close_packet_absent():
    ...
    assert artifact["status"] == "position_lifecycle_hold_with_hard_stop"
    assert artifact["position_lifecycle_plan"]["next_step"] == (
...
def test_lifecycle_routes_flat_runtime_to_closed_review():
    ...
    assert artifact["status"] == "position_lifecycle_ready_for_closed_review"
    assert artifact["position_lifecycle_plan"]["allows_new_attempt_now"] is False
...
def test_lifecycle_blocks_forbidden_effect():
    ...
    blocked = script.build_lifecycle_artifact(
        gate_classification=_gate_classification(
            safety_invariants={
                "forbidden_effects": {
    ...
    assert blocked["position_lifecycle_plan"]["next_step"] == (
...
def test_lifecycle_cli_tolerates_log_prefixed_json(tmp_path, capsys):
    ...
    monitor_path.write_text(json.dumps(_monitor_artifact()), encoding="utf-8")
    ...
    assert stdout_artifact["status"] == "position_lifecycle_hold_or_standing_recovery_ready"
    assert file_artifact["deployment_context"]["deployed_head"] == "f8871634"
