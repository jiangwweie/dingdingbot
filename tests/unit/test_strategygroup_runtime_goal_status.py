...
def test_goal_status_cli_writes_explicit_local_diagnostic_report(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    assert packet["ready_for_real_order_action"] is False
    assert packet["non_authority_checkpoint"] == "continue_watcher_observation"
    assert packet["evidence"]["candidate_pool_source_mode"] == "not_provided"
    assert packet["evidence"]["legacy_candidate_pool_json_read"] is False
    ...
    assert stdout_packet["evidence"]["candidate_pool_source_mode"] == "not_provided"
    assert stdout_packet["evidence"]["legacy_candidate_pool_json_read"] is False
