...
from scripts import verify_runtime_next_attempt_gate_evidence
...
def test_next_attempt_gate_env_loader_fills_empty_existing_env(monkeypatch, tmp_path):
    ...
    verify_runtime_next_attempt_gate_evidence._load_env_file(str(env_file))
...
def test_next_attempt_gate_scope_uses_runtime_defaults_and_cli_overrides():
    ...
    scope = verify_runtime_next_attempt_gate_evidence._runtime_scope(args, runtime_context)
...
def test_next_attempt_gate_evidence_clears_without_execution_authority(monkeypatch):
    ...
    monkeypatch.setattr(
        verify_runtime_next_attempt_gate_evidence,
    ...
    payload = __import__("asyncio").run(
        verify_runtime_next_attempt_gate_evidence._build_gate_evidence(
            argparse.Namespace(
    ...
    assert payload["scope"] == "runtime_next_attempt_gate_evidence"
    assert payload["status"] == "clear_for_next_attempt_preflight"
    assert "operator_command_plan" not in payload
    assert payload["next_attempt_gate_plan"]["next_preflight_allowed"] is True
    assert payload["next_attempt_gate_plan"]["live_submit_allowed"] is False
...
def test_next_attempt_gate_evidence_blocks_when_lifecycle_gate_blocks(monkeypatch):
    async def fake_runtime_context(runtime_instance_id):
        return {
            "runtime_instance_id": runtime_instance_id,
            "symbol": "AVAX/USDT:USDT",
    ...
    assert "operator_command_plan" not in payload
    assert payload["next_attempt_gate_plan"]["next_preflight_allowed"] is False
...
def test_next_attempt_gate_cli_stdout_is_json_only(monkeypatch, capsys):
    async def fake_build_gate_evidence(args):
    ...
    monkeypatch.setattr(
        ...
        fake_build_gate_evidence,
    )
    monkeypatch.setattr(
        sys,
        "argv",
        [
            "verify_runtime_next_attempt_gate_evidence.py",
            "--runtime-instance-id",
            "runtime-1",
        ],
    )

    assert verify_runtime_next_attempt_gate_evidence.main() == 0
...
def test_next_attempt_gate_evidence_does_not_expose_legacy_packet_builder():
    assert not hasattr(verify_runtime_next_attempt_gate_evidence, "_build_packet")
