...
def test_three_strategy_portfolio_selects_mpg_brf2_and_sor():
    ...
    assert packet["checks"]["cpm_live_trial_portfolio_candidate"] is True
    assert packet["checks"]["cpm_replaces_existing_seat"] is False

    cpm = {
        row["strategy_group_id"]: row
        for row in packet["additional_live_trial_candidates"]
    }["CPM-RO-001"]
    assert cpm["portfolio_relationship"] == "fourth_live_trial_portfolio_candidate"
    assert cpm["does_not_displace_existing_three_standby"] is True
    assert cpm["stage"] == "trial_asset_admission_candidate"
    assert cpm["first_blocker"]["verdict"] == "not_tradable_facts"
    assert cpm["first_blocker"]["first_blocker_class"] == (
        "cpm_required_facts_mapping_gap"
    )
    assert cpm["first_blocker"]["blocker_owner"] == "engineering"
    assert cpm["runtime_readiness"]["armed_observation_ready"] is False
    assert cpm["absorbed_observation_evidence"]["event_count"] == 4
    assert cpm["absorbed_observation_evidence"]["current_authority"] == "observe_only"
