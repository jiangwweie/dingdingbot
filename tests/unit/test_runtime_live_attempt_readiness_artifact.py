...
from scripts import runtime_live_attempt_readiness_artifact as script
...
def test_live_attempt_readiness_classifies_mixed_signal_and_runtime_gate_blockers():
    monitor = _active_monitor_artifact(
        [
            _summary(
                "runtime-ada",
    ...
    artifact = script.build_readiness_artifact(
        active_monitor_artifact=monitor,
    ...
    assert artifact["status"] == "live_attempt_blocked_by_runtime_or_signal_gate"
    assert artifact["readiness_counts"]["waiting_signal_count"] == 2
    assert artifact["readiness_counts"]["runtime_gate_blocked_count"] == 1
    assert artifact["active_runtime_count"] == 3
    assert artifact["safety_invariants"]["no_forbidden_live_side_effects"] is True
    assert artifact["safety_invariants"]["no_prepare_records_created_in_this_artifact"] is True
    assert "no_prepare_records_created_in_this_packet" not in artifact["safety_invariants"]
    assert "operator_command_plan" not in artifact
    assert artifact["live_attempt_readiness_plan"]["places_order"] is False
    assert artifact["live_attempt_readiness_plan"]["calls_exchange"] is False
    assert artifact["live_attempt_readiness_plan"][
        "uses_legacy_pre_attempt_rehearsal_as_primary_gate"
    ] is False
    assert artifact["deployment_context"]["deployed_head"] == "20843e89"
    assert artifact["deployment_context"]["live_ready"] is False
...
def test_live_attempt_readiness_waits_when_all_runtimes_wait_for_signal():
    ...
    artifact = script.build_readiness_artifact(active_monitor_artifact=monitor)

    assert artifact["status"] == "live_attempt_waiting_for_strategy_signal"
    assert artifact["live_attempt_readiness_plan"]["next_step"] == (
...
def test_live_attempt_readiness_reports_ready_for_prepare_review():
    ...
    assert artifact["status"] == "live_attempt_ready_for_prepare_review"
    assert artifact["readiness_counts"]["ready_for_prepare_count"] == 1
    assert artifact["live_attempt_readiness_plan"]["creates_shadow_candidate"] is False
...
def test_live_attempt_readiness_blocks_forbidden_effects():
    ...
    assert artifact["status"] == "live_attempt_blocked_forbidden_effect"
...
def test_live_attempt_readiness_cli_writes_json(tmp_path, capsys):
    ...
    assert stdout_artifact["status"] == "live_attempt_waiting_for_strategy_signal"
    assert file_artifact["deployment_context"]["deployed_head"] == "20843e89"
