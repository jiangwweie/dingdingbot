...
def test_daily_table_accepts_fresh_absent_action_time_row_as_market_wait_path():
    builder = _builder()
    candidate_pool = _candidate_pool()
    for row in candidate_pool["symbol_readiness_rows"]:
            ...
            row.update(
                {
                    ...
                    "action_time": {
                        ...
                        "first_blocker": "fresh_cpm-long_absent",
                        "public_facts_ready": False,
                        "private_action_time_facts_ready": False,
                    },
                    "server_runtime_coverage": {
                        "state": "active_watcher_scope",
                        "active_runtime_instance_ids": ["runtime-cpm-eth"],
                        "selected_runtime_instance_ids": ["runtime-cpm-eth"],
                    },
                }
            )
    ...
    cpm_row = next(row for row in table["rows"] if row["strategy_group_id"] == "CPM-RO-001")
    validation = cpm_row["market_wait_validation"]
    assert cpm_row["first_blocker"] == "market_wait_validated"
    assert validation["valid"] is True
    assert validation["checks"]["action_time_path"] is True
    assert validation["checks"]["fresh_signal_absent"] is True
    assert _errors(table) == []
