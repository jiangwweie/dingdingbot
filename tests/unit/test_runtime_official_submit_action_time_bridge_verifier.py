...
from scripts.verify_runtime_official_submit_action_time_bridge import (
    build_official_submit_action_time_bridge_report,
)


@pytest.mark.asyncio
async def test_official_submit_action_time_bridge_covers_disabled_and_refusals(tmp_path):
    report = await build_official_submit_action_time_bridge_report(
        work_dir=tmp_path / "action-time-inputs",
    )

    assert report["status"] == "rtf051_official_submit_action_time_bridge_passed"
...
def test_official_submit_action_time_bridge_script_writes_json(tmp_path):
    output_path = tmp_path / "rtf051-official-submit-action-time-bridge.json"
