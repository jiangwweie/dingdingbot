...
def test_first_real_submit_enablement_packet_can_be_ready_without_authority():
    first_submit = _first_submit_confirmed()
    packet = build_runtime_execution_first_real_submit_enablement_packet(
    ...
    assert packet.promotion_gate_ready is True
    assert packet.submit_rehearsal_ready is True
    assert packet.duplicate_submit_replay_proof_ready is True
    assert packet.duplicate_submit_replay_proof is not None
    assert packet.duplicate_submit_replay_proof.status == (
        RuntimeExecutionDuplicateSubmitReplayProofStatus
        .READY_FOR_FIRST_REAL_SUBMIT_REPLAY_GUARD
    )
    assert packet.prerequisite_evidence_proof_ready is True
    assert packet.prerequisite_evidence_proof is not None
    assert packet.prerequisite_evidence_proof.status == (
        RuntimeExecutionSubmitPrerequisiteEvidenceProofStatus
    ...
    assert packet.not_live_action_authorization is True
    assert packet.not_exchange_submit_authority is True
    assert packet.not_order_lifecycle_authority is True
    ...
    assert packet.owner_bounded_execution_called is False
    assert packet.runtime_state_mutated is False
    assert packet.withdrawal_or_transfer_created is False


def test_first_real_submit_enablement_packet_blocks_missing_owner_evidence():
    first_submit = _first_submit_confirmed(
    ...
    assert packet.status == RuntimeExecutionFirstRealSubmitEnablementPacketStatus.BLOCKED
    assert "submit_rehearsal_not_ready" in packet.blockers
    assert (
...
def test_first_real_submit_enablement_packet_blocks_stale_gateway_readiness():
    first_submit = _first_submit_confirmed()
    packet = build_runtime_execution_first_real_submit_enablement_packet(
        submit_rehearsal=_submit_rehearsal(
    ...
    assert packet.submit_rehearsal_ready is False
    assert packet.exchange_called is False


def test_first_real_submit_enablement_packet_blocks_missing_duplicate_replay_proof():
    first_submit = _first_submit_confirmed()
    packet = build_runtime_execution_first_real_submit_enablement_packet(
        submit_rehearsal=_submit_rehearsal(),
        first_real_submit_confirmations=first_submit,
    ...
    assert packet.status == RuntimeExecutionFirstRealSubmitEnablementPacketStatus.BLOCKED
    assert "duplicate_submit_replay_proof_missing" in packet.blockers
    assert packet.duplicate_submit_replay_proof_ready is False
    assert packet.exchange_called is False


def test_first_real_submit_enablement_packet_blocks_missing_prerequisite_proof():
    first_submit = _first_submit_confirmed()
    packet = build_runtime_execution_first_real_submit_enablement_packet(
        submit_rehearsal=_submit_rehearsal(),
        first_real_submit_confirmations=first_submit,
    ...
    assert packet.status == RuntimeExecutionFirstRealSubmitEnablementPacketStatus.BLOCKED
    assert "prerequisite_evidence_proof_missing" in packet.blockers
    assert packet.prerequisite_evidence_proof_ready is False
    assert packet.exchange_called is False


def test_first_real_submit_enablement_packet_blocks_unready_prerequisite_proof():
    first_submit = _first_submit_confirmed()
    ...
    assert packet.status == RuntimeExecutionFirstRealSubmitEnablementPacketStatus.BLOCKED
    assert "prerequisite_evidence_proof_not_ready" in packet.blockers
    assert (
...
def test_first_real_submit_enablement_packet_rejects_execution_metadata():
    packet = build_runtime_execution_first_real_submit_enablement_packet(
        submit_rehearsal=_submit_rehearsal(),
    ...
    with pytest.raises(ValueError, match="forbidden execution field"):
        RuntimeExecutionFirstRealSubmitEnablementPacket.model_validate(payload)


@pytest.mark.asyncio
async def test_first_real_submit_enablement_packet_service_defaults_evidence_ids():
    calls: list[dict] = []
    ...
    assert packet.first_real_submit_confirmations.exchange_submit_enablement_decision_id == (
        "runtime-exchange-submit-enable-auth-1"
    )
    assert packet.first_real_submit_confirmations.runtime_submit_rehearsal_id == (
...
@pytest.mark.asyncio
async def test_first_real_submit_enablement_packet_service_resolves_recorded_evidence_ids():
    calls: list[dict] = []
...
async def test_first_real_submit_enablement_packet_blocks_mismatched_evidence_ids():
    ...
    assert packet.status == RuntimeExecutionFirstRealSubmitEnablementPacketStatus.BLOCKED
    assert "exchange_submit_enablement_decision_id_mismatch" in packet.blockers
    assert "runtime_submit_rehearsal_id_mismatch" in packet.blockers
    assert packet.order_created is False
    assert packet.order_lifecycle_called is False
    assert packet.exchange_called is False
    assert packet.withdrawal_or_transfer_created is False
...
@pytest.mark.asyncio
async def test_first_real_submit_enablement_packet_blocks_underlying_evidence_mismatch():
    class FakeAdapterService:
        async def exchange_submit_enablement_decision_for_authorization(
            self,
            authorization_id,
    ...
    assert packet.status == RuntimeExecutionFirstRealSubmitEnablementPacketStatus.BLOCKED
    assert "trusted_submit_fact_snapshot_id_mismatch" in packet.blockers
    assert packet.submit_rehearsal.trusted_submit_fact_snapshot_id == (
...
async def test_trading_console_first_real_submit_packet_api(monkeypatch):
    ...
    packet = await (
        trading_console_api
        .runtime_execution_first_real_submit_enablement_packet_for_authorization(
            "auth-1",
            strategy_family_confirmed=True,
            implementation_source_confirmed=True,
            required_facts_confirmed=True,
    ...
    assert packet.promotion_gate_result is not None
