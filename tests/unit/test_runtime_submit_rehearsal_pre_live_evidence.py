...
@pytest.mark.asyncio
async def test_pre_live_evidence_blocks_current_head_not_deployed_and_owner_auth_missing():
    module = _load_module()
    assert not hasattr(module, "build_pre_live_packet")

    report = await module.build_pre_live_evidence(
    ...
    assert report["scope"] == "runtime_submit_rehearsal_pre_live_evidence"
...
@pytest.mark.asyncio
async def test_pre_live_evidence_can_read_release_manifest_without_git(
    monkeypatch,
    tmp_path,
):
    module = _load_module()
...
@pytest.mark.asyncio
async def test_pre_live_evidence_still_blocks_when_owner_and_deploy_gates_are_present():
    module = _load_module()
    ...
    assert report["first_real_submit_evidence"]["status"] == "blocked"
    assert report["pipeline"]["submit_rehearsal_status"] == "blocked"
    assert report["pipeline"]["submit_adapter_preview_status"] == (
        "inputs_ready_adapter_not_implemented"
    )
...
@pytest.mark.asyncio
async def test_pre_live_evidence_can_exercise_local_registration_before_exchange_submit():
    module = _load_module()
    ...
    submit_preview = local_rehearsal["exchange_submit_preview"]
    ...
    assert submit_preview["status"] == "ready_for_exchange_submit_adapter_design"
    assert submit_preview["exchange_called"] is False
    assert submit_preview["exchange_order_submitted"] is False
...
@pytest.mark.asyncio
async def test_pre_live_evidence_can_reach_exchange_submit_adapter_pre_execution_boundary():
    module = _load_module()
...
@pytest.mark.asyncio
async def test_pre_live_evidence_can_simulate_enabled_exchange_execution_in_memory():
    module = _load_module()
