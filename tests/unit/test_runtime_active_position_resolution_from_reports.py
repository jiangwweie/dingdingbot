...
def test_resolution_from_reports_handles_noisy_json_prefix(tmp_path):
    ...
    monitor_path.write_text(
        json.dumps({"packet": _monitor().model_dump(mode="json")}),
        encoding="utf-8",
    )
    ...
    followup_path.write_text(
        json.dumps({"packet": _followup().model_dump(mode="json")}),
        encoding="utf-8",
    )

    packet = runtime_active_position_resolution_from_reports._build_packet(
        argparse.Namespace(
    ...
    assert packet["status"] == "hold_with_hard_stop"
    assert packet["packet"]["next_attempt_blocked_by_active_position"] is True
    assert packet["packet"]["full_reduce_only_close_feasible"] is True
