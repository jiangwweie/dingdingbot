...
def _policy_confirmation() -> dict:
    policy_points = [
        (
            "BRF-001",
            "signal-observation-brf-001",
            "promote_review_lane_approved_without_live_scope_change",
        ),
        (
            "BTPC-001",
            "signal-observation-btpc-001",
            "keep_l2_shadow_and_continue_fact_classifier_revision",
        ),
        (
            "LSR-001",
            "signal-observation-lsr-001",
            "short_revival_rewrite_review_lane_approved",
        ),
        (
            "MI-001",
            "signal-observation-mi-001",
            "formal_candidate_review_opened_without_registry_admission",
        ),
        (
            "CPM-RO-001",
            "signal-observation-cpm-ro-001",
            "observation_asset_merge_review_opened_without_registry_admission",
        ),
        (
            "MPG-001",
            "signal-observation-mpg-001",
    ...
    return {
        ...
        "confirmed_policy_items": [
            {
                "policy_item_id": f"{group}:owner_policy_decision",
                "strategy_group_id": group,
                "queue_id": queue_id,
                "review_only_policy_effect": effect,
            }
            for group, queue_id, effect in policy_points
        ],
            ...
            "policy_items": [
                {
                    ...
                    "strategy_review_checkpoint": f"{group}_next_review",
                }
                for group in groups
            ]
        },
...
def _btpc_artifact(**counts) -> dict:
    return {
        "status": "ready",
        "counts": counts,
...
def _build_artifact() -> dict:
    ...
    return module.build_review_only_evidence_closure_wave(
        ...
        btpc_fact_quality=_btpc_artifact(fact_gap_count=5, fact_source_pending_count=4),
        btpc_source_mapping=_btpc_artifact(
            expected_live_fact_source_count=8,
            source_attachment_pending_count=8,
            live_required_fact_gap_count=8,
        ),
        btpc_classifier_review=_btpc_artifact(
            rule_review_count=2,
            implementation_ready_count=2,
        ),
        btpc_keep_revise=_btpc_artifact(
            proxy_reviewable_would_enter_count=2,
            revise_case_count=3,
        ),
        btpc_proxy_replay=_btpc_artifact(replay_case_count=5),
    )
...
def test_review_only_evidence_closure_wave_builds_three_phases():
    ...
    assert artifact["status"] == "review_only_evidence_closure_wave_ready"
    assert artifact["phase_status"]["phase_1_owner_perception_projection"] == "ready"
    assert artifact["phase_status"]["phase_2_evidence_closure_queue"] == "ready"
    assert artifact["phase_status"]["phase_3_next_owner_policy_package"] == (
        "ready_for_owner_policy"
    )
    assert artifact["owner_progress_projection"]["p0_state"] == "waiting_for_market"
    assert artifact["owner_progress_projection"]["signal_observation_review_state"] == (
        "review_only_evidence_closure_active"
    )
    assert len(artifact["evidence_closure_artifacts"]) == 6
    assert "evidence_closure_packets" not in artifact
    assert {
        row["status"] for row in artifact["evidence_closure_artifacts"]
    } == {"review_only_evidence_artifact_ready"}
    assert artifact["owner_progress_projection"]["evidence_artifact_count"] == 6
    assert artifact["next_owner_policy_package"]["owner_policy_item_count"] == 6
...
def test_review_only_evidence_closure_wave_keeps_live_authority_closed():
    ...
    assert artifact["completion_boundary"]["owner_policy_confirmation_required_now"] is True
    assert artifact["completion_boundary"]["runtime_owner_intervention_required"] is False
    assert all(
        "real_order_authority" not in row
        for row in artifact["evidence_closure_artifacts"]
    )

    policy_items = {
        policy_item["strategy_group_id"]: policy_item
        for policy_item in artifact["next_owner_policy_package"]["owner_policy_items"]
    }
    assert policy_items["MI-001"]["default_recommendation"] == (
        "open_formal_candidate_review_with_overlap_and_concentration_checks"
    )
    assert policy_items["MPG-001"]["does_not_authorize_live_execution"] is True
...
def test_review_only_evidence_closure_wave_rejects_legacy_authority_mirror():
    module = _load_module()
    policy_confirmation = _policy_confirmation()
    policy_confirmation["safety_invariants"]["real_order_authority"] = True
...
def test_review_only_evidence_closure_wave_rejects_actionable_now_authority_mirror():
    module = _load_module()
    policy_confirmation = _policy_confirmation()
    policy_confirmation["safety_invariants"]["actionable_now"] = True
...
def test_review_only_evidence_closure_wave_cli_writes_outputs(tmp_path, capsys):
    ...
    inputs = {
        ...
        "owner.json": _owner_policy_package(),
        "btpc-fact.json": _btpc_artifact(fact_gap_count=5),
        "btpc-source.json": _btpc_artifact(source_attachment_pending_count=8),
        "btpc-classifier.json": _btpc_artifact(rule_review_count=2),
        "btpc-keep.json": _btpc_artifact(proxy_reviewable_would_enter_count=2),
        "btpc-proxy.json": _btpc_artifact(replay_case_count=5),
    }
    ...
    assert artifact["next_owner_policy_package"]["status"] == (
        "next_owner_policy_package_ready"
    )
    assert "evidence_closure_packets" not in artifact
    assert "Evidence Closure Artifacts" in markdown
    assert "Real order authority" not in markdown
    assert "StrategyGroup Owner Progress" in owner_progress_text


def test_review_only_evidence_closure_wave_cli_omitted_inputs_do_not_read_defaults(
    tmp_path,
):
    module = _load_module()
    ...
    try:
        module.main(
            [
                ...
                "--output-md",
                str(tmp_path / "wave.md"),
                "--output-owner-progress",
                str(tmp_path / "owner-progress.md"),
            ]
        )
    except ValueError as exc:
        assert "inputs are not ready" in str(exc)
    else:
        raise AssertionError("expected omitted review-only wave inputs to fail")
