...
def test_signal_input_packet_env_loader_fills_empty_existing_env(monkeypatch, tmp_path):
    env_file = tmp_path / "runtime.env"
    env_file.write_text("PG_DATABASE_URL=postgresql+asyncpg://signal")
    monkeypatch.setenv("PG_DATABASE_URL", "")

    build_runtime_strategy_signal_input_packet._load_env_file(str(env_file))
...
def test_build_btpc_signal_input_uses_runtime_boundary_and_placeholder_account():
    signal_input = build_runtime_strategy_signal_input_packet._build_signal_input(
        runtime=_runtime(),
        one_hour=_btpc_1h(),
...
def test_signal_input_packet_ready_for_btpc_shadow_prepare(monkeypatch, tmp_path):
    ...
    payload = __import__("asyncio").run(
        build_runtime_strategy_signal_input_packet._build_packet(
            argparse.Namespace(
                runtime_instance_id="runtime-btpc-1",
                env_file=None,
                source="live_market",
...
def test_signal_input_packet_cli_stdout_is_json_only(monkeypatch, capsys):
    async def fake_build_packet(args):
        print("noisy market source")
        return {"status": "ready_for_shadow_candidate_prepare", "ok": True}

    monkeypatch.setattr(build_runtime_strategy_signal_input_packet, "_build_packet", fake_build_packet)
    monkeypatch.setattr(
        sys,
        "argv",
        [
            "build_runtime_strategy_signal_input_packet.py",
            "--runtime-instance-id",
            "runtime-1",
        ],
    )

    assert build_runtime_strategy_signal_input_packet.main() == 0
