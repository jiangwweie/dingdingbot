...
from scripts.verify_runtime_next_attempt_submit_preparation_evidence import (
    build_next_attempt_submit_preparation_evidence_report,
)


@pytest.mark.asyncio
async def test_next_attempt_submit_preparation_evidence_covers_ready_and_blocks():
    report = await build_next_attempt_submit_preparation_evidence_report()

    assert report["status"] == "rtf050_next_attempt_submit_preparation_evidence_passed"
    ...
    assert "readiness_packet" not in ready
    assert "disabled_handoff_packet" not in ready
    assert "real_mode_handoff_packet" not in ready
    assert ready["readiness_artifact"]["status"] == "ready_for_executable_submit"
    assert ready["disabled_handoff_artifact"]["status"] == "ready_for_official_submit_call"
    assert ready["real_mode_handoff_artifact"]["status"] == "ready_for_official_submit_call"
...
def test_next_attempt_submit_preparation_evidence_script_writes_json(tmp_path):
    output_path = tmp_path / "rtf050-next-attempt-submit-preparation-evidence.json"

    completed = subprocess.run(
        [
            sys.executable,
            "scripts/verify_runtime_next_attempt_submit_preparation_evidence.py",
            "--output-json",
            str(output_path),
        ],
        check=True,
