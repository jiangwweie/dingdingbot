...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "build_runtime_coverage_review_packet.py"
)


def _load_module():
    spec = importlib.util.spec_from_file_location(
        "build_runtime_coverage_review_packet",
        SCRIPT_PATH,
    )
...
def test_coverage_review_surfaces_running_narrow_coverage_without_execution():
    ...
    packet = module.build_coverage_review_packet(_operator_packet())

    assert packet["status"] == "coverage_review_observation_running"
    assert packet["summary"]["coverage_ratio"] == "2/5"
    assert packet["coverage"]["uncovered_preview_families"] == [
        "BRF-001",
        "LSR-001",
        "RBR-001",
    ]
    assert packet["operator_command_plan"]["starts_runtime"] is False
    assert packet["operator_command_plan"]["changes_strategy_parameters"] is False
    assert packet["operator_command_plan"]["places_order"] is False
    assert packet["owner_gate"]["coverage_review_only"] is True


def test_coverage_review_after_window_requests_owner_review_not_runtime_start():
    module = _load_module()

    packet = module.build_coverage_review_packet(
        _operator_packet(stop_reason="max_iterations_reached")
    )

    assert packet["status"] == "coverage_review_needed_after_no_signal_window"
    ...
    assert "new runtime activation" in packet["owner_gate"]["does_not_authorize"]
...
def test_coverage_review_defers_to_ready_signal():
    ...
    assert packet["status"] == "runtime_signal_ready_not_coverage_review"
    assert packet["operator_command_plan"]["allowed_next_actions"] == [
...
def test_coverage_review_blocks_forbidden_source_effect():
    ...
    packet = module.build_coverage_review_packet(_operator_packet(forbidden=True))
...
def test_coverage_review_cli_reads_and_writes_json(tmp_path, capsys):
    ...
    assert file_payload["scope"] == "runtime_coverage_review_packet"
