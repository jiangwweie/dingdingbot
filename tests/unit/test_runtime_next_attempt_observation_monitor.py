...
def test_monitor_waiting_signal_runs_requested_cycles_without_side_effects():
    ...
    payload = runtime_next_attempt_observation_monitor._build_monitor_packet(
        _args(max_cycles=3),
    ...
    assert payload["operator_command_plan"]["next_step"] == "wait_for_next_observation_cycle"
...
def test_monitor_stops_when_ready_for_prepare():
    ...
    assert payload["operator_command_plan"]["signal_input_json"] == "/tmp/ready-signal.json"
...
def test_monitor_stops_on_blocked_by_default():
    ...
    assert payload["operator_command_plan"]["next_step"] == "resolve_latest_observation_blocker"
...
def test_monitor_returns_blocked_json_when_cycle_raises():
    ...
    assert payload["latest_packet"]["exception"]["message"] == "auth config missing"
    assert payload["safety_invariants"]["exchange_write_called"] is False
