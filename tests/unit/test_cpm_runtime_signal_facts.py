...
def test_cpm_runtime_signal_facts_rejects_legacy_fallback_json_arg(tmp_path: Path):
    module = _load_module()
    fallback_path = tmp_path / "fallback.json"

    with pytest.raises(SystemExit):
        module.main(["--fallback-json", str(fallback_path)])

    assert not hasattr(module, "_fallback_runtime_signal_facts")
