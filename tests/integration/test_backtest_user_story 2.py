...
class MockExchangeGateway:
    ...
    Generates candles with natural wicks (2% range) and injects
    pinbar patterns at regular intervals to ensure signal detection.

    Pinbar injection pattern (every 10th candle):
    ...
    async def fetch_historical_ohlcv(self, symbol: str, timeframe: str, limit: int = 100):
        """Return mock K-line data with realistic OHLCV patterns including pinbars."""
        ...
        for i in range(limit):
            ...
            # Every 10th candle: inject a clear pinbar pattern
            if i > 0 and i % 10 == 0:
                if i % 20 == 0:
                    # Bullish pinbar: long lower wick, small body at top
                    # wick_ratio = (price - low) / (high - low) > 0.6
                    # body_ratio = (close - open) / (high - low) < 0.3
                    low = price * Decimal("0.95")       # 5% lower wick
                    high = price * Decimal("1.005")     # 0.5% upper wick
                    o = price * Decimal("0.995")        # body near top
                    c = price * Decimal("1.002")        # close slightly above open
                else:
                    # Bearish pinbar: long upper wick, small body at bottom
                    high = price * Decimal("1.05")      # 5% upper wick
                    low = price * Decimal("0.995")      # 0.5% lower wick
                    o = price * Decimal("1.002")        # body near bottom
                    c = price * Decimal("0.995")        # close slightly below open
                ...
                klines.append(KlineData(
                    ...
                    low=low,
                    close=c,
                    volume=Decimal("5000"),  # Higher volume on pinbar
                    is_closed=True,
                ))
            else:
                # Normal candle: 2% range, no pinbar
                klines.append(KlineData(
                    symbol=symbol,
                    timeframe=timeframe,
                    timestamp=timestamp,
...
# ============================================================
# FIX: Module-scoped fixtures ensure all tests in this module share
# the SAME temporary database file. Function-scoped fixtures created
# a new temp dir per test method, causing data isolation breaks:
#   Step 1: tmp_aaa/test.db ← writes 18 signals
#   Step 2: tmp_bbb/test.db ← fresh empty DB, reads total=0 → SKIP
# ============================================================
@pytest.fixture(scope="module")
def temp_config_dir():
    """Create temporary directory with valid test config files (shared across module)."""
...
def temp_db_path(temp_config_dir):
    """Return path to temporary database file (shared across module)."""
    return str(Path(temp_config_dir) / "test.db")


@pytest.fixture(scope="module")
def config_manager(temp_config_dir, temp_db_path):
    """Create ConfigManager with test config (database-driven, shared across module).
...
@pytest.fixture(scope="module")
def mock_gateway():
    """Create mock exchange gateway (shared across module)."""
    return MockExchangeGateway()


@pytest.fixture(scope="module")
def mock_repository(config_manager):
    """Create a real SignalRepository sharing ConfigManager's DB connection (shared across module).
...
def test_client(config_manager, mock_gateway, mock_repository):
    """Create FastAPI TestClient with injected dependencies (shared across module).
...
class TestPMSBacktestFlow:
    ...
    def test_step_1_initiate_pms_backtest(self, test_client, mock_gateway):
            ...
            "strategies": [
                {
                    ...
                    "filters": [
                        ...
                        {"type": "atr", "params": {"period": 14, "min_atr_ratio": 0.005}},
                        {"type": "mtf", "params": {}},
                    ],
                    "filter_logic": "AND",
        ...
        resp_json = response.json()
        if "error" in resp_json:
            # DEBUG: print error before skip
            print(f"\n[PMS DEBUG] Error: {resp_json['error']}")
            import pytest
        ...
        # DEBUG: print position count
        print(f"\n[PMS DEBUG] PMS backtest: {len(positions)} positions, {total_trades} trades")
        if positions:
            pos = positions[0]
