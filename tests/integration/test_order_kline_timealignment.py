...
@pytest.mark.asyncio
async def test_order_chain_timeline_alignment(order_repository):
    ...
    # 保存订单链
    await order_repository.save_order(entry)
    await order_repository.save_order(tp1)
    await order_repository.save_order(sl)
    ...
    with patch('ccxt.async_support.binanceusdm') as mock_exchange_class:
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            response = await ac.get(
                f"/api/v3/orders/{entry.id}/klines?symbol=BTC/USDT:USDT"
            )
        ...
        # 3. 验证订单时间戳在 K 线范围内
        kline_timestamps = [k[0] for k in data["klines"]]
        ...
        assert order_data["filled_at"] == entry_time


@pytest.mark.asyncio
async def test_partial_filled_order_chain(order_repository):
    ...
    await order_repository.save_order(entry)
    await order_repository.save_order(tp1)
    await order_repository.save_order(tp2)
    await order_repository.save_order(sl)
    ...
    with patch('ccxt.async_support.binanceusdm') as mock_exchange_class:
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            response = await ac.get(
                f"/api/v3/orders/{entry.id}/klines?symbol=ETH/USDT:USDT"
            )
        ...
        # 验证订单信息
        order_data = data["order"]
        assert order_data["order_id"] == entry.id
...
@pytest.mark.asyncio
async def test_no_filled_at_fallback(order_repository):
    ...
    with patch('ccxt.async_support.binanceusdm') as mock_exchange_class:
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            response = await ac.get(
                f"/api/v3/orders/{order.id}/klines?symbol=BTC/USDT:USDT"
            )
...
@pytest.mark.asyncio
async def test_multi_order_timeline_alignment(order_repository):
...
@pytest.mark.asyncio
async def test_kline_range_covers_full_order_cycle(order_repository):
    ...
    await order_repository.save_order(entry)
    await order_repository.save_order(tp4)
...
@pytest.mark.asyncio
async def test_order_chain_with_multiple_tp_levels(order_repository):
    ...
    for i, (role, price, fill_time) in enumerate([
        ...
        tp_orders.append(tp)
        await order_repository.save_order(tp)
...
@pytest.mark.asyncio
async def test_very_old_order_kline_fetch(order_repository):
