...
def test_stable_policy_v4_contract_defers_deployed_identity_to_roadmap() -> None:
    ...
    # The roadmap owns volatile Owner controls; resumed trading must not fail
    # architecture certification solely because an old Policy was paused.
    assert re.search(
        r"\| Owner controls \| Policy version `[1-9][0-9]*` has "
        r"`new_entry_submit_enabled=(?:true|false)`", roadmap,
    )
    assert re.search(r"\| Runtime ownership \|[^\n]*Entry[^\n]*fence", roadmap)
    assert "0007_exit_profile_authority_v1" in roadmap
    assert "| Dynamic Selection postflight |" in roadmap
