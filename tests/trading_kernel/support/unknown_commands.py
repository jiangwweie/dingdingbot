...
def cancel_command() -> ExchangeCommand:
    ticket = make_issue_request(ticket=make_ticket(), now_ms=1_001, claim_owner="unit-test").capacity_claim.to_ticket()
    command_id = build_command_id(ticket_id=ticket.identity.ticket_id, kind=ExchangeCommandKind.CANCEL_ORDER, generation=1)
    return ExchangeCommand(
        ...
        payload=CancelCommandPayload(exchange_order_id="venue-stop-1", order_namespace="regular", purpose="runner_old_stop"),
