...
EXPECTED_EVENTS = {
    ...
    ("RSRVCB-001", "RSRVCB-LONG-15M", "long"),
}


def test_registry_contains_the_seven_owner_accepted_events() -> None:
...
def test_registry_preserves_exact_fact_semantics_without_membership() -> None:
    ...
    assert contracts["RSRVCB-LONG-15M"].required_fact_names == (
        "regime_eligible",
        "rsr_top2_eligible",
        "vcb_armed",
...
def test_registry_uses_explicit_versioned_identities() -> None:
    for contract in registered_strategy_contracts():
        assert contract.strategy_version_id == (
            f"sgv:{contract.strategy_group_id}:v{contract.strategy_version}"
        )
        assert contract.event_spec_id == (
            f"event_spec:{contract.strategy_group_id}:{contract.event_id}:"
            f"v{contract.event_version}"
        )
...
def test_registry_semantic_hash_does_not_include_universe_members() -> None:
    source = (
        "src.trading_kernel.domain.strategy_registry.RegisteredStrategyContract"
    )
    assert "candidate_instruments" not in RegisteredStrategyContract.model_fields
    assert "venue_symbols" not in RegisteredStrategyContract.model_fields
    assert source
