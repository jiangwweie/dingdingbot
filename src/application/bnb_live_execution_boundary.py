"""BNB one-shot live execution boundary dry run.
...
class BnbLiveExecutionBoundaryDryRunRequest(BaseModel):
...
class BnbLiveExecutionBoundaryTableAudit(BaseModel):
...
class BnbLiveExecutionBoundaryAuthorizationState(BaseModel):
    exists: bool
    status: str
...
class BnbLiveExecutionBoundaryGateFactState(BaseModel):
    state: str
    status: str
    source: str
...
class BnbLiveExecutionBoundaryPersistenceReadiness(BaseModel):
    execution_intents: bool
    orders: bool
...
class BnbLiveExecutionBoundaryFinalGateReadModel(BaseModel):
    result: Literal["passed", "blocked"]
    exact_blockers: list[str] = Field(default_factory=list)
    runtime_safety_state: str
    startup_guard: BnbLiveExecutionBoundaryGateFactState
    gks: BnbLiveExecutionBoundaryGateFactState
    account_facts: BnbLiveExecutionBoundaryGateFactState
    market_metadata: BnbLiveExecutionBoundaryGateFactState
    protection_readiness: BnbLiveExecutionBoundaryGateFactState
    recording_readiness: BnbLiveExecutionBoundaryGateFactState
    active_position: BnbLiveExecutionBoundaryGateFactState
    open_order: BnbLiveExecutionBoundaryGateFactState
    bnb_position: BnbLiveExecutionBoundaryGateFactState
    bnb_open_order: BnbLiveExecutionBoundaryGateFactState
    persistence_readiness: BnbLiveExecutionBoundaryPersistenceReadiness
    execution_boundary_status: BoundaryStatus
...
class BnbLiveExecutionBoundaryDryRunResponse(BaseModel):
    generated_from: Literal["bnb_live_execution_boundary_dry_run_v1"] = (
        "bnb_live_execution_boundary_dry_run_v1"
    )
    ...
    symbol: str
    side: Literal["long", "short"]
    projection_status: BoundaryStatus
    final_preflight_result: Literal["passed", "blocked"]
    hard_blockers: list[str] = Field(default_factory=list)
    authorization_state: BnbLiveExecutionBoundaryAuthorizationState
    final_gate_read_model: BnbLiveExecutionBoundaryFinalGateReadModel
    ...
    table_audit: BnbLiveExecutionBoundaryTableAudit
...
class BnbLiveExecutionBoundaryDryRunService:
    def __init__(
        self,
        *,
    ...
    async def run(
        self,
        request: BnbLiveExecutionBoundaryDryRunRequest | None = None,
        *,
        fact_snapshot: TrialPreflightFactsSnapshot | None = None,
    ) -> BnbLiveExecutionBoundaryDryRunResponse:
        request = request or BnbLiveExecutionBoundaryDryRunRequest()
        ...
        return BnbLiveExecutionBoundaryDryRunResponse(
    ...
    async def run_action_spec(
        ...
        *,
        ...
        return response.model_copy(
            update={
                **action_spec_provenance,
                "projection_status": "blocked_before_execution_boundary",
                ...
                "owner_execution_trigger": response.owner_execution_trigger.model_copy(
                    update={
                        ...
                        "creates_execution_intent_when_invoked": False,
                        "creates_order_when_invoked": False,
                    }
                ),
    ...
    async def _audit_tables(self) -> BnbLiveExecutionBoundaryTableAudit:
        ...
        return BnbLiveExecutionBoundaryTableAudit(
...
def _request_from_generic_action_spec(
    action_spec: GenericActionSpec,
) -> tuple[BnbLiveExecutionBoundaryDryRunRequest, list[str]]:
    blockers: list[str] = []
...
def _authorization_state(authorization: Any | None) -> BnbLiveExecutionBoundaryAuthorizationState:
    if authorization is None:
        return BnbLiveExecutionBoundaryAuthorizationState(
            exists=False,
...
def _final_gate_read_model(
    *,
    ...
    projection_status: BoundaryStatus,
    hard_blockers: list[str],
) -> BnbLiveExecutionBoundaryFinalGateReadModel:
    ...
    return BnbLiveExecutionBoundaryFinalGateReadModel(
        ...
        execution_boundary_status=projection_status,
    )
...
def _owner_execution_trigger_read_model(
    *,
    ...
    return BnbOwnerExecutionTriggerReadModel(
        ...
        endpoint=endpoint,
        reason=(
            f"Owner action creates one scoped ExecutionIntent, submits one real "
            f"{request.carrier_id} entry order for {request.symbol}, and attach one TP plus one "
            "SL after the final hard gate is rechecked."
        ),
        blockers=[],
        creates_execution_intent_when_invoked=True,
        creates_order_when_invoked=True,
        exact_scope=exact_scope,
    )
...
def _gate_fact_state(
    fact_checks: dict[str, dict[str, Any]],
    fact_id: str,
) -> BnbLiveExecutionBoundaryGateFactState:
    fact = fact_checks.get(fact_id)
    if fact is None:
        return BnbLiveExecutionBoundaryGateFactState(
            state="missing",
            status="missing",
            source="not_supplied",
