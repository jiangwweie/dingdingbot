...
def project_account_budget_current(
    ...
    *,
    ...
    exposure_ticket_ids = {str(row.get("owner_ticket_id") or "") for row in exposures if row.get("owner_ticket_id")}
    open_risk = sum((_decimal(row.get("actual_directional_risk")) for row in exposures if str(row.get("exposure_state")) in {"open_protected", "working_entry", "partially_exited", "runner_active"}), _ZERO)
    working_risk = sum((_decimal(row.get("held_risk")) for row in exposures if str(row.get("exposure_state")) == "working_entry"), _ZERO)
    reserved_risk = sum((_decimal(row.get("risk_at_stop")) for row in reservations if str(row.get("status")) in {"active", "consumed"} and str(row.get("ticket_id") or "") not in exposure_ticket_ids), _ZERO)
    unknown_risk = sum((_decimal(row.get("held_risk")) for row in exposures if str(row.get("reconciliation_state")) in {"unknown", "mismatch"}), _ZERO)
    blockers = sorted({str(row["first_blocker"]) for row in exposures if row.get("first_blocker")})
    held = max(open_risk, working_risk) + reserved_risk + unknown_risk
    slots = sum(1 for row in exposures if bool(row.get("position_slot_claimed")))
    pending = sum(1 for row in reservations if str(row.get("status")) in {"active", "consumed"} and str(row.get("ticket_id") or "") not in exposure_ticket_ids)
    ...
    result = AccountBudgetCurrent(
        ...
        reserved_risk=reserved_risk, portfolio_held_risk=held,
        claimed_position_slots=slots + pending, new_entry_allowed=not blockers,
...
def _persist(conn: sa.Connection, result: AccountBudgetCurrent, snapshot: FullAccountRiskSnapshot, policy: AccountRiskPolicy, margin_limit: Decimal, risk_limit: Decimal, now_ms: int, existing: dict[str, object] | None) -> None:
    ...
    values = {
        ...
        **result.model_dump(), "total_wallet_balance": snapshot.total_wallet_balance, "available_balance": snapshot.available_balance,
        "exchange_total_initial_margin": snapshot.exchange_total_initial_margin, "working_entry_risk": _ZERO,
        "unknown_held_risk": _ZERO, "unreflected_pending_margin": _ZERO, "portfolio_margin_used": margin_used,
        ...
        "portfolio_margin_limit": margin_limit, "portfolio_margin_remaining": max(_ZERO, margin_limit - margin_used),
        "pending_ticket_claims": result.claimed_position_slots, "max_concurrent_positions": policy.max_concurrent_positions,
