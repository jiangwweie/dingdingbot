...
class ConfigParser:
    ...
    # ============================================================
    # Convenience Methods (向后兼容 ConfigManager 用法)
    # ============================================================

    def load_core_config_from_yaml(self, config_dir: Path) -> CoreConfig:
        """
        Load core config from YAML file with fallback to defaults.

        Args:
            config_dir: Directory containing core.yaml

        Returns:
            CoreConfig instance (from file or defaults)
        """
        core_path = config_dir / 'core.yaml'

        if not core_path.exists():
            self._logger.warning(f"core.yaml not found at {core_path}, using defaults")
            return self.create_default_core_config()

        try:
            data = self.parse_yaml_file(core_path)
            return self.parse_core_config(data)
        except Exception as e:
            self._logger.error(f"Failed to load core.yaml, using defaults: {e}")
            return self.create_default_core_config()

    def load_user_config_from_yaml(self, config_dir: Path) -> UserConfig:
        """
        Load user config from YAML file with fallback to defaults.

        Args:
            config_dir: Directory containing user.yaml

        Returns:
            UserConfig instance (from file or defaults)
        """
        user_path = config_dir / 'user.yaml'

        if not user_path.exists():
            self._logger.warning(f"user.yaml not found at {user_path}, using defaults")
            return self.create_default_user_config()

        try:
            data = self.parse_yaml_file(user_path)
            return self.parse_user_config(data)
        except Exception as e:
            self._logger.error(f"Failed to load user.yaml, using defaults: {e}")
            return self.create_default_user_config()
