"""LLM context packet builders.

These helpers translate already-trusted BRC runtime facts into the structured
packet consumed by the advisory plane. They do not fetch exchange facts or
    ...
    *,
    ...
    return LlmContextPacket(
        packet_id=str(
            ...
            or f"llm-packet-{uuid.uuid4().hex[:12]}"
        ),
        packet_type=str(packet.get("packet_type") or "llm_advisory_context"),
        produced_at_ms=int(packet.get("produced_at_ms") or now_ms),
        market=_merge_dict(packet.get("market"), market),
        runtime=_merge_dict(packet.get("runtime"), runtime),
        strategies=_merge_dict(packet.get("strategies"), strategies),
        audit=_merge_dict(packet.get("audit"), audit),
        external_facts=_merge_dict(packet.get("external_facts"), external_facts),
        review=_merge_dict(packet.get("review"), review),
        safety={
            ...
            **dict(packet.get("safety") or {}),
        },
        source_refs=_merge_refs(packet.get("source_refs"), source_refs),
    )
...
def build_trade_review_context_packet(
    *,
    ...
    return build_llm_context_packet(
        now_ms=now_ms,
