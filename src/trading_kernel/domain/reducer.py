...
def reduce_event(
    ...
    if isinstance(event, CancelOrderStillOpenConfirmed):
        _require_status(current, AggregateStatus.CANCEL_OUTCOME_UNKNOWN)
        if event.exchange_order_id != current.pending_cancel_exchange_order_id:
            raise InvalidLifecycleTransition("still-open cancel identity mismatch")
        return _transition(
            current,
            event,
            status=AggregateStatus.CANCEL_REJECTED,
