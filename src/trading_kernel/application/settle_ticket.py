...
class ReviseTradeReviewRequest(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
async def revise_trade_review(
    uow: KernelUnitOfWork,
    request: ReviseTradeReviewRequest,
) -> SettleTicketResult:
    aggregate = await uow.aggregates.get(request.ticket_id)
    if aggregate is None:
        raise ValueError("Ticket aggregate does not exist")
    if aggregate.status is not AggregateStatus.TERMINAL:
        raise ValueError("Trade Review revision requires terminal Ticket")
    if aggregate.review_id != request.supersedes_review_id:
        raise ValueError("Trade Review supersedes identity is not current")
    predecessor = await uow.reviews.get(request.supersedes_review_id)
    if predecessor is None or predecessor.ticket_id != request.ticket_id:
        raise ValueError("Superseded Trade Review does not exist for Ticket")
    await uow.reviews.add(
        TradeReviewRecord(
            review_id=request.review_id,
            ticket_id=request.ticket_id,
    ...
    return SettleTicketResult(status=SettleTicketStatus.REVIEW_REVISED)
