...
async def list_profiles():
    ...
    说明：
        这里返回的是旧配置域（config_profiles）的管理视图，不是
        Sim-1/runtime freeze 使用的 runtime profile 列表。
    """
    try:
        from src.infrastructure.config_profile_repository import ConfigProfileRepository
        from src.application.config_profile_service import ConfigProfileService
...
async def create_profile(request: ProfileCreateRequest):
    ...
    Args:
        ...
        switch_immediately: 创建后是否立即在配置域中激活（不影响当前已冻结的 runtime）

    Returns:
        创建的 Profile 信息

    说明：
        这里创建的是旧配置域 profile，不会直接改变当前已经冻结的
        execution runtime 配置真源。
    """
    try:
        from src.infrastructure.config_profile_repository import ConfigProfileRepository
        from src.application.config_profile_service import ConfigProfileService
...
async def switch_profile(name: str, confirm: bool = Query(False, description="必须显式确认切换（仅影响旧配置域，下次启动生效，不热切当前 runtime）")):
    """
    ...
    说明：
        当前运行中的 execution runtime 已由启动期解析出的
        RuntimeConfigProvider / ResolvedRuntimeConfig 冻结。
        因此 profile switch 默认应理解为：
        - 更新旧配置域中的 active profile
        - 对后续启动 / 显式 reload 流程生效
        - 不应被解读为“静默热切当前 execution 主链”
    """
    if not confirm:
        raise HTTPException(
            status_code=409,
