"""Non-executing exchange submit submit preview for runtime local orders.
...
class RuntimeExecutionExchangeSubmitPreviewModel(BaseModel):
    model_config = ConfigDict(extra="forbid")


class RuntimeExecutionExchangeSubmitPreviewStatus(str, Enum):
    BLOCKED = "blocked"
...
class RuntimeExecutionExchangeOrderSubmitRequestPreview(
    RuntimeExecutionExchangeSubmitPreviewModel
):
...
class RuntimeExecutionExchangeSubmitPreview(
    RuntimeExecutionExchangeSubmitPreviewModel
):
    ...
    status: RuntimeExecutionExchangeSubmitPreviewStatus
    symbol: str = Field(min_length=1, max_length=128)
    ...
    @model_validator(mode="after")
    def _validate_preview(self) -> "RuntimeExecutionExchangeSubmitPreview":
        forbidden = {
            "client_order_id",
            "exchange_order_id",
            "exchange_payload",
        ...
        for key in _walk_keys(scanned):
            if key.lower() in forbidden:
                raise ValueError(
                    "exchange submit submit preview contains forbidden "
                    f"execution field: {key}"
                )
        if self.exchange_called or self.exchange_order_submitted:
            raise ValueError("exchange submit submit preview cannot call exchange")
        if self.order_lifecycle_submit_called:
            raise ValueError(
                "exchange submit submit preview cannot call OrderLifecycle.submit_order"
            )
        if self.execution_intent_status_changed:
            raise ValueError("exchange submit submit preview cannot mutate intent status")
        if self.owner_bounded_execution_called:
            raise ValueError("exchange submit submit preview cannot call OwnerBoundedExecution")
        if self.withdrawal_or_transfer_created:
            raise ValueError("exchange submit submit preview cannot create withdrawal/transfer")
        if (
            self.status
            ...
            raise ValueError("ready exchange submit preview cannot have blockers")
        return self


def build_runtime_execution_exchange_submit_preview(
    *,
    ...
    for order in local_orders:
        blockers.extend(
            _validate_local_order_for_submit_preview(
                order=order,
                binding=binding,
    ...
    status = (
        RuntimeExecutionExchangeSubmitPreviewStatus.BLOCKED
        if blockers
        else RuntimeExecutionExchangeSubmitPreviewStatus
        .READY_FOR_EXCHANGE_SUBMIT_ADAPTER_DESIGN
    )
    symbol = (
        entry_order.symbol
    ...
    return RuntimeExecutionExchangeSubmitPreview(
        submit_preview_id=f"runtime-exchange-submit-preview-{binding.authorization_id}",
        ...
        metadata={
            "scope": "runtime_execution_exchange_submit_preview",
...
def _validate_local_order_for_submit_preview(
    *,
    order: Order,
    binding: RuntimeExecutionIntentLocalOrderBinding,
