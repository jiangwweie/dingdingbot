"""Runtime next-attempt release packet.

This packet joins the post-close evidence surfaces:

RuntimeActivePositionResolutionPacket
-> optional next-attempt gate packet
-> release decision for strategy signal observation / shadow planning

It is packet-only. It does not create ExecutionIntents, create orders, call
...
class RuntimeNextAttemptReleasePacket(BaseModel):
    model_config = ConfigDict(extra="forbid")

    packet_id: str = Field(min_length=1, max_length=420)
    ...
    def _status_contract(self) -> "RuntimeNextAttemptReleasePacket":
        if self.status == RuntimeNextAttemptReleaseStatus.BLOCKED and not self.blockers:
            raise ValueError("blocked release packet requires blockers")
        if self.status == RuntimeNextAttemptReleaseStatus.READY_FOR_STRATEGY_SIGNAL:
            if self.blockers:
                raise ValueError("ready release packet cannot have blockers")
            if not self.strategy_signal_observation_allowed:
                raise ValueError("ready release packet must allow strategy observation")
            if not self.shadow_candidate_planning_allowed:
                raise ValueError("ready release packet must allow shadow planning")
        return self


def build_runtime_next_attempt_release_packet(
    *,
    active_position_resolution: RuntimeActivePositionResolutionPacket,
    next_attempt_gate_packet: dict[str, Any] | None,
    now_ms: int,
) -> RuntimeNextAttemptReleasePacket:
    """Build a non-executing release packet from already-generated evidence."""

    resolution = active_position_resolution
    gate = _extract_gate(next_attempt_gate_packet)
    gate_status = _gate_status(next_attempt_gate_packet, gate)
    gate_name = _gate_name(gate)
    warnings = _dedupe([
        *resolution.warnings,
        *_gate_warnings(next_attempt_gate_packet, gate),
    ])
    blockers: list[str] = []
    ...
    if resolution.status == RuntimeActivePositionResolutionStatus.BLOCKED:
        ...
        recommended = resolution.recommended_next_action
        required_steps = list(resolution.required_steps)
    ...
    elif next_attempt_gate_packet is None:
        status = RuntimeNextAttemptReleaseStatus.WAITING_FOR_NEXT_ATTEMPT_GATE
        recommended = "run_non_executing_next_attempt_gate_packet"
        required_steps = ["verify_next_attempt_gate"]
        completed_steps.extend(resolution.completed_steps)
    elif _gate_clear(next_attempt_gate_packet, gate):
        ...
        status = RuntimeNextAttemptReleaseStatus.BLOCKED
        blockers.extend(_gate_blockers(next_attempt_gate_packet, gate))
        if not blockers:
            blockers.append("next_attempt_gate_not_clear")
        recommended = (
            _required_next_step(next_attempt_gate_packet)
    ...
    return RuntimeNextAttemptReleasePacket(
        packet_id=(
...
def _extract_gate(packet: dict[str, Any] | None) -> dict[str, Any]:
    if not isinstance(packet, dict):
        return {}
    gate = packet.get("next_attempt_gate")
    return gate if isinstance(gate, dict) else {}


def _gate_clear(packet: dict[str, Any], gate: dict[str, Any]) -> bool:
    return bool(
        packet.get("status") == "clear_for_next_attempt_preflight"
        or (
...
def _gate_status(packet: dict[str, Any] | None, gate: dict[str, Any]) -> str | None:
    if not isinstance(packet, dict):
        return None
    value = gate.get("status") or packet.get("status")
...
def _gate_warnings(packet: dict[str, Any] | None, gate: dict[str, Any]) -> list[str]:
    if not isinstance(packet, dict):
        return []
    values: list[Any] = []
    values.extend(packet.get("warnings") or [])
    values.extend(gate.get("warnings") or [])
    return [str(value) for value in values if value]


def _gate_blockers(packet: dict[str, Any] | None, gate: dict[str, Any]) -> list[str]:
    if not isinstance(packet, dict):
        return ["next_attempt_gate_packet_missing"]
    blockers: list[str] = []
...
def _required_next_step(packet: dict[str, Any] | None) -> str | None:
    if not isinstance(packet, dict):
        return None
    value = packet.get("required_next_step")
    return str(value) if value else None
