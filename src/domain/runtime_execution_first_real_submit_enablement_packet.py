"""First-real-submit enablement review packet.

The packet is a read-only aggregation for Owner/Codex review. It combines the
...
class RuntimeExecutionFirstRealSubmitEnablementPacketModel(BaseModel):
    model_config = ConfigDict(extra="forbid")


class RuntimeExecutionFirstRealSubmitEnablementPacketStatus(str, Enum):
    BLOCKED = "blocked"
    READY_FOR_OWNER_FINAL_REVIEW = "ready_for_owner_final_review"


class RuntimeExecutionFirstRealSubmitEnablementPacket(
    RuntimeExecutionFirstRealSubmitEnablementPacketModel
):
    packet_id: str = Field(min_length=1, max_length=620)
    ...
    status: RuntimeExecutionFirstRealSubmitEnablementPacketStatus
    ...
    def _validate_packet(
        ...
        _reject_forbidden_execution_fields(
            "first real submit enablement packet",
        ...
        if (
            ...
            raise ValueError("ready first-real-submit packet cannot have blockers")
        if (
            ...
            raise ValueError("first-real-submit packet cannot perform execution")
        return self


def build_runtime_execution_first_real_submit_enablement_packet(
    *,
    submit_rehearsal: RuntimeExecutionSubmitRehearsal,
    first_real_submit_confirmations: FirstRealSubmitConfirmationFacts,
    ...
    status = (
        ...
        else RuntimeExecutionFirstRealSubmitEnablementPacketStatus.BLOCKED
    )
    return RuntimeExecutionFirstRealSubmitEnablementPacket(
        packet_id=(
            "runtime-first-real-submit-enablement-packet-"
        ...
        metadata={
            "scope": "runtime_execution_first_real_submit_enablement_packet",
            "read_only_aggregation": True,
            "first_real_submit_owner_codex_review_packet": True,
