# AGENTS.md - 盯盘狗 🐶 项目开发指南

> **最后更新**: 2026-04-02
> **项目阶段**: v3.0 Phase 5 实盘集成（编码完成，待审查修复）
...
**Codex 侧用法**：
在 Codex 对话中直接输入同样的指令（如 `/pm`、`/architect`、`/backend`、`/kaigong`、`/shougong`），会触发本仓库 `.agents/skills/` 下的等价入口 skills；这些入口以 `.claude/**` 作为单一真源读取规范，保证双端一致。
...
- `README.md` - 用户文档
- `docs/` - 架构与任务文档
- `.claude/skills/` - Claude Code 项目技能定义（完整保留）
- `.agents/skills/` - Codex 项目技能定义（本仓库内）
