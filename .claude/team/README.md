...
└─────────────────────────────────────────────────────────────────┘
        ...
        ▼             ▼             ▼             ▼             ▼             ▼
┌───────────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌──────────────┐ ┌──────────────┐
│ Frontend Dev  │ │ Backend   │ │   QA      │ │  Code     ││  Diagnostic  │ │   DevOps     │
│    (前端)     │ │  (后端)   │ │  Tester   │ │ Reviewer  ││   Analyst    │ │  (运维工程师)│
│               │ │           │ │  (测试)   │ │  (审查)   ││  (诊断分析师)│ │              │
└───────────────┘ └───────────┘ └───────────┘ └───────────┘ └──────────────┘ └──────────────┘
```
...
- **核心原则**: **只改配置，不改代码**
- **触发场景**: 代码部署、配置调整、日志分析、生产故障排查
- **使用方式**: 输入 `/devops` 或分配运维任务
...
| 文件路径 | Frontend | Backend | QA | Coordinator | Reviewer | Diagnostic | DevOps |
|----------|----------|---------|----|-------------|----------|------------|--------|
| `web-front/**` | ✅ 全权 | ❌ 禁止 | ⚠️ 仅测试 | ⚠️ 仅配置 | 🔍 审查 | 🔍 审查 | ❌ 禁止 |
| `src/**` | ❌ 禁止 | ✅ 全权 | ⚠️ 仅测试 | ⚠️ 仅协调 | ✅ 修改测试 | 🔍 审查 | ❌ 禁止 |
| `tests/**` | ⚠️ 协助 | ⚠️ 协助 | ✅ 全权 | ⚠️ 仅协调 | ✅ 修改测试 | ⚠️ 运行验证 | ❌ 禁止 |
| `config/**` | ❌ 禁止 | ✅ 全权 | ❌ 禁止 | ⚠️ 仅协调 | 🔍 审查 | 🔍 审查 | ⚠️ 仅 `-prod` |
| `docker-compose.yml` | ❌ 禁止 | ⚠️ 建议 | ❌ 禁止 | ⚠️ 协调 | 🔍 审查 | ❌ 禁止 | ✅ 全权 |
| `Dockerfile` | ❌ 禁止 | ⚠️ 建议 | ❌ 禁止 | ⚠️ 协调 | 🔍 审查 | ❌ 禁止 | ✅ 全权 |
| `CLAUDE.md` | ❌ 禁止 | ❌ 禁止 | ❌ 禁止 | ✅ 全权 | 🔍 审查 | ❌ 禁止 | ❌ 禁止 |
| `.claude/team/**` | ⚠️ 建议 | ⚠️ 建议 | ⚠️ 建议 | ✅ 全权 | 🔍 审查 | ❌ 禁止 | ⚠️ 建议 |
| `docs/**` | ⚠️ 建议 | ⚠️ 建议 | ⚠️ 建议 | ✅ 全权 | ⚠️ 建议 | ✅ 全权 | ✅ 全权 |
| `data/*.db` | ❌ 禁止 | ❌ 禁止 | ❌ 禁止 | ❌ 禁止 | ❌ 禁止 | ⚠️ 只读查询 | ✅ 全权 |
| `logs/**` | ❌ 禁止 | ⚠️ 查看 | ❌ 禁止 | ⚠️ 查看 | 🔍 审查 | 🔍 审查 | ✅ 全权 |
...
#### DevOps 边界
```
✅ 可修改：docker-compose.yml, Dockerfile, config/*-prod.yaml, logs/**, data/**
🔍 查看：src/**, web-front/** (仅排查问题，不修改)
❌ 禁止：src/** (业务代码), web-front/** (前端代码), tests/** (测试代码)
```

**DevOps 核心原则**: **只改配置，不改代码**
