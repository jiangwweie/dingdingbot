...
| ⚠️ **受限权限** | 需特定条件或 Coordinator 授权 | 修改生产配置、部署 |
| ❌ **禁止** | 无权调用 | 越权修改其他模块 |
...
### 1. Team Coordinator (`/coordinator`)
...
- 修改 .claude/team/ 配置（需 Coordinator 协调）

# ⚠️ 需要协调的操作
- 修改 API 接口 Schema → 通知 Coordinator 分配给 frontend-dev 对接
- 修改数据库 schema → 通知 Coordinator 记录到 findings.md
```
...
# ⚠️ 需要协调的操作
- 需要 API 字段变更 → 通知 Coordinator 分配给 backend-dev
...
// 3. 实现组件后调用简化
/simplify web-front/src/components/PreviewButton.tsx
...
# ⚠️ 需要协调的操作
- 测试发现 Bug → 通知 Coordinator 分配修复任务
- 需要修改测试断言策略 → 通知 Coordinator 确认
```
...
mcp__filesystem__write_file:
  path: web-front/src/components/...  # 后端角色禁止
```
