#!/usr/bin/env python3
"""Build a read-only strategy signal input packet for one runtime."""
...
async def _build_packet(args: argparse.Namespace) -> dict[str, Any]:
    ...
    signal_input = _build_signal_input(
        runtime=runtime,
        one_hour=one_hour,
        four_hour=four_hour,
    ...
    return {
        "scope": "runtime_strategy_signal_input_packet",
        ...
        "operator_command_plan": {
            "scope": "runtime_strategy_signal_input_operator_command_plan",
            "next_step": (
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a read-only strategy signal input packet for one runtime.",
    )
