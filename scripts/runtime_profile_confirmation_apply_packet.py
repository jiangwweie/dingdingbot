#!/usr/bin/env python3
"""Build an auditable apply packet for a runtime profile confirmation.

RTF-037 consumes an RTF-036 decision packet and prepares the exact official API
...
BLOCKED_STATUS = "blocked_runtime_profile_confirmation_apply_packet"
...
def _proposal(decision_packet: dict[str, Any]) -> dict[str, Any]:
    template = decision_packet.get("promotion_confirmation_request_template") or {}
...
def _expected_owner_confirmation(decision_packet: dict[str, Any]) -> str:
    proposal = _proposal(decision_packet)
    strategy_family_id = str(decision_packet.get("strategy_family_id") or "")
    strategy_family_version_id = str(
        decision_packet.get("strategy_family_version_id") or ""
    )
    symbol = str(decision_packet.get("symbol") or "")
    side = str(decision_packet.get("side") or "").lower()
...
def _safety_invariants() -> dict[str, bool]:
    return {
        "decision_packet_replay_only": True,
...
def _blocked(
    *,
    ...
    expected = _expected_owner_confirmation(decision_packet)
    return {
        "scope": "runtime_profile_confirmation_apply_packet",
        "status": BLOCKED_STATUS,
        "source_status": decision_packet.get("status"),
        "strategy_family_id": decision_packet.get("strategy_family_id"),
        "strategy_family_version_id": decision_packet.get("strategy_family_version_id"),
        "symbol": decision_packet.get("symbol"),
        "side": decision_packet.get("side"),
        "owner_confirmation": {
...
def build_packet(
    *,
    ...
    blockers: list[str] = []
    warnings = [
        *list(decision_packet.get("warnings") or []),
        "apply_packet_does_not_call_api",
        "apply_packet_does_not_create_runtime",
        "apply_packet_does_not_place_order",
    ]
    if decision_packet.get("status") != "ready_for_owner_codex_runtime_profile_confirmation":
        blockers.append("runtime_profile_decision_packet_not_ready")
    if not decision_packet.get("promotion_confirmation_request_template"):
        blockers.append("promotion_confirmation_request_template_missing")
    if not decision_packet.get("runtime_draft_request_template"):
        blockers.append("runtime_draft_request_template_missing")
    gate = decision_packet.get("promotion_gate_preview") or {}
    if gate.get("status") != "ready_for_controlled_runtime_execution_design":
        blockers.append("promotion_gate_preview_not_ready")
    source_safety = decision_packet.get("safety_invariants") or {}
    if source_safety.get("runtime_created") or source_safety.get("exchange_write_called"):
        blockers.append("source_decision_packet_contains_mutation")
    if blockers:
        return _blocked(
    ...
    if not provided or not matches:
        ...
        return {
            ...
            "source_status": decision_packet.get("status"),
            "strategy_family_id": decision_packet.get("strategy_family_id"),
            "strategy_family_version_id": decision_packet.get(
    ...
    confirmation_request = dict(
        decision_packet["promotion_confirmation_request_template"]
    )
    ...
    confirmation_request["metadata"] = {
        **dict(confirmation_request.get("metadata") or {}),
        ...
        "rtf037_apply_packet_ready": True,
    }
    confirmation_id = str(confirmation_request["confirmation_id"])
    runtime_draft_body = {
        **dict(
            (decision_packet.get("runtime_draft_request_template") or {}).get("body")
            or {}
        ),
    ...
    runtime_draft_body["metadata"] = {
        **dict(runtime_draft_body.get("metadata") or {}),
        "source": "rtf037_runtime_profile_confirmation_apply_packet",
    ...
    return {
        ...
        "symbol": decision_packet.get("symbol"),
        "side": decision_packet.get("side"),
        "runtime_instance_id": decision_packet.get("runtime_instance_id"),
        "trial_binding_id": trial_binding_id,
        "owner_confirmation": {
            "required_value": expected,
            "provided": True,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build RTF-037 runtime profile confirmation apply packet.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    packet = build_packet(
        decision_packet=_load_json(args.decision_json),
