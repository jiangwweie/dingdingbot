#!/usr/bin/env python3
"""Build a live-attempt readiness artifact from active runtime observation.

RTF-094 is a summarizer over an already produced
``runtime_active_observation_monitor`` artifact.  It does not call the API,
...
into a compact operator artifact for the next tiny-live attempt decision.
"""
...
def _runtime_summaries(active_monitor_artifact: dict[str, Any]) -> list[dict[str, Any]]:
    summaries = active_monitor_artifact.get("runtime_summaries")
...
def _forbidden_effects_from_monitor(artifact: dict[str, Any]) -> dict[str, bool]:
...
def _ready_effects_from_monitor(artifact: dict[str, Any]) -> dict[str, bool]:
    safety = artifact.get("safety_invariants")
    if not isinstance(safety, dict):
        safety = {}
...
def build_readiness_artifact(
    *,
    ...
    rows = [_runtime_row(summary) for summary in _runtime_summaries(active_monitor_artifact)]
    forbidden_effects = _forbidden_effects_from_monitor(active_monitor_artifact)
    ready_effects = _ready_effects_from_monitor(active_monitor_artifact)
    counts = _counts(rows)
    ...
    warnings = list(active_monitor_artifact.get("warnings") or [])

    return {
        "scope": "runtime_live_attempt_readiness_artifact",
        "status": status,
        "source_monitor_scope": active_monitor_artifact.get("scope"),
        "source_monitor_status": active_monitor_artifact.get("status"),
        "active_runtime_count": int(active_monitor_artifact.get("active_runtime_count") or 0),
        "monitored_runtime_count": int(
            active_monitor_artifact.get("monitored_runtime_count") or len(rows)
        ),
        "selected_runtime_instance_ids": list(
            active_monitor_artifact.get("selected_runtime_instance_ids") or []
        ),
        ...
        "safety_invariants": {
            "uses_official_trading_console_api": _bool(
                (active_monitor_artifact.get("safety_invariants") or {}).get(
            ...
            "no_prepare_records_created_in_this_artifact": not any(
        ...
        "live_attempt_readiness_plan": _operator_plan(status),
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a live attempt readiness artifact from active monitor JSON.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    artifact = build_readiness_artifact(
        active_monitor_artifact=_read_json(args.active_monitor_json),
