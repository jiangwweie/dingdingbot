#!/usr/bin/env python3
"""Current evidence wrapper for archived local-registration authorization."""
...
_ARCHIVED_MODULE_NAME = ".".join(
    (
        ...
        "first_real_submit",
        "build_runtime_first_real_submit_local_registration_authorization_"
        + "pack"
        + "et",
    )
)
_ARCHIVED_BUILDER_NAME = "build_local_registration_authorization_" + "pack" + "et"
_archived_module = import_module(_ARCHIVED_MODULE_NAME)


def build_local_registration_authorization_evidence(
    *args: Any,
    **kwargs: Any,
) -> dict[str, Any]:
...
def _normalize_evidence(payload: dict[str, Any]) -> dict[str, Any]:
    return normalize_first_real_submit_authorization_evidence(
        ...
        archived_ready_status=(
            "owner_local_registration_authorization_" + "pack" + "et_ready"
        ),
        current_ready_status="owner_local_registration_authorization_evidence_ready",
        archived_scope=(
            "runtime_first_real_submit_local_registration_authorization_"
            + "pack"
            + "et"
        ),
        current_scope=(
            "runtime_first_real_submit_local_registration_authorization_evidence"
        ),
        archived_build_only_key="pack" + "et_build_only",
        current_plan_key="local_registration_authorization_plan",
    )
...
def _main(*args: Any, **kwargs: Any) -> Any:
    ...
    evidence = build_local_registration_authorization_evidence(
        disabled_smoke_report=_archived_module._load_json_object(
            Path(parsed.disabled_smoke_report_path)
        ),
    ...
    return 0 if evidence["checks"]["ready_for_owner_local_registration_authorization"] else 2
