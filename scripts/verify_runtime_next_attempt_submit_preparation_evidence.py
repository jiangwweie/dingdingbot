#!/usr/bin/env python3
"""Verify next-attempt strategy planning -> submit-preparation evidence locally.
...
async def _ready_submit_preparation(
    *,
    planning_artifact: RuntimeNextAttemptStrategyPlanningArtifact,
) -> dict[str, Any]:
    readiness_service = RuntimeExecutableSubmitReadinessService()
    handoff_service = RuntimeOfficialSubmitHandoffService()
    evidence = _evidence(
        runtime_instance_id=planning_artifact.runtime_instance_id,
        source_authorization_id=planning_artifact.source_authorization_id,
    )
    readiness = await readiness_service.preview_from_strategy_planning_artifact(
        strategy_planning_artifact=planning_artifact,
        evidence=evidence,
        additional_warnings=["rtf050_local_submit_preparation_evidence"],
    )
    fresh_submit_authorization_id = (
        f"fresh-submit-auth-rtf050-{planning_artifact.runtime_instance_id}"
    )
    missing_auth_handoff = await handoff_service.preview_from_readiness_artifact(
        readiness_artifact=readiness,
        fresh_submit_authorization_id=None,
        mode=RuntimeOfficialSubmitHandoffMode.DISABLED_SMOKE,
    )
    disabled_handoff = await handoff_service.preview_from_readiness_artifact(
        readiness_artifact=readiness,
        fresh_submit_authorization_id=fresh_submit_authorization_id,
        mode=RuntimeOfficialSubmitHandoffMode.DISABLED_SMOKE,
    )
    real_mode_handoff = await handoff_service.preview_from_readiness_artifact(
        readiness_artifact=readiness,
        fresh_submit_authorization_id=fresh_submit_authorization_id,
    ...
    checks = {
        "planning_ready_for_final_gate_preflight": (
            planning_artifact.status == READY_FOR_FINAL_GATE_PREFLIGHT
        ),
    ...
    return {
        ...
        "strategy_planning_status": planning_artifact.status.value,
        ...
        "strategy_planning_artifact": _json_value(planning_artifact),
        "readiness_artifact": _json_value(readiness),
        "disabled_handoff_artifact": _json_value(disabled_handoff),
        "real_mode_handoff_artifact": _json_value(real_mode_handoff),
    }
...
def _non_ready_submit_preparation(
    *,
    ...
    checks = {
        ...
        "no_execution_side_effects": (
            planning_artifact.execution_intent_created is False
            and planning_artifact.order_created is False
            and planning_artifact.order_lifecycle_called is False
            and planning_artifact.exchange_called is False
            and planning_artifact.exchange_order_submitted is False
            and planning_artifact.withdrawal_or_transfer_created is False
        ),
    }
    return {
        ...
        "blockers": list(planning_artifact.blockers),
        "warnings": list(planning_artifact.warnings),
        "checks": checks,
        "strategy_planning_artifact": _json_value(planning_artifact),
    }


async def build_next_attempt_submit_preparation_evidence_report() -> dict[str, Any]:
    planning_report = await build_next_attempt_gate_strategy_planning_report()
    scenarios: list[dict[str, Any]] = []

    for scenario in planning_report["scenarios"]:
        planning_artifact = RuntimeNextAttemptStrategyPlanningArtifact.model_validate(
            scenario["strategy_planning_artifact"],
        )
        if planning_artifact.status == READY_FOR_FINAL_GATE_PREFLIGHT:
            scenarios.append(
    ...
    return {
        "scope": "rtf050_next_attempt_submit_preparation_evidence",
        "status": (
            "rtf050_next_attempt_submit_preparation_evidence_passed"
            if passed
            else "rtf050_next_attempt_submit_preparation_evidence_failed"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Verify next-attempt strategy planning to submit-preparation evidence "
            "locally."
        ),
    )
    parser.add_argument("--output-json")
    args = parser.parse_args()
    report = asyncio.run(build_next_attempt_submit_preparation_evidence_report())
