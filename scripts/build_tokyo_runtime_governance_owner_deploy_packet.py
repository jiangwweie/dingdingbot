#!/usr/bin/env python3
"""Build the Owner decision packet for Tokyo runtime-governance deployment.

This script aggregates existing non-mutating evidence into a concise deploy
authorization packet. It does not deploy, run migrations, restart services,
...
class OwnerDeployPacketError(RuntimeError):
    """Raised when the owner deploy packet cannot be built."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        _print_human(packet)
    return 0 if packet["checks"]["ready_for_owner_deploy_decision"] else 2
...
async def _build_owner_deploy_packet_from_args(
    ...
    return build_owner_deploy_packet(
...
def build_owner_deploy_packet(
    *,
    ...
    """Aggregate existing readiness reports into one Owner-facing packet."""
    ...
    packet = {
        ...
        "scope": "tokyo_runtime_governance_owner_deploy_decision_packet",
...
def _repo_root() -> Path:
    ...
    if completed.returncode != 0 or not completed.stdout.strip():
        raise OwnerDeployPacketError("not inside a git repository")
    return Path(completed.stdout.strip())


def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a non-mutating Owner deployment decision packet."
    )
    ...
    parser.add_argument(
        "--skip-remote-probe",
        action="store_true",
        help="Skip the Tokyo read-only probe. The packet will not be deploy-ready.",
    )
    parser.add_argument(
        "--skip-pre-live-packet",
        action="store_true",
        help="Skip the pre-live submit packet. The packet will not be deploy-ready.",
    )
...
def _print_human(packet: dict[str, Any]) -> None:
    ...
    print(f"candidate_head={packet['candidate']['head']}")
    print(f"deploy_commands_planned={packet['deploy_plan_summary']['commands_planned']}")
    print(f"deploy_confirmation_phrase={packet['owner_gate']['deploy_confirmation_phrase']}")
    if checks["blockers"]:
...
if __name__ == "__main__":
    ...
    except OwnerDeployPacketError as exc:
        print(f"owner_deploy_packet_error={exc}", file=sys.stderr)
        raise SystemExit(2)
