#!/usr/bin/env python3
"""Build the Owner confirmation record for git-based Tokyo deployment."""
...
class GitOwnerDeployArtifactError(RuntimeError):
    """Raised when the git owner deploy evidence cannot be built."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 0 if artifact["checks"]["ready_for_owner_git_deploy_decision"] else 2
...
async def _build_owner_deploy_artifact_from_args(
    ...
    return build_git_owner_deploy_artifact(
...
def build_git_owner_deploy_artifact(
    *,
    ...
    if pre_live_evidence_skipped:
        warnings.append("pre_live_evidence_skipped_for_deploy_only")
    ...
    return {
        ...
        "scope": "tokyo_runtime_governance_git_owner_deploy_confirmation_record",
...
def _forbidden_effects(
    *,
    ...
    sources = {
        ...
        "pre_live": (pre_live_evidence or {}).get("safety_invariants", {}),
    }
    allowed_true = {"planning_run_only", "artifact_build_only"}
    forbidden: list[str] = []
...
def _repo_root() -> Path:
    ...
    if completed.returncode != 0 or not completed.stdout.strip():
        raise GitOwnerDeployArtifactError("not inside a git repository")
...
def _git(repo_root: Path, *args: str) -> str:
    ...
    if completed.returncode != 0:
        raise GitOwnerDeployArtifactError(f"git {' '.join(args)} failed")
    return completed.stdout.strip()


def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a non-mutating git Owner deployment confirmation record."
    )
...
if __name__ == "__main__":
    ...
    except GitOwnerDeployArtifactError as exc:
        print(f"git_owner_deploy_artifact_error={exc}", file=sys.stderr)
        raise SystemExit(2)
