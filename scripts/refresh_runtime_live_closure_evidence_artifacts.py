...
from scripts import runtime_live_closure_evidence_artifact as artifact_builder  # noqa: E402
    ...
    *,
    ...
    for path in discovered:
        try:
            ...
            continue
        non_live_reason = _non_live_source_reason(path=path, artifact=artifact)
    ...
    verification_artifact = verifier.build_live_closure_evidence_verification(
        evidence_artifact,
        generated_at_ms=generated_at_ms,
    )
    ...
    _write_json(output_json, evidence_artifact)
    _write_json(verification_output_json, verification_artifact)
...
def _refresh_report(
    *,
    ...
    verification_status = str(verification_artifact.get("status") or "")
    if read_errors and strict_read_errors:
        ...
        owner_state = "需要介入"
    completion = verification_artifact.get("completion")
    ...
    return {
        ...
        "source_counts": {
            ...
            "included_json": len(included_artifacts),
            "skipped_json": len(skipped_files),
            "read_errors": len(read_errors),
        },
        "evidence": {
            "present_evidence_keys": list(evidence_artifact.get("present_evidence_keys") or []),
            "missing_evidence_keys": list(evidence_artifact.get("missing_evidence_keys") or []),
            "reject_reasons": list(evidence_artifact.get("reject_reasons") or []),
        },
        "verification": {
            "status": verification_status,
            "owner_state": verification_artifact.get("owner_state"),
            "first_incomplete_stage": verification_artifact.get("first_incomplete_stage"),
            "completed_stage_count": verification_artifact.get("completed_stage_count"),
            "stage_count": verification_artifact.get("stage_count"),
            "reject_reasons": list(verification_artifact.get("reject_reasons") or []),
...
def _non_live_source_reason(*, path: Path, artifact: dict[str, Any]) -> str | None:
    text = f"{path.name} {_status_text(artifact)}".lower()
...
def _status_text(artifact: dict[str, Any]) -> str:
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Refresh first bounded live-order closure evidence artifacts."
    )
