#!/usr/bin/env python3
"""
验证正式 Optuna 任务路径下 fixed_params 是否生效

目标：
1. 通过 OptimizationRequest.fixed_params 传入固定参数
2. 启动正式 Optuna 任务（start_optimization）
3. 验证 fixed_params 是否真正进入 objective function
"""
import asyncio
import sys
...
async def verify_fixed_params():
    """验证正式 Optuna 任务路径下 fixed_params 是否生效"""

    print("=" * 80)
    print("验证正式 Optuna 任务路径下 fixed_params 是否生效")
    print("=" * 80)
    ...
    try:
        ...
        end_ts = int(datetime(2024, 3, 31).timestamp() * 1000)  # 只跑 3 个月，加速验证

        # 优化请求（通过 fixed_params 传入固定参数）
        # 注意：limit=1000 是 OptimizationRequest 的约束上限
        # 但 v3_pms 模式下 MTF 数据加载使用 max(request.limit, 1000)
        # 所以这里设置 limit=1000，实际 MTF 数据会加载 1000 条
        # 如果需要更多 MTF 数据，需要在 BacktestRequest 中设置更大的 limit
        request = OptimizationRequest(
            ...
            fixed_params=fixed_params,  # ⭐ 关键：通过 OptimizationRequest 传入
        )

        print("\n【验证配置】")
        print(f"  Symbol: {request.symbol}")
        print(f"  Timeframe: {request.timeframe}")
        print(f"  时间范围: 2024-01-01 ~ 2024-03-31 (3个月)")
        ...
        print(f"  tp_slippage_rate: {fixed_params['tp_slippage_rate']}")
        print()

        # 启动优化任务
        print("启动正式 Optuna 任务...")
        ...
        for i, result in enumerate(results, 1):
            ...
            if result.total_trades == 0:
                print("  ⚠️  警告：0 trades，可能 fixed_params 未生效")
                success = False

        # 最终判断
        print("\n" + "=" * 80)
        if success and any(r.total_trades > 0 for r in results):
            print("✅ 验证通过：fixed_params 已正确传入正式 Optuna 任务")
            print("   - 所有试验都有交易")
            print("   - fixed_params 已生效")
            return True
        else:
            print("❌ 验证失败：fixed_params 可能未正确传入")
            return False
...
if __name__ == "__main__":
    try:
        success = asyncio.run(verify_fixed_params())
