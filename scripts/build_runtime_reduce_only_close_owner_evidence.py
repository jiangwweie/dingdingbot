#!/usr/bin/env python3
"""Build a non-executing standing recovery projection for reduce-only close review."""
...
def _reduce_only_close_owner_safety_invariants(
    *, exchange_read_only: bool
) -> dict[str, bool]:
    return {
        "reduce_only_close_owner_projection_only": True,
...
async def _build_evidence(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "scope": "runtime_reduce_only_close_standing_recovery_evidence",
            "status": evidence.status.value,
            "evidence": _json_value(evidence),
            "source_exit_plan": _json_value(exit_plan),
            "safety_invariants": _reduce_only_close_owner_safety_invariants(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a non-executing standing recovery projection for runtime reduce-only close.",
    )
    ...
    parser.add_argument(
        "--skip-exchange",
        action="store_true",
        help="Use only local PG facts; evidence may block on missing exchange facts.",
    )
    parser.add_argument(
        "--skip-reconciliation",
        action="store_true",
    ...
    payload = asyncio.run(_build_evidence(args))
