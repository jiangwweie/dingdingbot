#!/usr/bin/env python3
"""Verify the non-executing next-attempt gate for a strategy runtime."""
...
async def _build_packet(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        "scope": "runtime_next_attempt_gate_packet",
        "status": status,
        ...
        "operator_command_plan": {
            "scope": "runtime_next_attempt_gate_operator_command_plan",
            "not_executed": True,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the non-executing next-attempt gate for a strategy runtime.",
    )
