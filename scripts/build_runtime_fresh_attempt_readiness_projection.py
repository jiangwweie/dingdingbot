#!/usr/bin/env python3
"""Build a fresh-attempt readiness projection from existing runtime reports.

The projection is an operator-facing guardrail over already-produced evidence:

operator live-fact evidence
...
job is to prevent an old authorization or rehearsal evidence from being mistaken
...
def _fresh_signal_summary(payload: dict[str, Any] | None) -> dict[str, Any]:
    if not payload:
        return {
            "status": "missing",
    ...
    observation = payload.get("observation_prepare_flow")
    if not isinstance(observation, dict):
        observation = {}
    return {
        "status": _status(payload),
        "signal_input_json": payload.get("signal_input_json")
        or observation.get("signal_input_json"),
        "prepared_authorization_id": payload.get("prepared_authorization_id"),
...
def _candidate_summary(
    *,
    ...
    projection_plan = (readiness_projection or {}).get("next_attempt_strategy_plan_flow")
    if not isinstance(projection_plan, dict):
        projection_plan = {}
    return {
        "signal_evaluation_id": (
            projection_plan.get("signal_evaluation_id")
            or (final_gate_preflight or {}).get("signal_evaluation_id")
        ),
        "order_candidate_id": (
            projection_plan.get("order_candidate_id")
            or (final_gate_preflight or {}).get("order_candidate_id")
        ),
        "fresh_signal_loop_status": _status(fresh_signal_loop),
        "readiness_projection_status": _status(readiness_projection),
...
def _chain_coverage(
    *,
    operator_live_fact_evidence: dict[str, Any],
    fresh_signal_loop: dict[str, Any] | None,
    readiness_projection: dict[str, Any] | None,
    readiness_evidence: dict[str, Any] | None,
    fresh_authorization_binding: dict[str, Any] | None,
    ...
    return {
        "live_fact_gate": {
            "status": "present",
            "source_status": _status(operator_live_fact_evidence),
            "ready": _status(operator_live_fact_evidence) == READY_LIVE_FACT_GATE,
            "runtime_instance_id": operator_live_fact_evidence.get(
                "runtime_instance_id"
            ),
        },
        "fresh_strategy_signal": {
            "status": "present" if fresh_signal_loop else "missing",
            "source_status": fresh_signal["status"],
...
def _status_from_chain(
    *,
    ...
    if _status(operator_live_fact_evidence) != READY_LIVE_FACT_GATE:
    ...
    projection_status = _status(readiness_projection)
    if not readiness_projection and not readiness_evidence:
        return "ready_for_readiness_evidence"
    if projection_status == READY_FOR_READINESS_EVIDENCE:
        return "ready_for_readiness_evidence"
    if projection_status == READY_FOR_FRESH_SUBMIT_AUTHORIZATION:
        return "waiting_for_fresh_authorization"
    if projection_status == READY_FOR_OFFICIAL_SUBMIT_CALL:
        return "ready_for_action_time_gate"
    if projection_status == "blocked":
        return "blocked_readiness_projection"
...
def _readiness_next_step(status: str) -> str:
...
def build_fresh_attempt_readiness_projection(
    *,
    operator_live_fact_evidence: dict[str, Any],
    ...
    sources = {
        "operator_live_fact_evidence": operator_live_fact_evidence,
        "fresh_signal_loop": fresh_signal_loop or {},
        "readiness_projection": readiness_projection or {},
    ...
    blockers = _list_values(
        operator_live_fact_evidence.get("blockers"),
        (fresh_signal_loop or {}).get("blockers"),
        (readiness_projection or {}).get("blockers"),
    ...
    return {
        ...
        "status": status,
        "runtime_instance_id": operator_live_fact_evidence.get("runtime_instance_id"),
        ...
        "warnings": sorted(
            set(
                _list_values(
                    operator_live_fact_evidence.get("warnings"),
                    (fresh_signal_loop or {}).get("warnings"),
                    (readiness_projection or {}).get("warnings"),
        ...
        "readiness_plan": {
            ...
            "next_step": _readiness_next_step(status),
        ...
        "safety_invariants": {
            "fresh_attempt_readiness_projection_only": True,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a P0-B fresh-attempt readiness artifact from reports.",
    )
    parser.add_argument("--operator-live-fact-evidence-json")
    parser.add_argument("--fresh-signal-loop-json")
    parser.add_argument("--readiness-projection-json")
    ...
    if not args.operator_live_fact_evidence_json:
        parser.error("--operator-live-fact-evidence-json is required")
    return args
...
def main(argv: list[str] | None = None) -> int:
    ...
    projection = build_fresh_attempt_readiness_projection(
        operator_live_fact_evidence=_read_json(args.operator_live_fact_evidence_json) or {},
        fresh_signal_loop=_read_json(args.fresh_signal_loop_json),
        readiness_projection=_read_json(args.readiness_projection_json),
        ...
        artifact_paths={
            key: value
            for key, value in {
                "operator_live_fact_evidence": args.operator_live_fact_evidence_json,
                "fresh_signal_loop": args.fresh_signal_loop_json,
                "readiness_handoff_evidence": args.readiness_projection_json,
