#!/usr/bin/env python3
"""Build a read-only projection that resolves closed-trade review order IDs."""
...
def _closed_trade_review_facts_safety_invariants() -> dict[str, bool]:
    return {
        "closed_trade_review_facts_projection_only": True,
        "pg_read_only": True,
...
async def _build_artifact(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "scope": "runtime_closed_trade_review_facts_artifact",
            "status": artifact.status.value,
            "artifact": _json_value(artifact),
            "safety_invariants": _closed_trade_review_facts_safety_invariants(),
        }
    finally:
