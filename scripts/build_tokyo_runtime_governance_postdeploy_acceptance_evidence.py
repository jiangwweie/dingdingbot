#!/usr/bin/env python3
"""Build a read-only post-deploy acceptance evidence for Tokyo runtime governance.

This script is intended to run after an Owner-authorized deployment. It
aggregates the post-deploy verifier and the pre-live runtime submit evidence to
...
class PostDeployAcceptanceEvidenceError(RuntimeError):
    """Raised when the post-deploy acceptance evidence cannot be built."""


def main(argv: list[str] | None = None) -> int:
    args = _parse_args(argv)
    artifact = asyncio.run(_build_postdeploy_acceptance_evidence_from_args(args))
    if args.json:
        print(json.dumps(artifact, indent=2, sort_keys=True))
    else:
        _print_human(artifact)
    return 0 if artifact["checks"]["postdeploy_acceptance_ready"] else 2


async def _build_postdeploy_acceptance_evidence_from_args(
    ...
    if args.pre_live_evidence_path:
        pre_live_evidence = _load_json_object(Path(args.pre_live_evidence_path))
    elif not args.skip_pre_live_evidence:
    ...
    return build_postdeploy_acceptance_evidence(
...
def build_postdeploy_acceptance_evidence(
    *,
    postdeploy_report: dict[str, Any],
    pre_live_evidence: dict[str, Any] | None,
    expected_current_head: str,
) -> dict[str, Any]:
    """Aggregate post-deploy facts into one acceptance evidence."""
    ...
    pre_live_checks = (
        pre_live_evidence.get("checks")
        if isinstance(pre_live_evidence, dict)
        and isinstance(pre_live_evidence.get("checks"), dict)
        else {}
    )
    ...
    if forbidden_effects:
        blockers.append("postdeploy_evidence_contains_forbidden_side_effect_flags")
    ...
    evidence = {
        ...
        "scope": "tokyo_runtime_governance_postdeploy_acceptance_evidence",
...
def _forbidden_effects(
    *,
    ...
    allowed_true = {"artifact_build_only"}
    forbidden: list[str] = []
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a read-only Tokyo post-deploy acceptance evidence."
    )
    ...
    parser.add_argument(
        ...
        dest="pre_live_evidence_path",
        help=(
            "Use an existing pre-live submit evidence instead of building one "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Skip the pre-live submit evidence. The evidence will not be acceptance-ready.",
    )
...
def _load_json_object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise PostDeployAcceptanceEvidenceError(f"{path} did not contain a JSON object")
    return payload
...
def _print_human(artifact: dict[str, Any]) -> None:
    ...
    print(f"expected_current_head={artifact['expected_current_head']}")
    print(f"current_head={artifact['postdeploy_summary']['current_head']}")
    print(
...
if __name__ == "__main__":
    ...
    except PostDeployAcceptanceEvidenceError as exc:
        print(f"postdeploy_acceptance_evidence_error={exc}", file=sys.stderr)
        raise SystemExit(2)
