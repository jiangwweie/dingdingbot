#!/usr/bin/env python3
"""Bridge fresh-signal readiness into executable readiness preview.

RTF-057 starts from an RTF-056 fresh-signal prepare loop packet:
...
def _output_paths(args: argparse.Namespace) -> dict[str, Path]:
    ...
    return {
        ...
        / f"{flow_id}-post-submit-finalize-packet.json",
        ...
        "cycle_adapter_packet": output_dir / f"{flow_id}-cycle-adapter-packet.json",
        "readiness_handoff_bridge": output_dir
        / f"{flow_id}-readiness-handoff-bridge.json",
    }


def _fresh_loop_status(packet: dict[str, Any]) -> str:
    return str(packet.get("status") or "")


def _post_submit_packet(fresh_loop: dict[str, Any]) -> dict[str, Any]:
    direct = fresh_loop.get("post_submit_finalize_packet")
    if isinstance(direct, dict):
        return direct
    flow = fresh_loop.get("post_submit_finalize_flow")
    if not isinstance(flow, dict):
        return {}
    nested = flow.get("post_submit_finalize_packet")
...
def _signal_input_from_loop(
    ...
    *,
    ...
    signal_packet = payload.get("signal_packet") if isinstance(payload, dict) else None
    signal_input = (
        signal_packet.get("signal_input") if isinstance(signal_packet, dict) else None
    )
...
def _cycle_packet(
    *,
    ...
    status = str(planning_packet.get("status") or "")
    ...
    return {
        "scope": "runtime_fresh_signal_readiness_bridge_cycle_adapter",
...
def _build_packet(
    ...
    *,
    ...
    if fresh_status not in {READY_FOR_PREPARE, READY_FOR_FINAL_GATE_PREFLIGHT}:
        blockers = list(fresh_loop.get("blockers") or [])
        if not blockers:
            blockers.append("fresh_signal_loop_not_ready_for_readiness_bridge")
        return _base_packet(
            args=args,
            status="blocked",
    ...
    post_submit = _post_submit_packet(fresh_loop)
    if not post_submit:
        return _base_packet(
            args=args,
            status="blocked",
            blocked_stage="post_submit_finalize_packet",
            fresh_loop=fresh_loop,
            blockers=["post_submit_finalize_packet_missing_from_fresh_signal_loop"],
            warnings=list(fresh_loop.get("warnings") or []),
            paths=paths,
            next_step="rerun_fresh_signal_prepare_loop_with_post_submit_packet",
        )
    ...
    handoff_builder = handoff_builder or handoff_bridge._build_packet
    ...
    cycle_packet = _cycle_packet(
        args=args,
        fresh_loop=fresh_loop,
        planning_packet=planning_packet,
    )
    _write_json(paths["cycle_adapter_packet"], cycle_packet)

    handoff_packet = handoff_builder(
        _handoff_args(args, cycle_packet_json=paths["cycle_adapter_packet"])
    )
    _write_json(paths["readiness_handoff_bridge"], handoff_packet)
    status = str(handoff_packet.get("status") or "blocked")
    return {
        "scope": "runtime_fresh_signal_readiness_bridge",
        "status": status,
        ...
        "fresh_signal_prepare_loop": fresh_loop,
        "next_attempt_strategy_plan_flow": planning_packet,
        "cycle_adapter_packet": cycle_packet,
        "readiness_handoff_bridge": handoff_packet,
        ...
        "operator_command_plan": {
            "next_step": (
                ...
                else "call_official_submit_endpoint_after_action_time_confirmation"
                if status == READY_FOR_OFFICIAL_SUBMIT_CALL
                else "resolve_readiness_bridge_blocker"
            ),
            "creates_executable_execution_intent": False,
            "places_order": False,
            "calls_order_lifecycle": False,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Bridge a fresh-signal prepare loop packet into executable "
    ...
    parser.add_argument(
        "--output-dir",
        default="output/runtime-fresh-signal-readiness-bridge",
    )
