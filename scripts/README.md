...
- `execute_tokyo_runtime_governance_deploy.py`: apply-gated deployment executor
  for the plan generated by `plan_tokyo_runtime_governance_deploy.py`. Its
  default behavior is dry-run only and executes no commands. Remote mutation
  requires `--apply` and the exact printed Owner confirmation phrase. When
  applied, it can run SSH/scp, stop/start the backend service, create a PG
  backup, run Alembic migrations, and switch `app/current`; it must therefore be
  treated as `mutation_restricted`. It does not create execution records, create
  orders, call OrderLifecycle, or call exchange APIs.
