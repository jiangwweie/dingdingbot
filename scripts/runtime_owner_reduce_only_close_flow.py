...
an exact ``OWNER_APPROVED_RUNTIME_REDUCE_ONLY_CLOSE`` env value generated by
``build_runtime_reduce_only_close_owner_packet.py`` from fresh facts.
"""
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        base = {
            ...
            "runtime_instance_id": args.runtime_instance_id,
            "owner_packet": _json_value(owner_packet),
        ...
        if owner_packet.status != RuntimeReduceOnlyCloseOwnerPacketStatus.READY_FOR_OWNER_AUTHORIZATION:
            return {
                **base,
                "status": "blocked_before_owner_authorization",
                "executed": False,
                "blockers": list(owner_packet.blockers),
                "safety_invariants": {
        ...
        if approval_value != owner_packet.owner_approval_value:
            return {
                **base,
                "status": "ready_for_owner_authorization",
                "executed": False,
                "required_approval_env": APPROVAL_ENV,
                "required_approval_value": owner_packet.owner_approval_value,
        ...
        if qty != owner_packet.close_quantity:
            return {
                **base,
                "status": "blocked_before_exchange_write",
                "executed": False,
                "blockers": ["active_position_quantity_changed_after_owner_packet"],
                "active_position_quantity": str(qty),
                "owner_packet_quantity": str(owner_packet.close_quantity),
                "safety_invariants": {
                    **base["safety_invariants"],
                    "exchange_write_called": False,
                    "order_created": False,
        ...
        result = await orchestrator.execute_controlled_close(
            ...
            max_amount=owner_packet.close_quantity,
