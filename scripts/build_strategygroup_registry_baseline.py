...
def _row_specs() -> list[dict[str, Any]]:
    return [
        ...
        _main_handoff_row(
            ...
            owner_label="CPM 回踩收复",
            edge_thesis="Capture trend-intact pullback followed by reclaim, distinct from MPG momentum continuation.",
            trade_logic="Long-only pullback-reclaim lane with trend break, failed reclaim, liquidity, funding, and action-time account disables.",
            regime_fit="Bullish rebound or trend-continuation regime where pullback depth remains normal and reclaim confirms.",
            supported_sides=["long"],
            default_tier="L3",
            trial_eligible=True,
            required_facts_summary={
                "market": "15m/1h/4h closed structure, liquidity, mark, funding",
                "strategy": "htf trend intact, pullback depth normal, reclaim confirmed",
                "risk": "invalidated below level, protection plan, exit plan, time stop",
                "account_exchange": "action-time available balance, active position, open orders, symbol availability, min notional, step, tick, leverage limit",
            },
            promotion_gate="Armed observation only; real submit still depends on fresh signal, action-time RequiredFacts, FinalGate, and Operation Layer.",
            downshift_rule="Downshift if pullback/reclaim facts, action-time account facts, liquidity, or funding context become unavailable.",
            park_rule="Park if identity evidence regresses or CPM becomes redundant with MPG after review.",
            kill_condition="Kill if replay/live review shows pullback-reclaim has persistent false continuation after costs and protection.",
            evidence_refs=[
                _evidence("handoff", "docs/current/strategy-group-handoffs/CPM-RO-001/handoff.json"),
                _evidence("identity_decision", "output/runtime-monitor/latest-cpm-identity-routing-decision.json"),
                _evidence("required_facts_mapping", "output/runtime-monitor/latest-cpm-required-facts-mapping.json"),
                _evidence("tier_policy", "docs/current/strategy-group-handoffs/main-control-runtime-tier-policy.json"),
            ],
            risk_gaps={
                "strategy_quality_risk": ["failed reclaim", "trend break after pullback", "choppy rebound regime"],
                "fact_coverage_risk": ["trend, pullback depth, reclaim, liquidity, funding, and action-time account facts"],
                "economic_risk": ["fee, funding, fill-gap slippage, and pullback continuation failure"],
                "execution_safety_risk": ["missing protection, stale account facts, open-order or active-position conflict"],
                "authority_risk": ["L3 armed observation is not real submit authority"],
            },
            evidence_status="standalone_trial_asset_identity_closed",
            required_next_evidence="fresh CPM-LONG signal followed by non-executing candidate authorization and action-time rehearsal evidence",
        ),
        _main_handoff_row(
