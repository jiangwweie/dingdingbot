#!/usr/bin/env python3
"""Build first bounded live-order closure evidence artifacts from official reports.

This is a local evidence assembler. It reads JSON reports that already exist and
...
EVIDENCE_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "required_facts_readiness_artifact_id": (
        "required_facts_readiness_artifact_id",
        "strategy_group_live_facts_readiness_artifact_id",
        "live_facts_readiness_artifact_id",
...
def build_live_closure_evidence_artifact(
    ...
    *,
    ...
    evidence = _extract_evidence(source_artifacts=source_artifacts, required_keys=required_keys)
    evidence.update(
        {
            str(key): value
    ...
    return {
        ...
        "evidence": evidence,
        "input_count": len(source_artifacts),
        "input_summaries": [_input_summary(artifact) for artifact in source_artifacts],
        "safety_invariants": {
...
def _extract_evidence(
    *,
    ...
    for key in required_keys:
        value = _first_present_value(source_artifacts, EVIDENCE_ALIASES.get(key, (key,)))
        if _present(value):
            evidence[key] = value
    return evidence


def _first_present_value(source_artifacts: list[dict[str, Any]], aliases: tuple[str, ...]) -> Any:
    for alias in aliases:
        for artifact in source_artifacts:
            value = _find_key(artifact, alias)
            if _present(value):
                return value
    return None
...
def _derive_reject_reasons(
    *,
    ...
    status_text = " ".join(_status_like_values(source_artifacts)).lower()
    ...
    if official_live_source and _present(evidence.get(LIVE_SIGNAL_CHAIN_KEY)):
        if _present(evidence.get("candidate_id")) and not _present(
            evidence.get("required_facts_readiness_artifact_id")
        ):
    ...
    if live_signal_chain_missing:
        if "required_facts_readiness_artifact_id" in live_signal_chain_missing:
            reasons.add("required_facts_signal_source_missing")
    ...
    if authorization_chain_missing:
        ...
        if ACTION_TIME_FINALGATE_CHAIN_KEY in authorization_chain_missing:
    ...
    if exchange_result_present:
        ...
        if False in _bool_values(source_artifacts, "executes_real_submit"):
            reasons.add("real_order_not_placed")
        if False in _bool_values(source_artifacts, "live_submit_allowed"):
            reasons.add("real_order_not_placed")
        if True in _bool_values(source_artifacts, "controlled_fake_gateway_called"):
            reasons.add("controlled_in_memory_execution")
        if True in _bool_values(source_artifacts, "controlled_order_lifecycle_submit_called"):
            reasons.add("controlled_in_memory_execution")
        if True in _bool_values(source_artifacts, "controlled_in_memory_execution_result_recorded"):
            reasons.add("controlled_in_memory_execution")
        if (
        ...
        if close_loop_missing:
            if "runtime_post_submit_finalize_payload_id" in close_loop_missing:
                reasons.add("post_submit_finalize_result_source_missing")
            if close_loop_missing - {"runtime_post_submit_finalize_payload_id"}:
...
def _live_signal_chain_proof(
    *,
    ...
    live_signal_evidence_id = _evidence_id(evidence.get(LIVE_SIGNAL_CHAIN_KEY))
    ...
    for key in LIVE_SIGNAL_CHAIN_EVIDENCE_KEYS:
        ...
        key_source_artifacts = _source_artifacts_for_evidence_id(
            source_artifacts,
            aliases=EVIDENCE_ALIASES[key],
            evidence_id=evidence_id,
        )
        signal_bound = bool(live_signal_evidence_id) and any(
            _artifact_has_evidence_id(
                artifact,
                aliases=EVIDENCE_ALIASES[LIVE_SIGNAL_CHAIN_KEY],
                evidence_id=live_signal_evidence_id,
            )
            for artifact in key_source_artifacts
        )
        if signal_bound:
    ...
    if live_signal_evidence_id:
        proof[LIVE_SIGNAL_CHAIN_KEY] = live_signal_evidence_id
    return proof
...
def _live_submit_proof(
    *,
    ...
    result_source_artifacts = _source_artifacts_for_evidence_id(
        source_artifacts,
    ...
    proof: dict[str, Any] = {
        ...
        "result_source_matched": bool(result_source_artifacts),
        "result_source_count": len(result_source_artifacts),
        "live_exchange_called": _any_true(
            result_source_artifacts,
            LIVE_EXCHANGE_CALLED_KEYS,
        ),
...
def _exchange_native_protection_proof(
    *,
    ...
    protection_source_artifacts = _source_artifacts_for_evidence_id(
        source_artifacts,
    ...
    *,
    ...
    for key in POST_SUBMIT_CLOSE_LOOP_EVIDENCE_KEYS:
        ...
        if result_bound:
            ...
            truth_values[truth_field] = _any_true(key_source_artifacts, truth_aliases)
        else:
            missing_source_match_keys.append(key)
    proof: dict[str, Any] = {
        "present_evidence_keys": present_keys,
...
def _runtime_boundary_proof(
    *,
    ...
    boundary_source_artifacts = _runtime_boundary_source_artifacts(
        source_artifacts=source_artifacts,
        evidence=evidence,
    )
    ...
    for field in RUNTIME_BOUNDARY_FIELDS:
        ...
        for artifact in boundary_source_artifacts:
            for alias in RUNTIME_BOUNDARY_ALIASES[field]:
                for value in _collect_values_for_keys(artifact, {alias}):
    ...
    return {
        "source_artifact_count": len(boundary_source_artifacts),
...
def _runtime_boundary_source_artifacts(
    *,
    source_artifacts: list[dict[str, Any]],
    evidence: dict[str, Any],
) -> list[dict[str, Any]]:
    ...
    for key, aliases in EVIDENCE_ALIASES.items():
        ...
        if not evidence_id:
            continue
        for artifact in _source_artifacts_for_evidence_id(
            source_artifacts,
            aliases=aliases,
            evidence_id=evidence_id,
        ):
...
def _source_artifacts_for_evidence_id(
    source_artifacts: list[dict[str, Any]],
    *,
    aliases: tuple[str, ...],
...
def _artifact_has_evidence_id(
    ...
    *,
    ...
    if not evidence_id:
        return False
    return any(
        _evidence_id(_find_key(artifact, alias)) == evidence_id
        for alias in aliases
    )


def _status_like_values(source_artifacts: list[dict[str, Any]]) -> list[str]:
    values: list[str] = []
    for artifact in source_artifacts:
        values.extend(_collect_values_for_keys(artifact, {"scope", "status", "source_kind", "execution_mode"}))
    return [str(item) for item in values if str(item)]


def _bool_values(source_artifacts: list[dict[str, Any]], target_key: str) -> list[bool]:
    return [
        item
        for artifact in source_artifacts
        for item in _collect_values_for_keys(artifact, {target_key})
        if isinstance(item, bool)
    ]


def _any_true(source_artifacts: list[dict[str, Any]], target_keys: tuple[str, ...]) -> bool:
    return any(
        item is True
        for artifact in source_artifacts
        for item in _collect_values_for_keys(artifact, set(target_keys))
        if isinstance(item, bool)
    )
...
def _input_summary(artifact: dict[str, Any]) -> dict[str, str | None]:
    return {
        "scope": _string_or_none(artifact.get("scope")),
        "status": _string_or_none(artifact.get("status")),
        "source_kind": _string_or_none(artifact.get("source_kind")),
    }
...
def _evidence_id(value: Any) -> str | None:
    if isinstance(value, str):
        ...
        return text or None
    if isinstance(value, dict):
        for field in verifier.EVIDENCE_ID_FIELDS:
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build first bounded live-order closure evidence artifact."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    artifact = build_live_closure_evidence_artifact(
        source_artifacts,
        source_kind=source_kind,
