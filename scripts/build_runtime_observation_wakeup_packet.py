#!/usr/bin/env python3
"""Build an Owner wake-up handoff packet from runtime observation evidence.

The packet consumes an existing runtime observation operator packet. It does
...
def build_wakeup_packet(operator_packet: dict[str, Any]) -> dict[str, Any]:
    safety = _as_dict(operator_packet.get("safety_invariants"))
    command_plan = _as_dict(operator_packet.get("operator_command_plan"))
    prepare_context = _as_dict(operator_packet.get("runtime_prepare_context"))
    active = _as_dict(operator_packet.get("active_runtime_observation"))
    signal_counts = _as_dict(operator_packet.get("signal_counts"))
    forbidden_effects = _forbidden_effects(operator_packet)
    ...
    if forbidden_effects:
    ...
    elif operator_packet.get("status") == "observation_running_no_signal":
    ...
    elif operator_packet.get("status") == "no_signal_window_complete":
        ...
        owner_attention = "review_when_available"
        next_step = "review_operator_packet_status"

    return {
        "scope": "runtime_observation_wakeup_packet",
        "status": status,
        "source_operator_status": operator_packet.get("status"),
        ...
        "safety_invariants": {
            ...
            "source_packet_read_only": safety.get("operator_packet_only") is True,
            "pg_observation_written": False,
            "runtime_resolver_called": False,
            "shadow_candidate_created_by_wakeup_packet": False,
...
def build_wakeup_packet_from_path(operator_packet_json: str | Path) -> dict[str, Any]:
    return build_wakeup_packet(_load_json_object(Path(operator_packet_json).expanduser()))
...
def _allowed_while_owner_asleep(
    *,
    ...
    return list(command_plan.get("allowed_next_actions") or [])
...
def _forbidden_effects(packet: dict[str, Any]) -> list[str]:
    ...
    for key in (
        ...
        if plan.get(key) is True:
            effects.append(f"command_plan.{key}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    packet = build_wakeup_packet_from_path(args.operator_packet_json)
