#!/usr/bin/env python3
"""Controlled tiny-live readiness projection to local runtime cycle proof.

RTF-102 composes:

RTF-101 readiness-projection official preflight proof
-> RTF-091 official fresh-candidate runtime cycle handoff proof

It proves the ready projection can be followed by the local official route
through controlled in-memory submit, durable execution result, post-submit
finalize, budget / attempt settlement, and next-attempt gate.
...
from scripts import runtime_controlled_tiny_live_readiness_to_preflight_proof as rtf101  # noqa: E402
...
ReadinessPreflightBuilder = Callable[[Path], dict[str, Any]]
...
def build_proof_report(
    output_dir: Path,
    *,
    readiness_preflight_builder: ReadinessPreflightBuilder | None = None,
    runtime_cycle_builder: RuntimeCycleBuilder | None = None,
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    readiness_builder = readiness_preflight_builder or rtf101.build_proof_report
    cycle_builder = runtime_cycle_builder or rtf091.build_proof_report

    readiness_preflight = readiness_builder(output_dir / "rtf101-readiness-preflight")
    ...
    artifacts = {
        "rtf101-readiness-preflight-report.json": readiness_preflight,
        "rtf091-runtime-cycle-report.json": runtime_cycle,
        "readiness-to-local-runtime-cycle-artifact.json": artifact,
    }
    ...
    report = {
        "scope": "runtime_controlled_tiny_live_readiness_to_local_cycle_proof",
        "status": (
            "controlled_tiny_live_readiness_to_local_cycle_passed"
        ...
        "readiness_to_local_runtime_cycle_artifact": artifact,
        "checks": checks,
        "safety_invariants": artifact["safety_invariants"],
        "readiness_to_local_cycle_plan": {
            "next_step": (
                "tokyo_integration_probe_for_readiness_to_runtime_cycle"
                if _contract_passed(checks)
                else "resolve_readiness_to_runtime_cycle_blockers"
            ),
...
def _proof_artifact(
    *,
    ...
    readiness_artifact = readiness_preflight.get("readiness_to_official_preflight_artifact") or {}
    cycle_artifact = (
        runtime_cycle.get("fresh_candidate_runtime_cycle_artifact") or {}
    )
    checks = _checks(
        readiness_preflight=readiness_preflight,
    ...
    return {
        "scope": "runtime_controlled_tiny_live_readiness_to_local_cycle_artifact",
        "status": (
            "readiness_projection_ready_local_runtime_cycle_completed"
            if _contract_passed(checks)
            else "blocked"
        ),
        "readiness_preflight_side": {
            "status": readiness_preflight.get("status"),
            "waiting_projection_status": (readiness_artifact.get("waiting_path") or {}).get(
                "projection_status"
            ),
            "ready_projection_status": (readiness_artifact.get("ready_path") or {}).get(
                "projection_status"
            ),
            "official_preflight_status": (
                readiness_artifact.get("official_preflight") or {}
            ).get("status"),
            "preflight_status": (
                (readiness_artifact.get("official_preflight") or {}).get(
        ...
        "runtime_cycle_side": {
            ...
            "cycle_status": cycle_artifact.get("status"),
            "controlled_action_side": cycle_artifact.get("controlled_action_side") or {},
            "post_submit_side": cycle_artifact.get("post_submit_side") or {},
        },
        "checks": checks,
...
def _checks(
    *,
    ...
    runtime_cycle: dict[str, Any],
    cycle_artifact: dict[str, Any],
) -> dict[str, bool]:
    readiness_checks = readiness_preflight.get("checks") or {}
    cycle_checks = runtime_cycle.get("checks") or {}
    post_submit = cycle_artifact.get("post_submit_side") or {}
    controlled_action = cycle_artifact.get("controlled_action_side") or {}
    ...
    return {
        "rtf101_readiness_preflight_passed": (
            readiness_preflight.get("status")
            == "controlled_tiny_live_readiness_to_official_preflight_passed"
        ),
        "waiting_projection_blocks_official_route": bool(
            readiness_checks.get("waiting_projection_blocks_official_route")
        ),
        "ready_projection_enters_official_prepare": bool(
            readiness_checks.get("ready_projection_enters_official_prepare")
        ),
        ...
        "runtime_cycle_current_artifact_present": (
            cycle_artifact.get("scope")
            == "runtime_official_fresh_candidate_runtime_cycle_artifact"
        ),
        "runtime_ids_match": bool(cycle_checks.get("runtime_ids_match")),
        "candidate_ids_match": (
            bool(cycle_checks.get("candidate_ids_match"))
            and readiness_preflight.get("order_candidate_id")
...
def _safety_invariants(
    *,
    ...
    readiness_safety = readiness_preflight.get("safety_invariants") or {}
    cycle_safety = runtime_cycle.get("safety_invariants") or {}
    return {
        "readiness_projection_no_forbidden_live_side_effects": bool(
            readiness_safety.get("readiness_projection_no_forbidden_live_side_effects")
        ),
        ...
        "runtime_cycle_status_completed": (
            cycle_artifact.get("status") == "fresh_candidate_cycle_handoff_completed"
        ),
    }
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a controlled tiny-live readiness projection to local runtime cycle proof."
        )
    )
    parser.add_argument(
        "--output-dir",
        default="output/rtf102-controlled-tiny-live-readiness-to-local-cycle",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    return (
        0
        if report["status"] == "controlled_tiny_live_readiness_to_local_cycle_passed"
        else 1
    )
