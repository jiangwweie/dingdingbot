...
async def main():
    ...
    print("\n" + "=" * 70)
    print(f"ETH 15m 结果: 可行解 {len(feasible_results)}/{total}")
    print("=" * 70)

    if not feasible_results:
        print("\n无可行解！显示所有结果（按 2024+2025 PnL 排序）:")
        results.sort(key=lambda x: x["pnl_2024"] + x["pnl_2025"], reverse=True)
        print(f"\n{'ema':>5} {'atr':>6} {'dist':>6} {'pnl24':>10} {'pnl25':>10} {'dd24':>8} {'dd25':>8} {'tr24':>5} {'tr25':>5}")
        print("-" * 75)
        for r in results[:15]:
        ...
        print(f"\nTop 15:")
        print(f"{'rank':>4} {'ema':>5} {'atr':>6} {'dist':>6} {'pnl24':>10} {'pnl25':>10} {'sh24':>7} {'sh25':>7} {'tr24':>5} {'tr25':>5}")
        print("-" * 80)
        for i, r in enumerate(feasible_results[:15], 1):
        ...
        print(f"  2024: pnl={best['pnl_2024']:+.2f}, sharpe={best['sharpe_2024']:.4f}, dd={best['max_dd_2024']:.2%}, trades={best['trades_2024']}")
        print(f"  2025: pnl={best['pnl_2025']:+.2f}, sharpe={best['sharpe_2025']:.4f}, dd={best['max_dd_2025']:.2%}, trades={best['trades_2025']}")

        # 与 ETH 1h 对比
        print("\n" + "-" * 70)
        print("与 ETH 1h 对比:")
        print(f"  ETH 1h Best:  ema=50, dist=0.005, pnl24=+5168, pnl25=+1645, trades=~80/yr")
        print(f"  ETH 15m Best: ema={best['ema']}, dist={best['dist']:.3f}, pnl24={best['pnl_2024']:+.0f}, pnl25={best['pnl_2025']:+.0f}, trades={best['trades_2024']+best['trades_2025']}/2yr")
