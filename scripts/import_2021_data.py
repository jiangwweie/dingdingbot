#!/usr/bin/env python3
"""
下载并导入 2021 年度 Binance 历史 K 线数据到本地数据库

功能:
1. 从 data.binance.vision 下载月度 K 线 ZIP 文件
2. 解压并解析 CSV
3. 转换为 KlineData 格式并写入 SQLite

使用方式:
    PYTHONPATH=. python3 scripts/import_2021_data.py
"""
import asyncio
import csv
import io
...
async def main():
    logger.info("=" * 60)
    logger.info("Binance 2021 年度历史 K 线数据导入")
    logger.info(f"  月份: 2021-01 至 2021-12")
    ...
    # 创建临时目录
    tmp_dir = tempfile.mkdtemp(prefix="binance_2021_klines_")
