#!/usr/bin/env python3
"""Build a non-mutating runtime profile confirmation record from RTF-035 output.
...
BLOCKED_STATUS = "blocked_runtime_profile_confirmation_record"
...
def _proposal_from_artifact(
    artifact: dict[str, Any],
) -> ExperimentalRuntimeProfileProposal | None:
    raw = artifact.get("experimental_runtime_profile_proposal")
    if raw is None:
        return None
...
def _blocked_record(
    *,
    ...
    return {
        ...
        "source_status": source_artifact.get("status"),
        "proposal_id": None,
...
def build_record(
    *,
    ...
    proposal = _proposal_from_artifact(proposal_artifact)
    if proposal is None:
        return _blocked_record(
            source_artifact=proposal_artifact,
            blockers=["runtime_profile_proposal_missing"],
        )
    blockers: list[str] = []
    warnings = [
        *list(proposal_artifact.get("warnings") or []),
        "confirmation_record_is_not_owner_confirmation_record",
        "confirmation_record_is_not_runtime_creation",
        "confirmation_record_is_not_execution_authority",
    ]
    if proposal_artifact.get("status") != "ready_for_owner_runtime_profile_decision":
        blockers.append("source_profile_artifact_not_ready")
    if (
        proposal.status
    ...
    record = StrategyRuntimePromotionGateConfirmationRecord(
        ...
        metadata={
            "source": "rtf036_runtime_profile_confirmation_record",
            "proposal_source_status": proposal_artifact.get("status"),
            "selected_signal_candidate_id": (
                proposal_artifact.get("selected_non_runtime_signal") or {}
            ).get("candidate_id"),
            "approval_not_recorded_by_this_record": True,
    ...
    return {
        ...
        "source_status": proposal_artifact.get("status"),
        ...
        "profile_confirmation_record_plan": {
            ...
            "this_record_creates_runtime": False,
            "this_record_places_order": False,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build RTF-036 runtime profile confirmation record from RTF-035 proposal output.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    record = build_record(
        proposal_artifact=_load_json(args.proposal_json),
    ...
    print(payload)
    return 0 if record["status"] == READY_STATUS else 2
