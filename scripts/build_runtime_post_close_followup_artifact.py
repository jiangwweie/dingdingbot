#!/usr/bin/env python3
"""Build a non-executing post-close follow-up projection for a runtime."""
...
def _post_close_followup_safety_invariants(*, exchange_read_only: bool) -> dict[str, bool]:
    return {
        "post_close_followup_projection_only": True,
...
def _post_close_followup_plan(
    *,
    ...
    approval_env = getattr(artifact, "owner_close_approval_env", None)
    approval_value = getattr(artifact, "owner_close_approval_value", None)
    ...
    followup_status = getattr(artifact, "status", "")
    post_close_complete = (
        str(getattr(followup_status, "value", followup_status)) == "post_close_complete"
    )
    standing_recovery_ready = (
        str(getattr(followup_status, "value", followup_status))
    ...
    return {
        ...
        "closed_review_command_args": (
            ...
            else list(getattr(artifact, "closed_review_command_args", []) or [])
        ),
        "post_close_required_sequence": (
            list(getattr(artifact, "required_steps", []) or [])
            if post_close_complete
            else [
                "refresh_followup",
                *(
        ...
        "safety_invariants": {
            "post_close_command_plan_projection_only": True,
...
async def _build_artifact(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        closed_review_facts_artifact = await closed_review_facts_service.build_artifact(
            runtime_instance_id=args.runtime_instance_id,
        )
        ...
        artifact = build_runtime_post_close_followup_artifact(
            ...
            closed_review_facts_artifact=closed_review_facts_artifact,
        ...
        post_close_followup_plan = _post_close_followup_plan(
            runtime_instance_id=args.runtime_instance_id,
        ...
        return {
            ...
            "owner_close_artifact": _json_value(owner_close_artifact)
            if owner_close_artifact is not None
            else None,
            "closed_review_facts_artifact": _json_value(closed_review_facts_artifact),
            "closed_lifecycle_review": _json_value(closed_review)
            if closed_review is not None
            else None,
            "post_close_followup_plan": post_close_followup_plan,
            "safety_invariants": _post_close_followup_safety_invariants(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a non-executing runtime post-close follow-up artifact.",
    )
