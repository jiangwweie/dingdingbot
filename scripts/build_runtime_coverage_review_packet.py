#!/usr/bin/env python3
"""Build a read-only ACTIVE runtime coverage review packet.

The packet consumes an existing runtime observation operator packet. It is a
posture review aid for no-signal windows: it compares ACTIVE runtime coverage
with the broader strategy preview shelf and summarizes no-action reasons.
...
def build_coverage_review_packet(operator_packet: dict[str, Any]) -> dict[str, Any]:
    safety = _as_dict(operator_packet.get("safety_invariants"))
    coverage = _as_dict(operator_packet.get("coverage"))
    active = _as_dict(operator_packet.get("active_runtime_observation"))
    counts = _as_dict(operator_packet.get("signal_counts"))
    diagnostics = _as_dict(operator_packet.get("no_action_diagnostics"))
    ...
    return {
        "scope": "runtime_coverage_review_packet",
        "status": status,
...
def build_coverage_review_packet_from_path(operator_packet_json: str | Path) -> dict[str, Any]:
    return build_coverage_review_packet(
        _load_json_object(Path(operator_packet_json).expanduser())
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    packet = build_coverage_review_packet_from_path(args.operator_packet_json)
