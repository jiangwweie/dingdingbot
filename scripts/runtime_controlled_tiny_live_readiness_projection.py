#!/usr/bin/env python3
"""Build controlled tiny-live readiness projection from selector refresh output.

RTF-100 consumes a selector-driven refresh artifact and decides whether the
runtime can move toward the existing official prepare / FinalGate /
controlled-submit preflight path.  It is projection-only and never creates
...
def _forbidden_effects(refresh_artifact: dict[str, Any]) -> dict[str, bool]:
    effects = {key: False for key in FORBIDDEN_EFFECT_KEYS}
    safety = refresh_artifact.get("safety_invariants")
...
def _selected(refresh_artifact: dict[str, Any]) -> dict[str, Any]:
    selected = refresh_artifact.get("selected_continuation")
    return selected if isinstance(selected, dict) else {}


def _status(refresh_artifact: dict[str, Any], effects: dict[str, bool]) -> str:
    if any(effects.values()):
        return "controlled_tiny_live_readiness_projection_blocked_forbidden_effect"
    refresh_status = str(refresh_artifact.get("status") or "")
    selected = _selected(refresh_artifact)
    if refresh_status not in READY_REFRESH_STATUSES:
        return "controlled_tiny_live_readiness_projection_waiting_for_ready_selector"
    if (
        refresh_status == "continuation_refresh_ready_for_final_gate_review"
        and selected.get("ready_for_final_gate_preflight") is True
    ):
        return "controlled_tiny_live_readiness_projection_ready_for_final_gate_review"
    if (
        refresh_status == "continuation_refresh_ready_for_prepare"
        and selected.get("ready_for_prepare") is True
    ):
        return "controlled_tiny_live_readiness_projection_ready_for_official_prepare"
    return "controlled_tiny_live_readiness_projection_blocked_inconsistent_selector"


def _readiness_plan(status: str, selected: dict[str, Any]) -> dict[str, Any]:
    if status == "controlled_tiny_live_readiness_projection_ready_for_final_gate_review":
        next_step = "run_official_final_gate_preflight_before_controlled_tiny_live_submit"
    elif status == "controlled_tiny_live_readiness_projection_ready_for_official_prepare":
        next_step = "run_official_prepare_then_final_gate_preflight"
    elif status == "controlled_tiny_live_readiness_projection_blocked_forbidden_effect":
        next_step = "stop_and_review_forbidden_side_effects"
    elif status == "controlled_tiny_live_readiness_projection_blocked_inconsistent_selector":
        next_step = "refresh_selector_before_readiness_projection"
    else:
...
def build_readiness_projection(
    *,
    refresh_artifact: dict[str, Any],
    deployed_head: str | None = None,
    release_name: str | None = None,
    remote_report_path: str | None = None,
) -> dict[str, Any]:
    selected = _selected(refresh_artifact)
    effects = _forbidden_effects(refresh_artifact)
    status = _status(refresh_artifact, effects)
    blockers: list[str] = []
    if status == "controlled_tiny_live_readiness_projection_waiting_for_ready_selector":
        blockers.append(str(refresh_artifact.get("status") or "selector_not_ready"))
    elif status == "controlled_tiny_live_readiness_projection_blocked_inconsistent_selector":
        blockers.append("selector_ready_status_without_matching_selected_flags")
    elif status == "controlled_tiny_live_readiness_projection_blocked_forbidden_effect":
        blockers.append("forbidden_live_side_effect_detected")
    return {
        "scope": "runtime_controlled_tiny_live_readiness_projection",
        "status": status,
        "source_refresh_status": refresh_artifact.get("status"),
        "source_readiness_status": refresh_artifact.get("readiness_status"),
        "source_selector_status": refresh_artifact.get("selector_status"),
        ...
        "readiness_inputs": {
            ...
            "ready_for_controlled_tiny_live_path": bool(
                (refresh_artifact.get("refresh_plan") or {}).get(
                    "ready_for_controlled_tiny_live_path"
                )
            ),
        },
        "blockers": blockers,
        "warnings": list(refresh_artifact.get("warnings") or []),
        "safety_invariants": {
            "projection_only": True,
            "no_forbidden_live_side_effects": not any(effects.values()),
            "forbidden_effects": effects,
            "api_called_by_projection": False,
            "pg_called_by_projection": False,
            "exchange_called_by_projection": False,
            "exchange_write_called_by_projection": False,
            "order_lifecycle_called_by_projection": False,
            "runtime_state_mutated_by_projection": False,
            "withdrawal_or_transfer_created_by_projection": False,
        },
        "readiness_plan": _readiness_plan(status, selected),
        "deployment_context": {
        ...
        "right_tail_objective_context": {
            ...
            "new_attempt_not_started_by_projection": True,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build controlled tiny-live readiness projection.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    projection = build_readiness_projection(
        refresh_artifact=_load_report(args.refresh_json),
        deployed_head=args.deployed_head,
        release_name=args.release_name,
        remote_report_path=args.remote_report_path,
    )
    if args.output_json:
        _write_json(args.output_json, projection)
    print(json.dumps(projection, ensure_ascii=False, indent=2, sort_keys=True, default=str))
