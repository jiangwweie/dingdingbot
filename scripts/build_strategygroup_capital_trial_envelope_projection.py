#!/usr/bin/env python3
"""Build a review-only Capital Trial Envelope projection.

This command converts the StrategyGroup Portfolio Board trial pool into a
pre-registered capital-trial evidence projection. It is non-executing: it never
...
SCHEMA = "brc.strategygroup_capital_trial_envelope_projection.v1"
PROJECTION_SCHEMA = "brc.strategygroup_capital_trial_envelope_projection.v1"
TRIAL_ENVELOPE_SCHEMA = "brc.strategygroup_capital_trial_envelope.v0"
...
BRF2_PROMOTION_TARGET = "paper_observation_or_experiment_evidence"
BRF2_PROMOTION_REASON = "promote_to_tiny_live_intake_candidate_not_live_ready"
BRF2_NEXT_CHECKPOINT = "BRF2-001_tiny_live_intake_evidence"
BRF2_REQUIRED_NEXT_EVIDENCE = (
    "owner_policy_scope",
    "paper_observation_evidence",
    "RequiredFacts draft",
    "disable facts",
    "path-risk review",
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-trial-envelope-json", default=str(DEFAULT_TRIAL_ENVELOPE_JSON)
    )
    parser.add_argument("--output-trial-envelope-md", default=str(DEFAULT_TRIAL_ENVELOPE_MD))
    parser.add_argument(
        "--research-intake-review-json",
        default="",
        help=(
            "Optional main-control research intake review artifact. When supplied, "
            "paper-observation short candidates can enter the experiment-candidate pool "
    ...
    projection = build_capital_trial_envelope_projection(
        portfolio_board=portfolio_board,
        capture_gap_audit=capture_gap_audit,
        research_intake_review=research_intake_review,
    )
    trial_envelope = projection["trial_envelope_v0"]

    output_json = Path(args.output_json)
    output_md = Path(args.output_owner_progress)
    trial_json = Path(args.output_trial_envelope_json)
    trial_md = Path(args.output_trial_envelope_md)
    _write_json(output_json, projection)
    _write_text(output_md, _projection_markdown(projection, output_json, trial_json))
    _write_json(trial_json, trial_envelope)
    _write_text(trial_md, _trial_envelope_markdown(trial_envelope, trial_json))

    print(
        json.dumps(
            {
                "status": projection["status"],
                "selected_non_mpg_strategy_group_id": projection[
                    "capital_trial_summary"
                ]["selected_non_mpg_strategy_group_id"],
                "trial_envelope_generated": projection["capital_trial_summary"][
    ...
    return 0 if projection["projection_status"] == "trial_envelope_projection_ready" else 2


def build_capital_trial_envelope_projection(
    *,
    ...
    selected = next(
        (
            ...
            and (
                row["trial_recommendation"].startswith(
    ...
    trial_envelope = _trial_envelope_v0(selected, generated_at=generated_at)
    continuation_queue = [
    ...
    reject_reasons = _projection_reject_reasons(
    ...
    status = (
        ...
        else "trial_envelope_projection_needs_work"
    )

    return {
        "schema": SCHEMA,
        "projection_schema": PROJECTION_SCHEMA,
        "scope": "strategygroup_capital_trial_envelope_projection_review_only",
        "projection_metadata": {
        ...
        "runtime_posture": {
            ...
            "signal_observation_grade": "capital_trial_prepare_candidate_selected",
        ...
        "capital_trial_summary": {
            ...
            "trial_envelope_generated": bool(trial_envelope),
            "live_permission_change_count": 0,
            "short_experiment_candidate_count": sum(
                1
                for row in eligibility_rows
    ...
    *,
    ...
    return {
        ...
        **promotion,
        ...
        "paper_observation_evidence_shape": _as_dict(
            research_row.get("paper_observation_evidence_shape")
        ),
    }
...
def _eligibility_row(
    *,
    ...
    return {
        ...
        "ranking_score": ranking_score,
        "trial_recommendation": recommendation,
        "strategy_asset_current_decision": "pending",
        ...
        "authority_boundary_summary": (
            "promotion_scope=not_applicable; non_executing_trial_readiness=false; "
...
def _trial_envelope_v0(
    ...
    *,
    ...
    return {
        "schema": TRIAL_ENVELOPE_SCHEMA,
        "envelope_id": f"trial-envelope-v0:{strategy_group_id}:20260622",
        "generated_at_utc": generated_at,
        "strategy_group_id": strategy_group_id,
        "policy_outcome": selected.get("strategy_asset_current_decision") or "pending",
        ...
        "hypothesis": (
            ...
            "as non-executing candidate-trade evidence, pending Owner policy "
        ...
        "paper_observation_evidence_shape": (
            selected.get("paper_observation_evidence_shape") or {}
        ),
        "trial_boundary": {
...
def _research_intake_blockers(research_row: dict[str, Any]) -> list[str]:
    ...
    if research_row.get("source_non_executing_trial_readiness") is not True:
        blockers.append("source_non_executing_trial_readiness_not_closed")
    blockers.extend(
        [
            "owner_capital_scope_not_confirmed",
...
def _research_trial_recommendation(
    *,
    ...
    if (
        ...
        return (
            "experiment_candidate_prepare_pending_owner_policy",
            "short_experiment_evidence_pending_owner_policy",
        )
    if strategy_group_id == "RBR2-001":
        return (
            "role_only_short_experiment_candidate_watchlist",
            "role_only_short_experiment_candidate_watchlist",
        )
...
def _research_promotion_semantics(
    *,
    ...
    if (
        strategy_group_id == "BRF2-001"
        and recommendation == "experiment_candidate_prepare_pending_owner_policy"
    ):
        return {
            "strategy_asset_current_decision": "promote",
            "reason": BRF2_PROMOTION_REASON,
            "strategy_asset_seed_source": "curated_research_intake_strategy_asset_seed",
            ...
            "authority_boundary_summary": (
                "promotion_scope=intake_only; non_executing_trial_readiness=false; "
                "no_finalgate_no_operation_layer_no_exchange_write"
            ),
        }
    return {
        ...
        "reason": recommendation,
        "strategy_asset_seed_source": "missing_strategy_asset_seed",
        "promotion_scope": "not_applicable",
        "promotion_target": "not_applicable",
        "tiny_live_ready": False,
...
def _projection_reject_reasons(
    *,
    ...
    if trial_envelope.get("schema") != TRIAL_ENVELOPE_SCHEMA:
        reasons.append("trial_envelope_missing")
    reasons.extend(
        legacy_authority_mirror_present_errors(
            trial_envelope,
    ...
    if trial_envelope.get("live_permission_change") is not False:
        reasons.append("trial_envelope_live_permission_change_not_false")
    if trial_envelope.get("policy_outcome") == "promote":
        if trial_envelope.get("promotion_scope") != BRF2_PROMOTION_SCOPE:
            reasons.append("unscoped_promote_forbidden")
        boundary = trial_envelope.get("authority_boundary")
        ...
        if trial_envelope.get("tiny_live_ready") is not False:
            reasons.append("trial_envelope_tiny_live_ready_not_false")
    for key, value in safety.items():
        if key in FORBIDDEN_EFFECTS and value is True:
            reasons.append(f"forbidden_effect:{key}")
        if key in LEGACY_AUTHORITY_MIRROR_TRUE_KEYS and value is True:
            reasons.append(f"legacy_authority_mirror_present:{key}")
    return _dedupe(reasons)


def _portfolio_rows_by_id(source_artifact: dict[str, Any]) -> dict[str, dict[str, Any]]:
    rows = source_artifact.get("portfolio_rows")
...
def _trial_pool_rows_by_id(source_artifact: dict[str, Any]) -> dict[str, dict[str, Any]]:
    pool = source_artifact.get("trial_candidate_pool")
...
def _audit_rows_by_id(source_artifact: dict[str, Any]) -> dict[str, dict[str, Any]]:
    rows = source_artifact.get("strategy_expectation_rows")
    if not isinstance(rows, list):
        return {}
    return {
        str(row.get("strategy_group_id")): row
...
def _research_intake_rows_by_id(source_artifact: dict[str, Any]) -> dict[str, dict[str, Any]]:
    rows = source_artifact.get("candidate_rows")
    if not isinstance(rows, list):
        return {}
    return {
        str(row.get("strategy_group_id")): row
...
def _would_enter_events_by_id(source_artifact: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    events = source_artifact.get("would_enter_events")
...
def _research_symbol_scope(row: dict[str, Any]) -> list[str]:
    shape = _as_dict(row.get("paper_observation_evidence_shape"))
...
def _validate_safety(name: str, source_artifact: dict[str, Any]) -> None:
    ...
    forbidden = [
        ...
        if safety.get(key) is True or source_artifact.get(key) is True
    ]
    legacy_mirrors = [
        key
    ...
    if legacy_mirrors:
        raise ValueError(
            f"{name} has legacy authority mirrors: {', '.join(legacy_mirrors)}"
        )
...
def _projection_markdown(
    projection: dict[str, Any], output_json: Path, trial_json: Path
) -> str:
    summary = projection["capital_trial_summary"]
    selected = projection["selected_non_mpg_trial_candidate"]
    lines = [
        "## StrategyGroup Capital Trial Envelope Projection",
        "",
        f"- Status: {projection['status']}",
        f"- Projection status: {projection['projection_status']}",
        f"- Generated: {projection['generated_at_utc']}",
        f"- Output JSON: {output_json}",
        f"- Trial Envelope v0 JSON: {trial_json}",
        ...
        f"- Trial envelope generated: {_yes_no(bool(summary['trial_envelope_generated']))}",
    ...
    for index, row in enumerate(projection["non_mpg_trial_candidate_ranking"], start=1):
        lines.append(
            "| "
            + " | ".join(
                [
...
def _trial_envelope_markdown(trial_envelope: dict[str, Any], output_json: Path) -> str:
    boundary = trial_envelope["authority_boundary"]
    lines = [
        "## StrategyGroup Capital Trial Envelope v0",
        "",
        f"- Envelope ID: {trial_envelope['envelope_id']}",
        f"- StrategyGroup: {trial_envelope['strategy_group_id']}",
        f"- Candidate status: {trial_envelope['candidate_status']}",
        f"- Policy outcome: {trial_envelope['policy_outcome']}",
        f"- Reason: {trial_envelope['reason']}",
        f"- Promotion scope: {trial_envelope['promotion_scope']}",
        f"- Promotion target: {trial_envelope['promotion_target']}",
        f"- Next checkpoint: {trial_envelope['next_checkpoint']}",
        f"- Output JSON: {output_json}",
        f"- Live permission change: {_yes_no(bool(trial_envelope['live_permission_change']))}",
        "",
        "## Boundary",
        "",
        "| Check | Value |",
