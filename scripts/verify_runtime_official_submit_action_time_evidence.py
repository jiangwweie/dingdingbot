#!/usr/bin/env python3
"""Verify official submit action-time evidence locally.

RTF-051 starts from the RTF-050 handoff artifact and drives the existing
...
from scripts.verify_runtime_next_attempt_submit_preparation_evidence import (  # noqa: E402
...
def _disabled_smoke_action_time_scenario(
    *,
    ...
    report = smoke_flow._build_report(
        _args(handoff_artifact_json=handoff_path, output=output_path),
        client=client,
    )
...
async def build_official_submit_action_time_evidence_report(
    *,
    ...
    submit_preparation_report = (
        await build_next_attempt_submit_preparation_evidence_report()
    )
    ...
    _write_json(
        disabled_handoff_path,
        {"handoff_artifact": ready["disabled_handoff_artifact"]},
    )
    _write_json(
        real_handoff_path,
        {"handoff_artifact": ready["real_mode_handoff_artifact"]},
    )
    ...
    return {
        "scope": "rtf051_official_submit_action_time_evidence",
        "status": (
            "rtf051_official_submit_action_time_evidence_passed"
            if passed
            else "rtf051_official_submit_action_time_evidence_failed"
        ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify official submit action-time evidence locally.",
    )
