...
def build_routing_packet(
    *,
    ...
    if selector_status == "runtime_compatible_would_enter_selected":
        ...
        if proposal_packet.get("status") == READY_PROFILE_STATUS:
            ...
            blockers = list(proposal_packet.get("blockers") or [])
    ...
    signal_input_json = selector_packet.get("output_signal_input_json")
    return {
        "scope": "runtime_live_signal_routing_packet",
        "status": status,
        "source_selector_status": selector_status,
        "runtime_instance_id": selector_packet.get("runtime_instance_id"),
        "runtime_profile": selector_packet.get("runtime_profile"),
        "selected_signal": selector_packet.get("selected_signal"),
        "non_runtime_would_enter_signals": list(
            selector_packet.get("non_runtime_would_enter_signals") or []
        ),
        "runtime_current_signal": selector_packet.get("runtime_current_signal"),
        "signal_input_json": signal_input_json,
        "profile_proposal_packet": proposal_packet,
        "blockers": _dedupe(blockers),
        "warnings": _dedupe(
            [
                *list(selector_packet.get("warnings") or []),
                "routing_packet_is_not_execution_authority",
...
async def _build_packet(
    ...
    *,
    ...
    builder = selector_builder or selector_script._build_packet
    ...
    selector_packet = await builder(selector_args)
    return build_routing_packet(
        selector_packet=selector_packet,
...
def _safety_invariants(
    *,
    ...
    selector_safety = _as_dict(selector_packet.get("safety_invariants"))
    proposal_safety = (
        _as_dict(proposal_packet.get("safety_invariants"))
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a read-only runtime live-signal routing packet.",
    )
...
def _main(
    ...
    *,
    ...
    with redirect_stdout(sys.stderr):
        packet = asyncio.run(_build_packet(args, selector_builder=selector_builder))
