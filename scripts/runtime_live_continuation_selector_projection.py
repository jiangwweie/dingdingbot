#!/usr/bin/env python3
"""Build a live runtime continuation selector projection.

RTF-097 combines the current live-attempt readiness artifact with optional
per-runtime follow-up artifacts such as the BNB position lifecycle artifact.  It is
projection-only: it does not call APIs, create candidates, close positions,
submit orders, mutate runtime state, call exchange, or move funds.
"""
...
def _runtime_rows(readiness_artifact: dict[str, Any]) -> list[dict[str, Any]]:
...
def _lifecycle_by_runtime(
    ...
    for artifact in lifecycle_artifacts:
        runtime_id = str(artifact.get("runtime_instance_id") or "")
        if runtime_id:
            result[runtime_id] = artifact
    return result
...
def _continuation_row(
    ...
    if not isinstance(signal, dict):
        signal = {}
    lifecycle_plan = lifecycle.get("position_lifecycle_plan") if lifecycle else None
    ...
    return {
        ...
        "lifecycle_next_step": lifecycle_plan.get("next_step"),
        "allows_new_attempt_now": bool(lifecycle_plan.get("allows_new_attempt_now")),
        "reduce_only_close_ready_for_owner_authorization": bool(
            lifecycle_plan.get("reduce_only_close_ready_for_owner_authorization")
        ),
        "reduce_only_recovery_ready_for_standing_authorization": bool(
            lifecycle_plan.get("reduce_only_recovery_ready_for_standing_authorization")
        ),
        "owner_close_approval_value": lifecycle_plan.get("owner_close_approval_value"),
        "standing_recovery_authorization_scope": lifecycle_plan.get(
...
def _selector_plan(status: str, selected: dict[str, Any] | None) -> dict[str, Any]:
...
def build_selector_projection(
    *,
    ...
    lifecycle_map = _lifecycle_by_runtime(lifecycle_artifacts)
    rows = [
        _continuation_row(row, lifecycle_map.get(str(row.get("runtime_instance_id") or "")))
        for row in _runtime_rows(readiness_artifact)
    ]
    rows.sort(key=lambda item: item.get("priority", 0), reverse=True)
    effects = _forbidden_effects(readiness_artifact, *lifecycle_artifacts)
    status = _selector_status(rows, effects)
    selected = rows[0] if rows else None
    blockers = _dedupe(
        [
            *(readiness_artifact.get("blockers") or []),
            *[
    ...
    warnings = _dedupe(
        [
            *(readiness_artifact.get("warnings") or []),
            *[
            ...
            *[
                f"{artifact.get('runtime_instance_id')}:{warning}"
                for artifact in lifecycle_artifacts
    ...
    return {
        "scope": "runtime_live_continuation_selector_projection",
        "status": status,
        ...
        "selector_plan": _selector_plan(status, selected),
        "deployment_context": {
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build live runtime continuation selector projection.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    projection = build_selector_projection(
        readiness_artifact=_load_report(args.readiness_json),
        lifecycle_artifacts=[
            _load_report(path)
