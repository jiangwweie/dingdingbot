#!/usr/bin/env python3
"""Build a main-control intake artifact from StrategyGroup handoff packs.

The builder is intentionally read-only. It reads research-window handoff JSON
files and emits a main-control artifact for Strategy Picker, RequiredFacts
...
def _group_row(path: Path, data: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]], list[str]]:
    ...
    if missing:
        intake_status = "blocked_intake_source_incomplete"
    ...
    row = {
        ...
        "sample_statuses": {
            "signal": _sample_status(data, "sample_signal_artifact"),
            "no_signal": _sample_status(data, "sample_no_signal_artifact"),
            "stale_signal": _sample_status(data, "sample_stale_signal_artifact"),
            "conflict": _sample_status(data, "sample_conflict_artifact"),
        },
...
def build_artifact(
    *,
    ...
    status = "blocked_strategy_intake_source" if blockers else "ready_for_main_control_intake"
    return {
        ...
        "source_anchor": {
            ...
            "intake_source_dir": str(handoff_dir),
        },
        "counts": {
        ...
        "next_main_control_steps": [
            "record StrategyGroup intake source evidence",
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a main-control StrategyGroup handoff intake artifact.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 0 if artifact["status"] == "ready_for_main_control_intake" else 2
