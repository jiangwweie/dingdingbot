#!/usr/bin/env python3
"""Build a position lifecycle / exit-path readiness projection.

RTF-096 consumes existing projection/evidence artifacts only:

- RTF-095 next-attempt gate blocker classification
- runtime live-position monitor
- optional runtime position exit plan
- optional post-close follow-up artifact
...
def build_lifecycle_artifact(
    *,
    ...
    monitor = _payload(live_position_monitor, "artifact")
    exit_plan = _payload(position_exit_plan, "plan")
    followup = _payload(post_close_followup, "artifact")
    ...
    return {
        ...
        "exit_path": {
            ...
            "recommended_recovery_action": exit_plan.get(
        ...
        "safety_invariants": {
            ...
            "pg_called_by_lifecycle_artifact": False,
            "exchange_called_by_lifecycle_artifact": False,
            "exchange_write_called_by_lifecycle_artifact": False,
            "order_lifecycle_called_by_lifecycle_artifact": False,
            "runtime_state_mutated_by_lifecycle_artifact": False,
            "withdrawal_or_transfer_created_by_lifecycle_artifact": False,
        },
        "position_lifecycle_plan": _operator_plan(status, followup),
        "deployment_context": {
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build position lifecycle / exit-path readiness artifact.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    artifact = build_lifecycle_artifact(
