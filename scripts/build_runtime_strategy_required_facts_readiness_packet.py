#!/usr/bin/env python3
"""Build a strategy RequiredFacts readiness packet from local semantics.

The packet is an operator-facing view over the current strategy semantics
...
def _source_packet(fact_sources: dict[str, Any], source_key: str) -> dict[str, Any]:
...
def _fact_coverage(
    *,
    ...
    for requirement in binding.required_facts:
        ...
        source = _source_packet(fact_sources, source_key)
        status = _source_status(source)
...
def _overall_status(
    *,
    strategy_packets: list[dict[str, Any]],
    missing_strategy_packets: list[dict[str, Any]],
    forbidden_effects: list[str],
) -> str:
    if forbidden_effects:
        return "blocked_forbidden_effect"
    if missing_strategy_packets:
        return "blocked_strategy_semantics_missing"
    statuses = {packet["status"] for packet in strategy_packets}
...
def build_strategy_required_facts_readiness_packet(
    *,
    ...
    strategy_packets = [
        _binding_packet(binding=binding, fact_sources=fact_sources)
        for binding in selected
    ]
    forbidden_effects = _truthy_keys(fact_sources, FORBIDDEN_TRUE_KEYS)
    status = _overall_status(
        strategy_packets=strategy_packets,
        missing_strategy_packets=missing,
    ...
    return {
        "scope": "runtime_strategy_required_facts_readiness_packet",
        "status": status,
        "generated_at_ms": generated_at_ms
        if generated_at_ms is not None
        else int(time.time() * 1000),
        "strategy_count": len(strategy_packets),
        "strategies": strategy_packets,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a P1-B strategy RequiredFacts readiness packet.",
    )
    parser.add_argument(
        "--strategy",
        action="append",
