#!/usr/bin/env python3
"""Refresh StrategyGroup runtime product-state artifacts from local readmodels.
...
Owner-readable artifacts used by heartbeat automation. It never creates
...
from src.application.readmodels.owner_projection import owner_non_authority_checkpoint
...
def _real_order_readiness_status_evidence(
...
def _goal_status_ready_for_real_order_action(
    goal_status_artifact: dict[str, Any],
) -> bool:
    ready_value = goal_status_artifact.get("ready_for_real_order_action")
    if isinstance(ready_value, bool):
        return ready_value
    checks = (
        goal_status_artifact.get("checks")
        if isinstance(goal_status_artifact.get("checks"), dict)
        else {}
    )
    ...
    if isinstance(ready_value, bool):
        return ready_value
    boundary = (
        goal_status_artifact.get("real_order_boundary")
        if isinstance(goal_status_artifact.get("real_order_boundary"), dict)
        else {}
    )
    return boundary.get("ready_for_real_order_action") is True


def _source_readiness_unavailable_evidence(
    *,
    output_dir: Path,
    generated_at_ms: int,
    selected_strategy_group_id: str | None,
    ...
    deploy_channel = _deploy_channel_status_evidence(output_dir)
    ...
    real_order_readiness = _real_order_readiness_status_evidence(goal_status)
    source_health = {
    ...
    return {
        ...
        "owner_state": {
            ...
            "non_authority_checkpoint": owner_non_authority_checkpoint(
                goal_status,
                goal_status.get("owner_state") or {},
                default="restore_owner_console_readmodel_api",
            ),
        },
        "owner_summary": {
        ...
        "safety_invariants": {
            ...
            "source_readiness_unavailable_evidence_only": True,
            "places_order": False,
...
def _deploy_channel_status_evidence(output_dir: Path) -> dict[str, Any]:
...
def refresh_product_state_artifacts(
    *,
    api_base: str,
    output_dir: Path,
    label: str,
    ...
    refreshed_dry_run_artifact: dict[str, Any] | None = None
    ...
    if refresh_dry_run_audit_chain:
        ...
        builder = dry_run_builder or build_audit_artifact
        ...
        try:
            dry_run_artifact = builder(resolved_dry_run_output_dir)
            refreshed_dry_run_artifact = dry_run_artifact
            _write_json(resolved_dry_run_output_json, dry_run_artifact)
            ...
            if (
                ...
                _write_json(goal_status_dry_run_json, dry_run_artifact)
            dry_run_audit_refresh = {
                "enabled": True,
                "status": dry_run_artifact.get("status"),
                ...
                "scenario_count": dry_run_artifact.get("scenario_count"),
                "dangerous_effects_absent": (
                    (dry_run_artifact.get("checks") or {}).get(
                        "dangerous_effects_absent"
                    )
                    is True
                ),
    ...
    if refresh_chain_closure_status:
        ...
        try:
            ...
            if audit_artifact is None:
                ...
                audit_artifact = _read_json_if_exists(audit_source_json)
            if not audit_artifact:
                ...
                closure_status_artifact = builder(audit_artifact=audit_artifact)
                _write_json(
                    resolved_chain_closure_output_json, closure_status_artifact
                )
                chain_closure_status_refresh = {
                    "enabled": True,
                    "status": closure_status_artifact.get("status"),
                    "output_json": str(resolved_chain_closure_output_json),
                    "audit_json": str(audit_source_json) if audit_source_json else None,
                    "real_order_allowed": (
                        (closure_status_artifact.get("real_execution") or {}).get(
                            "real_order_allowed"
                        )
                        is True
                    ),
                ...
                if (
                    closure_status_artifact.get("status")
    ...
    if refresh_live_closure_evidence:
        from scripts.refresh_runtime_live_closure_evidence_artifacts import (
    ...
    source_readiness_unavailable_evidence: dict[str, Any] = {
        "enabled": False,
        "status": "skipped",
    }
    ...
    if refresh_api_readmodels and cookie is None:
        ...
        _write_json(evidence_path, unavailable_evidence)
        source_readiness_unavailable_evidence = {
            "enabled": True,
            "status": unavailable_evidence.get("status"),
            "output_json": str(evidence_path),
            "reason": "operator_cookie_unavailable",
            "goal_status_included": bool(
                unavailable_evidence.get("raw_status_refs", {}).get(
    ...
    return {
        ...
        "source_readiness_unavailable_evidence": source_readiness_unavailable_evidence,
        "blockers": blockers,
        "warnings": warnings,
        "selected_scope_config": {
            "selected_strategy_group_id": selected_strategy_group_id,
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Refresh StrategyGroup runtime product-state readmodel artifacts.",
    )
...
def _degraded_local_refresh_is_continuable(artifact: dict[str, Any]) -> bool:
    if artifact.get("status") != "refresh_blocked":
        return False
    ...
    if not isinstance(unavailable_evidence, dict):
        return False
    if unavailable_evidence.get("enabled") is not True:
        return False
    if unavailable_evidence.get("reason") != "operator_cookie_unavailable":
        return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        and _degraded_local_refresh_is_continuable(artifact)
    ):
        exit_code = 0
        artifact["cli_exit_policy"] = {
